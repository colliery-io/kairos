...
- **Auth**: Authorization Code + PKCE against Keycloak (A-0010); access token held in memory, silent refresh; unauthenticated hits show the login redirect
  - *Amendment (2026-08-09, KAIROS-T-0071, approved by Dylan)*: the **refresh token** is persisted in `sessionStorage` (per-tab, cleared on tab close — not localStorage, not a cookie) so a page reload restores the session via a silent refresh grant before the login redirect fires. Motivation: "reload = re-login" was acceptable only when the IdP holds an SSO session; the dev Dex password connector has none, making every reload a full login form. The access token remains memory-only; logout and any failed refresh remove the stored token. Threat-model delta is minimal: an XSS able to read sessionStorage could equally exfiltrate the in-memory token, and sessionStorage adds no offline persistence.
