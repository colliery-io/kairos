...
- 2026-08-25 (post-completion review fix): PO found the add-a-field picker dead in UAT. Root cause (pre-existing T-0065 bug, first exercised now — no test ever ADDED a field): the picker's Effect reset the signal it watched, re-entering the running effect → reactive_graph panic → the whole WASM app died silently on first pick. First fix attempt (promotion inside the change handler) surfaced bug #2: a draft RwSignal created inside a DOM event handler (no reactive owner) is dead — the editor's writes never propagated, Save never enabled. Final shape: MetadataForm pre-creates EVERY definition's FieldRow (and signal) at render time; a `visible: RwSignal<Vec<slug>>` drives both the row list and the picker options; on_pick only pushes a slug and resets the select node to its placeholder (web-sys HtmlSelectElement feature added). API layer verified unaffected throughout (curl PATCH worked all along). NEW permanent regression spec e2e/tests/metadata.spec.ts (add→save→persist + stamped re-edit). Diagnosed headlessly with scratch Playwright specs against the live UAT server.