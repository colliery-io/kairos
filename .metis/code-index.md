...
└── uat/
    ...
    │   ├── incident.journey.ts
    │   ├── local-login.journey.ts
    │   ├── machine-access.journey.ts
    │   ├── multi-repo-agent.journey.ts
...
- pub `SearchArgs` struct L21-102 — `{ query: Option<String>, entity_type: Vec<String>, board: Option<String>, column...` — Search and traverse all entity types (POST /api/search).
- pub `build_request` function L134-215 — `(&self) -> Result<SearchRequest, CliError>` — Compose the S-0005 request body from the flags (or take
- pub `run` function L274-283 — `(self) -> Result<(), CliError>` — (KAIROS-A-0007 / KAIROS-T-0037).
-  `SearchArgs` type L104-284 — `= SearchArgs` — (KAIROS-A-0007 / KAIROS-T-0037).
-  `has_flag_query` function L107-130 — `(&self) -> bool` — Whether any flag other than `--query-json` (and the pagination
-  `parse_metadata` function L218-232 — `(&self) -> Result<Option<BTreeMap<String, String>>, CliError>` — `--metadata k=v` pairs into the S-0005 metadata map.
-  `build_traverse` function L237-272 — `(&self) -> Result<Option<SearchTraverse>, CliError>` — The traverse clause: `--from`/`--from-id` anchor it; the companion
-  `non_empty` function L286-288 — `(values: &[String]) -> Option<Vec<String>>` — (KAIROS-A-0007 / KAIROS-T-0037).
-  `print_results` function L295-329 — `(response: &SearchResponse)` — The human rendering: one CODE/TITLE/VER section per non-empty group.
-  `section` function L296-314 — `(label: &str, items: &[T])` — (KAIROS-A-0007 / KAIROS-T-0037).
-  `tests` module L332-482 — `-` — (KAIROS-A-0007 / KAIROS-T-0037).
-  `flags_compose_the_s0005_body` function L338-375 — `()` — The filter flags compose the S-0005 body: metadata k=v parsing,
-  `traverse_flags` function L381-419 — `()` — Traversal flags: anchored by --from, direction defaults to
-  `repo_alone_is_a_filter` function L426-437 — `()` — `kairos search --repo <slug>` on its own is a complete request
-  `query_json_escape_hatch` function L443-481 — `()` — --query-json is the verbatim escape hatch: parsed as the full
...
- pub `discover_issuer` function L145-195 — `( http: &reqwest::Client, deployment_url: &str, ) -> Result<String, CliError>` — Discover the deployment's OIDC issuer from its RFC 9728
- pub `discover_endpoints` function L198-240 — `( http: &reqwest::Client, issuer: &str, ) -> Result<IssuerEndpoints, CliError>` — OIDC discovery: the issuer's token + device-authorization endpoints.
- pub `start_device_grant` function L243-270 — `( http: &reqwest::Client, endpoints: &IssuerEndpoints, client_id: &str, ) -> Res...` — Start the Device Authorization Grant (RFC 8628 §3.1).
- pub `poll_device_grant` function L274-324 — `( http: &reqwest::Client, endpoints: &IssuerEndpoints, client_id: &str, grant: &...` — Poll the token endpoint until the user approves, declines, or the code
- pub `refresh_grant` function L328-353 — `( http: &reqwest::Client, token_endpoint: &str, client_id: &str, refresh_token: ...` — The refresh grant (RFC 6749 §6).
...
-  `Discovery` struct L218-222 — `{ token_endpoint: String, device_authorization_endpoint: Option<String> }` — 4.
-  `tests` module L356-442 — `-` — 4.
-  `poll_classification` function L361-412 — `()` — Every RFC 8628 poll outcome classifies correctly.
-  `token_response_without_refresh` function L417-421 — `()` — A missing refresh_token in a token response stays None (some IdPs
-  `bearer_selection` function L426-441 — `()` — `bearer_for` picks the right token; `id_token` mode fails cleanly when
...
-  `cli_login_whoami_refresh_logout_live` function L152-448 — `()` — credentials to the form's URL, landing on `/device/callback`.
...
-  `cli_command_tree_golden_path_live` function L175-651 — `()` — members list → orgs show → teams list.
...
- pub `CreateTaskRequest` struct L264-296 — `{ board_id: Option<String>, column_id: Option<String>, title: String, content: S...` — Body of `POST /api/tasks`.
- pub `SetLifecycleRequest` struct L304-307 — `{ lifecycle: String }` — Body of `POST /api/tasks/{short_code}/work-class` (KAIROS-T-0077): move
- pub `SetWorkClassRequest` struct L311-314 — `{ work_class: String }` — transitions â the board rules engine is never consulted.
- pub `CreateDocumentRequest` struct L321-334 — `{ title: String, content: Option<String>, template_id: Option<String>, parent_sh...` — Body of `POST /api/documents`.
- pub `CreateAdrRequest` struct L340-357 — `{ board_id: Option<String>, column_id: Option<String>, title: String, content: S...` — Body of `POST /api/adrs`.
- pub `UpdateContentRequest` struct L364-372 — `{ title: Option<String>, content: String, version: i32 }` — Body of `PATCH /api/{family}/{short_code}` â the KAIROS-A-0004
- pub `TransitionRequest` struct L376-381 — `{ to_column_id: String }` — Body of `POST /api/{family}/{short_code}/transition`.
- pub `MoveTaskRequest` struct L387-389 — `{ board: String }` — Body of `POST /api/tasks/{short_code}/move` (KAIROS-I-0012): the
- pub `ListEnvelope` struct L397-407 — `{ items: Vec<T>, total: i64, limit: i64, offset: i64 }` — The S-0005 list envelope: `{items, total, limit, offset}`.
- pub `Pagination` struct L413-420 — `{ limit: Option<i64>, offset: Option<i64> }` — `?limit=&offset=` pagination for list endpoints (S-0005: the only list
- pub `ListQuery` struct L435-448 — `{ limit: Option<i64>, offset: Option<i64>, include_deleted: bool }` — `?limit=&offset=&include_deleted=` â the query of the five entity
- pub `including_archived` function L453-458 — `() -> Self` — The archived-inclusive whole-family listing.
- pub `DeleteResponse` struct L474-481 — `{ short_code: String, cascade_count: i64, cascaded_short_codes: Vec<String> }` — Response of `DELETE /api/{family}/{short_code}` â the soft delete and
- pub `RestoreResponse` struct L491-498 — `{ short_code: String, still_archived_count: i64, still_archived_short_codes: Vec...` — Response of `POST /api/{entity_type}/{short_code}/restore`
- pub `CascadePreviewResponse` struct L508-516 — `{ short_code: String, cascade_count: i64, cascaded_short_codes: Vec<String> }` — Response of `GET /api/{entity_type}/{short_code}/cascade-preview`
- pub `ErrorEnvelope` struct L520-522 — `{ error: ErrorBody }` — The S-0005 error envelope: `{"error": {"code", "message", "details"}}`.
- pub `ErrorBody` struct L526-537 — `{ code: String, message: String, details: serde_json::Value }` — The `error` object of [`ErrorEnvelope`].
-  `ListQuery` type L450-459 — `= ListQuery` — `kairos-server` (`api::convert`), keeping this crate free of diesel.
-  `ListQuery` type L461-469 — `= ListQuery` — `kairos-server` (`api::convert`), keeping this crate free of diesel.
-  `from` function L462-468 — `(page: Pagination) -> Self` — `kairos-server` (`api::convert`), keeping this crate free of diesel.
...
- pub `RelatedProposal` struct L286-299 — `{ short_code: String, title: String, entity_type: String, claim: String, score: ...` — One related-work **proposal**: a claim, its evidence, and nothing asserted.
- pub `RelatedWorkResponse` struct L303-318 — `{ short_code: String, proposals: Vec<RelatedProposal>, lexical: bool, vector: bo...` — The answer to "what is related to this?".
- pub `EdgeProposalDto` struct L323-341 — `{ id: String, source: String, target: String, relationship: String, state: Strin...` — A proposed graph edge awaiting a human (KAIROS-A-0021 rule 6,
...
- pub `ADMINISTER_MEMBERS` variable L73 — `: &str` — Add/remove users from the board, grant/revoke capabilities.
- pub `GLOB_ALL` variable L76 — `: &str` — Glob: all capabilities on the board (full access).
- pub `GLOB_MANAGE` variable L80 — `: &str` — Glob: all `manage_<entity>` capabilities — and, since KAIROS-T-0183
- pub `GLOB_CONFIGURE` variable L86 — `: &str` — Glob: all `configure_*` capabilities — currently just
- pub `GLOB_TRANSITION` variable L89 — `: &str` — Glob: all `transition_*` capabilities (currently just
- pub `CAPABILITIES` variable L92-101 — `: &[&str]` — Every specific (non-glob) capability in the A-0006 vocabulary.
- pub `GLOBS` variable L104 — `: &[&str]` — Every sanctioned glob form (trailing-`*` only, per A-0006).
- pub `TEAM_IMPLIED_CAPABILITIES` variable L111 — `: &[&str]` — The capabilities IMPLIED by membership of a board's owning team
- pub `team_implies` function L117-119 — `(required: &str) -> bool` — Does membership of the board's owning team satisfy `required` on its
- pub `FILE_BACKLOG` variable L128 — `: &str` — Cross-team Backlog filing (KAIROS-T-0105, A-0019 §4 amending A-0006):
- pub `COMPUTED_CAPABILITIES` variable L133 — `: &[&str]` — Every capability that is COMPUTED rather than granted — never in the
- pub `COLLABORATIVE_RELATIONSHIPS` variable L144 — `: &[&str]` — Relationship types a NON-admin may write between items they can
- pub `is_collaborative_relationship` function L147-149 — `(relationship: &str) -> bool` — May a non-admin write `relationship` (see [`COLLABORATIVE_RELATIONSHIPS`])?
- pub `TenantConfigResource` enum L159-166 — `Templates | MetadataDefinitions | Relationships` — Tenant-wide configuration resources that live on no board.
- pub `org_admin_only` function L174-176 — `(self) -> bool` — Whether writes to this resource require `organization_members.role =
- pub `capability_matches` function L192-199 — `(granted: &str, required: &str) -> bool` — Does the stored grant `granted` satisfy the `required` capability?
- pub `is_authorized` function L227-231 — `(grants: &[String], required: &str) -> bool` — The A-0006 decision over a user's loaded grants for one board: allowed
-  `TenantConfigResource` type L168-177 — `= TenantConfigResource` — grant time by API-layer validation, not relied upon.
-  `glob_match` function L203-222 — `(pattern: &str, text: &str) -> bool` — LIKE-style match: `*` in `pattern` matches any (possibly empty) sequence;
-  `tests` module L234-472 — `-` — grant time by API-layer validation, not relied upon.
-  `exact_capability_matches_itself_only` function L240-252 — `()` — grant time by API-layer validation, not relied upon.
-  `bare_star_grants_every_capability` function L257-264 — `()` — grant time by API-layer validation, not relied upon.
-  `manage_glob_matches_manage_capabilities_only` function L269-288 — `()` — grant time by API-layer validation, not relied upon.
-  `manage_glob_does_not_confer_board_administration` function L302-317 — `()` — KAIROS-T-0183, asserted BY NAME rather than left to be derived from the
-  `configure_and_transition_globs` function L320-332 — `()` — grant time by API-layer validation, not relied upon.
-  `non_matching_prefixes_are_rejected` function L335-341 — `()` — grant time by API-layer validation, not relied upon.
-  `empty_strings_mirror_sql_like` function L346-356 — `()` — grant time by API-layer validation, not relied upon.
-  `stored_percent_is_literal_not_wildcard` function L361-369 — `()` — grant time by API-layer validation, not relied upon.
-  `stored_underscore_is_literal_not_single_char_wildcard` function L372-378 — `()` — grant time by API-layer validation, not relied upon.
-  `stored_backslash_is_literal` function L381-386 — `()` — grant time by API-layer validation, not relied upon.
-  `embedded_star_wildcards_like_the_sql_translation` function L389-402 — `()` — grant time by API-layer validation, not relied upon.
-  `is_authorized_is_a_whitelist_over_grants` function L407-418 — `()` — grant time by API-layer validation, not relied upon.
-  `tenant_config_resources_are_org_admin_only` function L423-431 — `()` — grant time by API-layer validation, not relied upon.
-  `team_implies_only_the_delivery_set` function L437-450 — `()` — KAIROS-T-0072: team membership implies exactly the day-to-day
-  `file_backlog_is_computed_never_grantable_never_team_implied` function L453-471 — `()` — grant time by API-layer validation, not relied upon.
...
#### crates/kairos-core/src/chunk.rs

- pub `Chunk` struct L41-56 — `{ ordinal: usize, heading: Option<String>, char_start: usize, char_end: usize, t...` — One chunk of an item's content.
- pub `len_chars` function L60-62 — `(&self) -> usize` — Length in characters.
- pub `ChunkConfig` struct L67-78 — `{ max_chars: usize, window: usize, overlap: usize }` — Chunking sizes.
- pub `chunk` function L102-104 — `(content: &str) -> Vec<Chunk>` — Split `content` on heading boundaries, with a sliding-window fallback for
- pub `chunk_with` function L107-142 — `(content: &str, config: &ChunkConfig) -> Vec<Chunk>` — [`chunk`] with explicit sizes.
-  `Chunk` type L58-63 — `= Chunk` — astral-plane characters needs to convert rather than index directly.
-  `ChunkConfig` type L80-88 — `impl Default for ChunkConfig` — astral-plane characters needs to convert rather than index directly.
-  `default` function L81-87 — `() -> Self` — astral-plane characters needs to convert rather than index directly.
-  `ChunkConfig` type L90-98 — `= ChunkConfig` — astral-plane characters needs to convert rather than index directly.
-  `stride` function L92-97 — `(&self) -> usize` — The distance between window starts.
-  `push` function L144-153 — `(out: &mut Vec<Chunk>, heading: Option<String>, start: usize, text: String)` — astral-plane characters needs to convert rather than index directly.
-  `Section` struct L155-159 — `{ heading: Option<String>, start: usize, text: String }` — astral-plane characters needs to convert rather than index directly.
-  `split_sections` function L162-192 — `(content: &str) -> Vec<Section>` — Split into sections at ATX heading lines, keeping each section's start offset.
-  `split_keeping_newlines` function L195-208 — `(content: &str) -> Vec<&str>` — Lines with their terminators kept, so character offsets stay exact.
-  `heading_text` function L216-230 — `(line: &str) -> Option<String>` — The text of an ATX heading line, or `None` if the line is not one.
-  `tests` module L233-492 — `-` — astral-plane characters needs to convert rather than index directly.
-  `headings` function L236-238 — `(chunks: &[Chunk]) -> Vec<Option<&str>>` — astral-plane characters needs to convert rather than index directly.
-  `assert_offsets_are_exact` function L242-252 — `(content: &str, chunks: &[Chunk])` — Offsets must point back into the original content exactly, or a citation
-  `ordinals_are_contiguous_from_zero` function L255-261 — `()` — astral-plane characters needs to convert rather than index directly.
-  `sections_split_on_headings_and_keep_the_literal_text` function L264-274 — `()` — astral-plane characters needs to convert rather than index directly.
-  `template_markers_in_headings_are_preserved_verbatim` function L280-287 — `()` — The template markers the corpus is full of must survive verbatim — 1,439
-  `content_before_the_first_heading_has_no_heading` function L290-297 — `()` — astral-plane characters needs to convert rather than index directly.
-  `content_with_no_headings_at_all_is_windowed` function L301-307 — `()` — The rule-3 fallback.
-  `a_short_headingless_document_is_one_chunk` function L310-316 — `()` — astral-plane characters needs to convert rather than index directly.
-  `an_oversized_section_is_windowed_but_keeps_its_heading` function L319-328 — `()` — astral-plane characters needs to convert rather than index directly.
-  `windows_overlap_by_the_configured_amount` function L332-346 — `()` — Windows overlap so a sentence crossing a boundary is intact somewhere.
-  `the_final_window_ends_exactly_at_the_end` function L350-361 — `()` — The last window must not be dropped, and must not run past the end.
-  `an_overlap_at_least_as_big_as_the_window_still_terminates` function L365-374 — `()` — A pathological configuration must terminate rather than allocate forever.
-  `empty_sections_are_dropped` function L377-381 — `()` — astral-plane characters needs to convert rather than index directly.
-  `empty_and_whitespace_content_produce_no_chunks` function L384-387 — `()` — astral-plane characters needs to convert rather than index directly.
-  `all_six_heading_levels_are_recognised` function L390-396 — `()` — astral-plane characters needs to convert rather than index directly.
-  `seven_hashes_is_not_a_heading` function L399-403 — `()` — astral-plane characters needs to convert rather than index directly.
-  `hashes_without_a_space_are_not_a_heading` function L408-412 — `()` — `#hashtag` is not a heading.
-  `a_closing_hash_sequence_is_not_part_of_the_heading` function L415-418 — `()` — astral-plane characters needs to convert rather than index directly.
-  `setext_underlines_and_thematic_breaks_do_not_split` function L423-428 — `()` — `---` is a thematic break and a front-matter fence as often as it is a
-  `offsets_count_characters_not_bytes` function L433-440 — `()` — Offsets are characters, not bytes: a citation into a document with
-  `crlf_line_endings_are_handled` function L443-448 — `()` — astral-plane characters needs to convert rather than index directly.
-  `a_realistic_document_chunks_the_way_the_corpus_did` function L453-491 — `()` — The shape the measurement found: every real document has headings, most
...
- pub `chunk` module L9 — `-` — `kairos-db` and `kairos-server`.
- pub `forge` module L10 — `-` — `kairos-db` and `kairos-server`.
- pub `graph` module L11 — `-` — `kairos-db` and `kairos-server`.
- pub `items` module L12 — `-` — `kairos-db` and `kairos-server`.
- pub `primary` module L13 — `-` — `kairos-db` and `kairos-server`.
- pub `repositories` module L14 — `-` — `kairos-db` and `kairos-server`.
- pub `retention` module L15 — `-` — `kairos-db` and `kairos-server`.
- pub `retrieval` module L16 — `-` — `kairos-db` and `kairos-server`.
- pub `search` module L17 — `-` — `kairos-db` and `kairos-server`.
- pub `short_code` module L18 — `-` — `kairos-db` and `kairos-server`.

#### crates/kairos-core/src/primary.rs

- pub `PrimaryInputs` struct L81-98 — `{ title: &'a str, repository: Option<&'a str>, team: Option<&'a str>, parent_tit...` — The structural facts about an item that go into its primary vector.
- pub `PrimaryConfig` struct L102-108 — `{ prose_chars: usize }` — How much of each input to use.
- pub `primary_text` function L117-119 — `(item_type: ItemType, inputs: &PrimaryInputs<'_>) -> String` — Compose the text whose embedding is the item's primary vector.
- pub `primary_text_with` function L127-159 — `( item_type: ItemType, inputs: &PrimaryInputs<'_>, config: &PrimaryConfig, ) -> ...` — [`primary_text`] with an explicit budget.
- pub `content_hash` function L400-412 — `(text: &str) -> String` — SHA-256 of the exact text that was embedded, as lowercase hex.
-  `PrimaryConfig` type L110-114 — `impl Default for PrimaryConfig` — emphatic.
-  `default` function L111-113 — `() -> Self` — emphatic.
-  `opening_prose` function L167-189 — `(content: &str, budget: usize) -> Option<String>` — The item's opening prose, up to `budget` characters.
-  `non_blank` function L191-193 — `(value: Option<&str>) -> Option<&str>` — emphatic.
-  `type_label` function L200-208 — `(item_type: ItemType) -> &'static str` — How an entity type is named to the model.
-  `tests` module L211-387 — `-` — emphatic.
-  `meta` function L214-219 — `(pairs: &[(&str, &str)]) -> Vec<(String, String)>` — emphatic.
-  `the_title_and_type_lead` function L222-232 — `()` — emphatic.
-  `every_structural_input_appears` function L235-257 — `()` — emphatic.
-  `absent_and_blank_inputs_leave_no_line` function L263-278 — `()` — Absent inputs must leave no trace — a dangling `repository:` line would
-  `composition_is_stable` function L283-295 — `()` — Stability is what makes content hashing able to skip work: the same inputs
-  `prose_is_bounded_and_does_not_end_mid_word` function L298-317 — `()` — emphatic.
-  `the_opening_skips_empty_sections` function L323-335 — `()` — The opening comes from the chunker, so a document whose first section is a
-  `content_that_is_only_headings_contributes_nothing` function L338-345 — `()` — emphatic.
-  `a_zero_budget_omits_prose_entirely` function L348-356 — `()` — emphatic.
-  `every_type_has_a_stable_label` function L361-371 — `()` — The labels are part of the embedded text, so they are asserted rather than
-  `prose_budget_counts_characters_not_bytes` function L374-386 — `()` — emphatic.
-  `hash_tests` module L415-435 — `-` — emphatic.
-  `it_is_the_known_sha256_of_the_empty_string` function L419-426 — `()` — emphatic.
-  `it_is_stable_and_sensitive` function L429-434 — `()` — emphatic.
...
#### crates/kairos-core/src/retrieval.rs

- pub `Claim` enum L51-61 — `ImplicitDependency | NearDuplicate | PriorArt` — What a proposal is claiming.
- pub `label` function L65-71 — `(self) -> &'static str` — The word a caller should read first.
- pub `GraphRelation` struct L87-96 — `{ directly_linked: bool, shared_parent: bool, same_repository: bool, same_board:...` — What the graph says about a candidate, relative to the item asked about.
- pub `Candidate` struct L100-117 — `{ short_code: String, title: String, entity_type: String, lexical_rank: Option<u...` — One item that might be worth telling the caller about.
- pub `Proposal` struct L121-135 — `{ short_code: String, title: String, entity_type: String, claim: Claim, score: f...` — A bounded, cited claim about one item.
- pub `RetrievalConfig` struct L139-161 — `{ limit: usize, rrf_k: f32, repository_weight: f32, board_weight: f32 }` — How to fuse and bound.
- pub `Sources` struct L177-182 — `{ lexical: bool, vector: bool }` — Which sources actually contributed, so a caller can tell a thin answer from a
- pub `degraded` function L189-191 — `(&self) -> bool` — Whether the answer is lexical-only, which is a **degraded** answer rather
- pub `note` function L194-204 — `(&self) -> &'static str` — A sentence for the response.
- pub `propose` function L212-255 — `( candidates: &[Candidate], sources: Sources, config: &RetrievalConfig, ) -> Vec...` — Fuse candidates into bounded proposals, best first.
- pub `graph_caveat` function L315-324 — `(proposals: &[Proposal]) -> Option<&'static str>` — The sentence that keeps "nothing joins them" honest, said once per response.
-  `Claim` type L63-72 — `= Claim` — *might*.
-  `Claim` type L74-78 — `= Claim` — *might*.
-  `fmt` function L75-77 — `(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result` — *might*.
-  `RetrievalConfig` type L163-172 — `impl Default for RetrievalConfig` — *might*.
-  `default` function L164-171 — `() -> Self` — *might*.
-  `Sources` type L184-205 — `= Sources` — *might*.
-  `classify` function L262-270 — `(c: &Candidate) -> Claim` — Which claim a candidate supports.
-  `explain` function L278-307 — `(c: &Candidate, claim: Claim, sources: Sources) -> String` — The sentence behind a claim.
-  `tests` module L327-600 — `-` — *might*.
-  `candidate` function L330-341 — `(code: &str) -> Candidate` — *might*.
-  `both` function L343-348 — `() -> Sources` — *might*.
-  `agreement_between_sources_beats_a_single_strong_hit` function L353-365 — `()` — Appearing in both lists must beat appearing at the top of one.
-  `already_linked_items_are_not_proposed` function L368-382 — `()` — *might*.
-  `the_three_claims_are_classified_by_the_disagreement` function L385-409 — `()` — *might*.
-  `prior_art_outranks_near_duplicate_when_both_apply` function L414-428 — `()` — A finished sibling is prior art, not a duplicate: being already done is
-  `repository_breaks_ties_without_filtering` function L434-463 — `()` — Repository is a weight.
-  `the_priors_are_smaller_than_the_gap_between_adjacent_ranks` function L469-481 — `()` — The calibration itself, pinned.
-  `results_are_bounded_and_ordered_deterministically` function L484-494 — `()` — *might*.
-  `ties_fall_back_to_short_code` function L499-510 — `()` — Identical scores must still produce one order, or the same question gets
-  `a_degraded_answer_admits_it` function L514-531 — `()` — Rule 7: a lexical-only answer says so, in the result itself.
-  `a_response_resting_on_absent_edges_carries_the_caveat_once` function L537-551 — `()` — The honesty requirement: "no edge" must not read as "the graph was
-  `the_caveat_is_absent_when_no_claim_rests_on_an_absent_edge` function L556-565 — `()` — And it is absent when nothing rests on it, so a response of pure prior art
-  `the_matched_heading_is_cited_verbatim` function L568-579 — `()` — *might*.
-  `no_candidates_is_an_empty_answer_not_an_error` function L582-584 — `()` — *might*.
-  `scoring_needs_no_similarity_value_at_all` function L591-599 — `()` — Nothing anywhere compares a similarity to a constant.

#### crates/kairos-core/src/search.rs

- pub `MAX_TRAVERSE_DEPTH` variable L51 — `: u32` — Server cap on `traverse.depth` (A-0007: "server-capped to prevent
- pub `MAX_LIMIT` variable L54 — `: i64` — Server cap on `limit`.
- pub `DEFAULT_LIMIT` variable L58 — `: i64` — Default `limit` when the request omits it (the S-0005 examples' page
- pub `SearchRequest` struct L69-89 — `{ q: Option<String>, filter: Option<SearchFilter>, traverse: Option<Traverse>, s...` — The `POST /api/search` request body (KAIROS-A-0007 / S-0005).
- pub `effective_limit` function L93-95 — `(&self) -> i64` — The page size to use: `limit` or [`DEFAULT_LIMIT`].
- pub `effective_offset` function L98-100 — `(&self) -> i64` — The offset to use: `offset` or 0.
- pub `effective_sort` function L109-118 — `(&self) -> Sort` — The sort to use: `sort` if given, else the default for this request.
- pub `SearchFilter` struct L125-171 — `{ entity_type: Option<Vec<SearchEntityType>>, board_id: Option<Uuid>, column_id:...` — The `filter` capability (A-0007: fields are AND with each other; array
- pub `is_constraining` function L183-196 — `(&self) -> bool` — Whether this filter says anything at all — the at-least-one-capability
- pub `SearchEntityType` enum L202-213 — `Strategy | Initiative | Task | Document | Adr` — The `filter.entity_type` vocabulary (the five S-0004 entity tables).
- pub `item_type` function L217-225 — `(self) -> ItemType` — The corresponding [`ItemType`].
- pub `SearchTaskType` enum L231-240 — `Task | Bug | TechDebt | Support` — The `filter.task_type` vocabulary (`tasks.task_type` CHECK set).
- pub `as_str` function L244-251 — `(self) -> &'static str` — The TEXT value stored in `tasks.task_type`.
- pub `SearchWorkClass` enum L257-262 — `Planned | Support` — A `tasks.work_class` lane in a search filter (KAIROS-T-0077).
- pub `as_str` function L266-271 — `(self) -> &'static str` — The TEXT value stored in `tasks.work_class`.
- pub `Traverse` struct L278-292 — `{ from: TraverseFrom, relationships: Vec<SearchRelationship>, direction: Directi...` — The `traverse` capability (A-0007: recursive walk of
- pub `TraverseFrom` struct L298-305 — `{ short_code: Option<String>, id: Option<Uuid> }` — `traverse.from`: exactly one of `short_code`/`id` (enforced by
- pub `SearchRelationship` enum L311-322 — `Parent | Supports | Informs | Supersedes | Blocks` — The `traverse.relationships` vocabulary (`item_relationships.
- pub `as_str` function L326-334 — `(self) -> &'static str` — The TEXT value stored in `item_relationships.relationship`.
- pub `Direction` enum L340-347 — `Outbound | Inbound | Both` — `traverse.direction` (A-0007).
- pub `Sort` struct L353-358 — `{ field: SortField, order: SortOrder }` — The `sort` clause; applies to the combined cross-type result set before
- pub `SortField` enum L364-375 — `CreatedAt | UpdatedAt | Title | Relevance` — Sortable fields — attributes every entity type carries, so the combined
- pub `requires_query` function L379-381 — `(&self) -> bool` — Whether this field needs `q` present to mean anything.
- pub `SortOrder` enum L387-392 — `Asc | Desc` — Sort direction.
- pub `SearchValidationError` enum L400-466 — `NoCapability | BlankQuery | EmptyEntityTypes | EmptyTaskTypes | EmptyWorkClasses...` — A structurally invalid search request (HTTP 400 at the API layer).
- pub `validate` function L471-559 — `(request: &SearchRequest) -> Result<(), SearchValidationError>` — Validate a [`SearchRequest`] against the module-docs contract.
- pub `metadata_like_pattern` function L569-581 — `(value: &str) -> String` — Translate a metadata filter value into a SQL LIKE pattern (module docs):
-  `SearchRequest` type L91-119 — `= SearchRequest` — translates to a LIKE pattern equivalent to equality.
-  `SearchFilter` type L173-197 — `= SearchFilter` — translates to a LIKE pattern equivalent to equality.
-  `SearchEntityType` type L215-226 — `= SearchEntityType` — translates to a LIKE pattern equivalent to equality.
-  `SearchTaskType` type L242-252 — `= SearchTaskType` — translates to a LIKE pattern equivalent to equality.
-  `SearchWorkClass` type L264-272 — `= SearchWorkClass` — translates to a LIKE pattern equivalent to equality.
-  `SearchRelationship` type L324-335 — `= SearchRelationship` — translates to a LIKE pattern equivalent to equality.
-  `SortField` type L377-382 — `= SortField` — translates to a LIKE pattern equivalent to equality.
-  `tests` module L584-1071 — `-` — translates to a LIKE pattern equivalent to equality.
-  `q` function L587-592 — `(text: &str) -> SearchRequest` — translates to a LIKE pattern equivalent to equality.
-  `traverse` function L594-604 — `(depth: Option<u32>) -> Traverse` — translates to a LIKE pattern equivalent to equality.
-  `empty_request_is_no_capability` function L609-614 — `()` — translates to a LIKE pattern equivalent to equality.
-  `empty_filter_is_no_capability` function L617-633 — `()` — translates to a LIKE pattern equivalent to equality.
-  `include_deleted_alone_is_a_capability` function L641-652 — `()` — KAIROS-T-0157 / KAIROS-A-0020: `include_deleted` on its own is a
-  `each_capability_alone_is_valid` function L655-670 — `()` — translates to a LIKE pattern equivalent to equality.
-  `blank_q_is_rejected_even_alongside_other_capabilities` function L673-679 — `()` — translates to a LIKE pattern equivalent to equality.
-  `empty_or_lists_are_rejected` function L684-718 — `()` — translates to a LIKE pattern equivalent to equality.
-  `work_class_filter_alone_is_a_capability` function L723-732 — `()` — KAIROS-T-0077: a work_class-only filter IS constraining — it must
-  `blank_metadata_keys_are_rejected` function L735-747 — `()` — translates to a LIKE pattern equivalent to equality.
-  `date_ranges_must_be_sane` function L750-787 — `()` — translates to a LIKE pattern equivalent to equality.
-  `traverse_depth_is_required_and_capped` function L792-811 — `()` — translates to a LIKE pattern equivalent to equality.
-  `traverse_from_names_exactly_one_reference` function L814-842 — `()` — translates to a LIKE pattern equivalent to equality.
-  `traverse_relationships_must_be_non_empty` function L845-857 — `()` — translates to a LIKE pattern equivalent to equality.
-  `limit_is_capped_and_offset_non_negative` function L862-891 — `()` — translates to a LIKE pattern equivalent to equality.
-  `effective_defaults` function L894-898 — `()` — translates to a LIKE pattern equivalent to equality.
-  `default_sort_is_relevance_only_when_there_is_a_query` function L903-918 — `()` — KAIROS-T-0186: the default sort depends on whether there is a query to
-  `explicit_sort_overrides_the_default` function L923-934 — `()` — An explicit sort is always honoured, including where it differs from the
-  `relevance_without_a_query_is_rejected` function L940-958 — `()` — Relevance to nothing is not a thing: it is a 400, not a quiet fallback
-  `relevance_round_trips_as_snake_case` function L961-966 — `()` — translates to a LIKE pattern equivalent to equality.
-  `full_s0005_request_deserializes` function L971-1019 — `()` — translates to a LIKE pattern equivalent to equality.
-  `unknown_vocabulary_is_rejected_by_serde` function L1022-1052 — `()` — translates to a LIKE pattern equivalent to equality.
-  `metadata_like_pattern_translates_globs_and_escapes_metacharacters` function L1057-1070 — `()` — translates to a LIKE pattern equivalent to equality.
...
### crates/kairos-core/tests

> *Semantic summary to be generated by AI agent.*

#### crates/kairos-core/tests/corpus_agreement.rs

-  `WINDOWED_FRACTION` variable L42 — `: f64` — What `scripts/corpus/extract.py` reported on 2026-09-24, for provenance:
-  `WINDOWED_TOLERANCE` variable L43 — `: f64` — written down in KAIROS-T-0190's status updates with the date they were taken.
-  `corpus_root` function L45-53 — `() -> Option<PathBuf>` — written down in KAIROS-T-0190's status updates with the date they were taken.
-  `documents` function L66-110 — `(root: &Path) -> Vec<String>` — Every Metis document under `root`, matching the Python extractor's rules:
-  `split_frontmatter` function L112-116 — `(text: &str) -> Option<(&str, &str)>` — written down in KAIROS-T-0190's status updates with the date they were taken.
-  `sections` function L126-134 — `(body: &str) -> Vec<kairos_core::chunk::Chunk>` — The document's sections before any windowing.
-  `the_rust_chunker_agrees_with_the_measured_one` function L137-234 — `()` — written down in KAIROS-T-0190's status updates with the date they were taken.
...
#### crates/kairos-db/src/embeddings.rs

- pub `EmbeddingError` enum L42-73 — `Database | DimensionMismatch | ItemNotFound | MixedDimensions` — Errors from the embedding store.
- pub `StoredModel` struct L80-87 — `{ provider: String, model: String, dimension: usize }` — Which model produced a stored vector, as recorded beside it.
- pub `PendingItem` struct L91-119 — `{ id: Uuid, entity_type: String, short_code: String, title: String, content: Str...` — An item that needs embedding, with everything needed to compose its text.
- pub `EmbeddingCounts` struct L123-132 — `{ items: i64, embedded: i64, wrong_model: i64 }` — How far behind the vectors are.
- pub `missing` function L136-138 — `(&self) -> i64` — Items with no primary vector at all.
- pub `pending_primary` function L201-240 — `( conn: &mut PgConnection, model: &StoredModel, limit: i64, offset: i64, ) -> Re...` — Items whose primary vector is missing or whose content hash has moved on.
- pub `item_count` function L243-254 — `(conn: &mut PgConnection) -> Result<i64, EmbeddingError>` — How many items a sweep has to walk to cover a tenant once.
- pub `metadata_for` function L267-296 — `( conn: &mut PgConnection, item_ids: &[Uuid], ) -> Result<Vec<(Uuid, String, Str...` — Stamped metadata for a batch of items, as `(item_id, label, value)`.
- pub `store_primary` function L303-334 — `( conn: &mut PgConnection, item_id: Uuid, entity_type: &str, vector: &[f32], mod...` — Write (or replace) an item's primary vector.
- pub `ChunkWrite` struct L338-360 — `{ ordinal: i32, heading: Option<&'a str>, char_start: i32, char_end: i32, text: ...` — One chunk as the caller wants it stored.
- pub `stored_chunk_hashes` function L365-391 — `( conn: &mut PgConnection, item_id: Uuid, model: &StoredModel, ) -> Result<Vec<(...` — Which of an item's chunks are already stored and current.
- pub `sync_chunks` function L404-461 — `( conn: &mut PgConnection, item_id: Uuid, entity_type: &str, chunks: &[ChunkWrit...` — Bring an item's stored chunks in line with `chunks`, in one transaction.
- pub `forget_item` function L470-481 — `(conn: &mut PgConnection, item_id: Uuid) -> Result<(), EmbeddingError>` — Forget everything stored for an item.
- pub `counts` function L489-521 — `( conn: &mut PgConnection, model: &StoredModel, ) -> Result<EmbeddingCounts, Emb...` — How far behind the vectors are, for the configured model.
- pub `IndexReport` struct L581-592 — `{ dimension: usize, pinned: bool, created: Vec<String>, elapsed_ms: u128 }` — What [`pin_and_index`] did.
- pub `pin_and_index` function L628-689 — `( conn: &mut PgConnection, dimension: usize, ) -> Result<IndexReport, EmbeddingE...` — Pin the vector columns to `dimension` and build approximate-search indexes.
- pub `Neighbour` struct L728-748 — `{ id: Uuid, short_code: String, title: String, entity_type: String, archived: bo...` — One neighbour of an item, as the database sees it.
- pub `vector_neighbours` function L763-800 — `( conn: &mut PgConnection, item_id: Uuid, model: &StoredModel, limit: i64, ) -> ...` — The nearest items by vector, excluding the item itself.
- pub `lexical_neighbours` function L808-828 — `( conn: &mut PgConnection, item_id: Uuid, query: &str, limit: i64, ) -> Result<V...` — The nearest items by text, excluding the item itself.
- pub `graph_facts` function L840-905 — `( conn: &mut PgConnection, item_id: Uuid, others: &[Uuid], ) -> Result<Vec<Graph...` — What the graph says about each of `others`, relative to `item_id`.
- pub `GraphFacts` struct L912-923 — `{ item_id: Uuid, directly_linked: bool, shared_parent: bool, same_repository: bo...` — What the graph knows about one candidate, relative to the item asked about.
- pub `item_by_short_code` function L930-945 — `( conn: &mut PgConnection, short_code: &str, ) -> Result<Neighbour, EmbeddingErr...` — Resolve a short code to the item it names, live or archived.
- pub `item_by_id` function L952-964 — `(conn: &mut PgConnection, id: Uuid) -> Result<Neighbour, EmbeddingError>` — Resolve an item id to its directory row.
-  `EmbeddingCounts` type L134-139 — `= EmbeddingCounts` — tell which section moved; a per-chunk hash can.
-  `vector_literal` function L147-164 — `(v: &[f32]) -> String` — Format a vector as a pgvector literal.
-  `check_dimension` function L166-174 — `(vector: &[f32], declared: usize) -> Result<(), EmbeddingError>` — tell which section moved; a per-chunk hash can.
-  `Row` struct L245-248 — `{ count: i64 }` — tell which section moved; a per-chunk hash can.
-  `Row` struct L275-282 — `{ item_id: Uuid, label: String, value: String }` — tell which section moved; a per-chunk hash can.
-  `Row` struct L371-376 — `{ ordinal: i32, content_hash: String }` — tell which section moved; a per-chunk hash can.
-  `Row` struct L494-501 — `{ items: i64, embedded: i64, wrong_model: i64 }` — tell which section moved; a per-chunk hash can.
-  `tests` module L524-577 — `-` — tell which section moved; a per-chunk hash can.
-  `a_vector_literal_is_pgvector_shaped` function L528-531 — `()` — tell which section moved; a per-chunk hash can.
-  `a_vector_literal_round_trips_f32` function L536-545 — `()` — The literal must round-trip: a lossy format would silently change every
-  `a_declared_dimension_that_lies_is_refused` function L548-558 — `()` — tell which section moved; a per-chunk hash can.
-  `missing_is_items_minus_embedded_and_never_negative` function L561-576 — `()` — tell which section moved; a per-chunk hash can.
-  `is_pinned` function L692-709 — `( conn: &mut PgConnection, table: &str, dimension: usize, ) -> Result<bool, Embe...` — Whether `table`'s `embedding` column is already `vector(dimension)`.
-  `CountRow` struct L712-715 — `{ count: i64 }` — tell which section moved; a per-chunk hash can.
-  `Row` struct L849-860 — `{ other: Uuid, directly_linked: bool, shared_parent: bool, same_repository: bool...` — tell which section moved; a per-chunk hash can.
...
- pub `embeddings` module L12 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `events` module L13 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `forge` module L14 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `graph` module L15 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `items` module L16 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `local_auth` module L17 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `migrations` module L18 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `models` module L19 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `pool` module L20 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `proposals` module L21 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `repositories` module L22 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `retention` module L23 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `schema` module L24 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `scim` module L25 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `search` module L26 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `seed` module L27 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `service_accounts` module L28 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `team_pages` module L29 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.
- pub `tenant` module L30 — `-` — escape hatch for recursive-CTE traversals and the search pipeline.

#### crates/kairos-db/src/local_auth.rs

- pub `LocalSession` struct L28-42 — `{ id: Uuid, user_id: Uuid, token_hash: String, created_at: DateTime<Utc>, expire...` — A session row (`public.local_sessions`).
- pub `is_valid_at` function L48-50 — `(&self, now: DateTime<Utc>) -> bool` — Whether the session is usable at `now`.
- pub `NewLocalSession` struct L56-60 — `{ user_id: Uuid, token_hash: String, expires_at: DateTime<Utc> }` — Insert for [`LocalSession`]; `id`/`created_at` come from column defaults.
- pub `create_session` function L63-68 — `(conn: &mut PgConnection, new: NewLocalSession) -> QueryResult<LocalSession>` — Insert a session row.
- pub `find_session_by_hash` function L73-82 — `( conn: &mut PgConnection, token_hash: &str, ) -> QueryResult<Option<LocalSessio...` — The session row with this token hash, if any.
- pub `revoke_session_by_hash` function L89-97 — `(conn: &mut PgConnection, token_hash: &str) -> QueryResult<usize>` — Revoke one session by its token hash, and report whether anything changed.
- pub `revoke_all_sessions_for_user` function L100-108 — `(conn: &mut PgConnection, user_id: Uuid) -> QueryResult<usize>` — Revoke every live session a user holds, and report how many.
- pub `touch_session` function L112-116 — `(conn: &mut PgConnection, id: Uuid) -> QueryResult<usize>` — Record that a session was just used.
- pub `find_user_by_email` function L124-131 — `(conn: &mut PgConnection, email: &str) -> QueryResult<Option<User>>` — The user with this email, whether or not they have a password.
- pub `list_sessions_for_user` function L138-147 — `( conn: &mut PgConnection, user_id: Uuid, ) -> QueryResult<Vec<LocalSession>>` — Every session a user holds, newest first, revoked and expired rows included.
- pub `LOCAL_EXTERNAL_ID_PREFIX` variable L157 — `: &str` — The `external_id` prefix for an account that exists only locally
- pub `local_external_id` function L160-162 — `(email: &str) -> String` — The synthetic `external_id` for a local account with this email.
- pub `upsert_local_user` function L179-210 — `( conn: &mut PgConnection, email: &str, display_name: &str, password_hash: &str,...` — Create a local account, or adopt the existing row for this email.
- pub `set_password` function L224-235 — `( conn: &mut PgConnection, user_id: Uuid, password_hash: &str, ) -> QueryResult<...` — Set (or replace) a user's password hash **and revoke every session they
- pub `clear_password` function L241-248 — `(conn: &mut PgConnection, user_id: Uuid) -> QueryResult<usize>` — Remove a user's password, revoking their sessions with it.
- pub `BootstrapSkipped` enum L252-258 — `UsersExist | RacedAnotherReplica` — Why a bootstrap did not happen.
- pub `bootstrap_admin_if_empty` function L274-310 — `( conn: &mut PgConnection, email: &str, display_name: &str, password_hash: &str,...` — Create the first-boot admin, but **only if the deployment has no users at all**
- pub `delete_expired_sessions` function L315-321 — `( conn: &mut PgConnection, before: DateTime<Utc>, ) -> QueryResult<usize>` — Delete sessions that expired before `before`.
-  `LocalSession` type L44-51 — `= LocalSession` — [`set_password`] takes a hash rather than a password.
...
- pub `MigrationError` enum L41-64 — `Connection | Migration | MissingExtension` — Errors from establishing a migration connection or applying migrations.
- pub `check_required_extensions` function L88-90 — `(conn: &mut PgConnection) -> Result<(), MigrationError>` — Check that every extension in [`REQUIRED_EXTENSIONS`] can be created on this
- pub `check_extensions` function L98-125 — `( conn: &mut PgConnection, required: &[(&'static str, &'static str)], ) -> Resul...` — [`check_required_extensions`] against an explicit list.
- pub `establish_migration_connection` function L130-132 — `(database_url: &str) -> Result<PgConnection, MigrationError>` — Establish the dedicated synchronous connection used for running
- pub `run_public_migrations` function L143-155 — `(conn: &mut PgConnection) -> Result<Vec<String>, MigrationError>` — Run all pending public-schema migrations on `conn`.
- pub `has_pending_public_migrations` function L162-165 — `(conn: &mut PgConnection) -> Result<bool, MigrationError>` — Whether the embedded public-schema migration tree has any migration not
-  `REQUIRED_EXTENSIONS` variable L75-80 — `: &[(&str, &str)]` — Extensions Kairos requires of the database it is pointed at, with the
-  `NameRow` struct L106-109 — `{ name: String }` — pool (per KAIROS-A-0009) remains the runtime data path.
...
#### crates/kairos-db/src/proposals.rs

- pub `MAX_PENDING_PER_ITEM` variable L39 — `: i64` — The most pending proposals one item may carry, in either direction.
- pub `ProposalError` enum L43-87 — `NotFound | AlreadyDecided | AlreadyPending | TooManyPending | NotHuman | NotProp...` — Errors from the proposal services.
- pub `EdgeProposal` struct L91-129 — `{ id: Uuid, source_id: Uuid, target_id: Uuid, relationship: String, state: Strin...` — A proposed edge.
- pub `NewProposal` struct L133-146 — `{ source_id: Uuid, target_id: Uuid, relationship: RelationshipType, claim: &'a s...` — What to propose.
- pub `is_proposable` function L154-159 — `(relationship: RelationshipType) -> bool` — Whether a relationship is one an agent may propose.
- pub `propose` function L178-234 — `( conn: &mut PgConnection, new: NewProposal<'_>, proposed_by: Uuid, ) -> Result<...` — Record a proposed edge.
- pub `pending_for_item` function L241-254 — `( conn: &mut PgConnection, item_id: Uuid, ) -> Result<Vec<EdgeProposal>, Proposa...` — Undecided proposals touching an item, either end.
- pub `confirm` function L316-347 — `( conn: &mut PgConnection, id: Uuid, actor: Uuid, ) -> Result<EdgeProposal, Prop...` — Confirm a proposal, creating the real edge.
- pub `reject` function L355-371 — `( conn: &mut PgConnection, id: Uuid, actor: Uuid, ) -> Result<EdgeProposal, Prop...` — Reject a proposal.
- pub `ProposalStats` struct L375-382 — `{ pending: i64, confirmed: i64, rejected: i64 }` — How the proposals are going.
- pub `confirm_rate` function L392-395 — `(&self) -> Option<f32>` — Confirmed as a fraction of everything decided, or `None` when nothing has
- pub `stats` function L399-421 — `(conn: &mut PgConnection) -> Result<ProposalStats, ProposalError>` — Counts by state, for an operator.
-  `relationship_str` function L161-169 — `(relationship: RelationshipType) -> &'static str` — keeps a confident wrong similarity from quietly restructuring a portfolio.
-  `Count` struct L190-193 — `{ count: i64 }` — keeps a confident wrong similarity from quietly restructuring a portfolio.
-  `load` function L256-268 — `(conn: &mut PgConnection, id: Uuid) -> Result<EdgeProposal, ProposalError>` — keeps a confident wrong similarity from quietly restructuring a portfolio.
-  `is_human` function L276-286 — `(conn: &mut PgConnection, actor: Uuid) -> Result<bool, ProposalError>` — Whether `actor` is a person rather than a service account.
-  `Kind` struct L278-281 — `{ kind: String }` — keeps a confident wrong similarity from quietly restructuring a portfolio.
-  `decide` function L288-305 — `( conn: &mut PgConnection, id: Uuid, state: &str, actor: Uuid, ) -> Result<EdgeP...` — keeps a confident wrong similarity from quietly restructuring a portfolio.
-  `ProposalStats` type L384-396 — `= ProposalStats` — keeps a confident wrong similarity from quietly restructuring a portfolio.
-  `Row` struct L401-408 — `{ pending: i64, confirmed: i64, rejected: i64 }` — keeps a confident wrong similarity from quietly restructuring a portfolio.
-  `tests` module L424-464 — `-` — keeps a confident wrong similarity from quietly restructuring a portfolio.
-  `only_parent_and_blocks_are_proposable` function L428-436 — `()` — keeps a confident wrong similarity from quietly restructuring a portfolio.
-  `the_confirm_rate_is_none_until_something_is_decided` function L439-463 — `()` — keeps a confident wrong similarity from quietly restructuring a portfolio.
...
#### crates/kairos-db/src/schema.rs

- pub `sql_types` module L12-16 — `-`
- pub `Vector` struct L15 — `-`
...
- pub `SearchError` enum L97-111 — `Invalid | TraverseRootNotFound | Database` — Errors from the search pipeline.
- pub `SearchResults` struct L117-134 — `{ strategies: Vec<Strategy>, initiatives: Vec<Initiative>, tasks: Vec<Task>, doc...` — Search results grouped by entity type (A-0007 response shape), each
- pub `SearchStats` struct L140-145 — `{ hydration_queries: usize, total_queries: usize }` — Query-count instrumentation for one `execute_search` run — the proof
- pub `execute_search` function L149-154 — `( conn: &mut PgConnection, request: &SearchRequest, ) -> Result<SearchResults, S...` — Run the A-0007 search pipeline (module docs) for `request` in the
- pub `execute_search_with_stats` function L158-269 — `( conn: &mut PgConnection, request: &SearchRequest, ) -> Result<(SearchResults, ...` — [`execute_search`], also returning the [`SearchStats`] query counters
-  `IdRow` struct L276-279 — `{ id: Uuid }` — read-only, so no transaction is opened.
-  `ScoredIdRow` struct L283-288 — `{ id: Uuid, score: f32 }` — An id plus its `ts_rank_cd` relevance against `q` (KAIROS-T-0186).
-  `resolve_root` function L295-330 — `( conn: &mut PgConnection, from: &TraverseFrom, include_deleted: bool, stats: &m...` — Resolve `traverse.from` to an entity id via `entity_directory`.
-  `traverse_ids` function L336-385 — `( conn: &mut PgConnection, root: Uuid, traverse: &Traverse, stats: &mut SearchSt...` — Recursive-CTE walk from `root` (module docs, step 1).
-  `text_match_ids` function L400-420 — `( conn: &mut PgConnection, q: &str, include_deleted: bool, stats: &mut SearchSta...` — Full-text match via the `searchable_items` view (module docs, step 2).
-  `metadata_match_ids` function L426-452 — `( conn: &mut PgConnection, metadata: &std::collections::BTreeMap<String, String>...` — Ids satisfying EVERY metadata entry (module docs, step 3): one query,
-  `TypedIdRow` struct L459-464 — `{ id: Uuid, entity_type: String }` — read-only, so no transaction is opened.
-  `parse_entity_type` function L469-482 — `(value: &str) -> Result<ItemType, SearchError>` — Parse an `entity_type` literal from type resolution.
-  `partition_by_type` function L490-519 — `( conn: &mut PgConnection, ids: &HashSet<Uuid>, include_deleted: bool, stats: &m...` — Resolve candidate ids to `(id, entity_type)` and partition by type
-  `applicable_types` function L525-557 — `(filter: Option<&SearchFilter>) -> Vec<ItemType>` — Which entity types can match the structural filter at all (module docs,
-  `model_task_type` function L566-573 — `(task_type: SearchTaskType) -> TaskType` — The stored counterpart of a [`SearchTaskType`].
-  `model_work_class` function L576-581 — `(work_class: SearchWorkClass) -> WorkClass` — The stored counterpart of a [`SearchWorkClass`] (KAIROS-T-0077).
-  `hydrate_strategies` function L583-617 — `( conn: &mut PgConnection, ids: Option<Vec<Uuid>>, filter: Option<&SearchFilter>...` — read-only, so no transaction is opened.
-  `hydrate_initiatives` function L619-658 — `( conn: &mut PgConnection, ids: Option<Vec<Uuid>>, filter: Option<&SearchFilter>...` — read-only, so no transaction is opened.
-  `hydrate_tasks` function L660-709 — `( conn: &mut PgConnection, ids: Option<Vec<Uuid>>, filter: Option<&SearchFilter>...` — read-only, so no transaction is opened.
-  `hydrate_documents` function L711-739 — `( conn: &mut PgConnection, ids: Option<Vec<Uuid>>, filter: Option<&SearchFilter>...` — read-only, so no transaction is opened.
-  `hydrate_adrs` function L741-775 — `( conn: &mut PgConnection, ids: Option<Vec<Uuid>>, filter: Option<&SearchFilter>...` — read-only, so no transaction is opened.
-  `AnyItem` enum L782-788 — `Strategy | Initiative | Task | Document | Adr` — One hydrated row of any entity type, for the combined sort.
-  `AnyItem` type L790-840 — `= AnyItem` — read-only, so no transaction is opened.
-  `created_at` function L791-799 — `(&self) -> DateTime<Utc>` — read-only, so no transaction is opened.
-  `updated_at` function L801-809 — `(&self) -> DateTime<Utc>` — read-only, so no transaction is opened.
-  `title` function L811-819 — `(&self) -> &str` — read-only, so no transaction is opened.
-  `short_code` function L821-829 — `(&self) -> &str` — read-only, so no transaction is opened.
-  `id` function L831-839 — `(&self) -> Uuid` — read-only, so no transaction is opened.
-  `sort_items` function L854-873 — `(items: &mut [AnyItem], sort: Sort, relevance: &HashMap<Uuid, f32>)` — Sort the combined rows by the requested field/order, tie-broken by
...
- pub `seed_demo` function L342-1086 — `(conn: &mut PgConnection, force: bool) -> Result<SeedDemoReport, SeedError>` — Seed the demo tenant (see module docs for the full inventory and the
...
-  `upsert_demo_users` function L252-279 — `(conn: &mut PgConnection) -> Result<Vec<Uuid>, SeedError>` — Upsert the three demo users (keyed on `external_id`, exactly like JIT
-  `seed_team` function L283-321 — `( conn: &mut PgConnection, name: &str, slug: &str, team_type: TeamType, member_i...` — Create a team + its delivery board (the same shape as the API's team
-  `stamp_priority` function L324-338 — `( conn: &mut PgConnection, priority_def: Uuid, item_id: Uuid, value: &str, ) -> ...` — Stamp one `priority` metadata value on an item.
-  `SeedTask` struct L592-606 — `{ board: Uuid, column: &'a str, title: &'a str, content: &'a str, task_type: Tas...` — ever touched; other tenants and users are invisible to this module.
...
- pub `User` struct L69-91 — `{ id: Uuid, external_id: String, user_name: String, email: String, display_name:...` — A user principal (`public.users`) — an OIDC-backed human
- pub `is_service_account` function L95-97 — `(&self) -> bool` — True when this principal is a service account (KAIROS-A-0017).
- pub `NewUser` struct L104-109 — `{ external_id: String, user_name: String, email: String, display_name: String }` — Insert for a human [`User`].
- pub `NewServiceAccountUser` struct L116-122 — `{ external_id: String, user_name: String, email: String, display_name: String, k...` — Insert for a service-account [`User`] (KAIROS-A-0017): sets
- pub `new` function L128-143 — `( external_id: impl Into<String>, email: impl Into<String>, display_name: impl I...` — A service-account insert with a synthetic `external_id` and the
- pub `UserChangeset` struct L149-154 — `{ external_id: Option<String>, email: Option<String>, display_name: Option<Strin...` — Partial update for [`User`].
- pub `OrganizationMember` struct L167-172 — `{ organization_id: Uuid, user_id: Uuid, role: OrgRole, joined_at: DateTime<Utc> ...` — Org membership + role (`public.organization_members`, composite PK).
- pub `NewOrganizationMember` struct L177-181 — `{ organization_id: Uuid, user_id: Uuid, role: OrgRole }` — Insert for [`OrganizationMember`].
- pub `OrganizationMemberChangeset` struct L186-188 — `{ role: Option<OrgRole> }` — Partial update for [`OrganizationMember`] (role changes).
- pub `SystemTemplate` struct L199-206 — `{ id: Uuid, name: String, slug: String, content: String, created_at: DateTime<Ut...` — System default template (`public.system_templates`), copied into tenant
- pub `NewSystemTemplate` struct L211-215 — `{ name: String, slug: String, content: String }` — Insert for [`SystemTemplate`].
- pub `SystemTemplateChangeset` struct L220-225 — `{ name: Option<String>, slug: Option<String>, content: Option<String>, updated_a...` — Partial update for [`SystemTemplate`].
- pub `SystemMetadataDefinition` struct L235-242 — `{ id: Uuid, name: String, slug: String, field_type: FieldType, created_at: DateT...` — System default metadata definition (`public.system_metadata_definitions`).
- pub `NewSystemMetadataDefinition` struct L247-251 — `{ name: String, slug: String, field_type: FieldType }` — Insert for [`SystemMetadataDefinition`].
- pub `SystemMetadataDefinitionChangeset` struct L256-261 — `{ name: Option<String>, slug: Option<String>, field_type: Option<FieldType>, upd...` — Partial update for [`SystemMetadataDefinition`].
- pub `SystemMetadataEnumOption` struct L273-278 — `{ id: Uuid, metadata_definition_id: Uuid, value: String, position: i32 }` — Enum option for a system metadata definition
- pub `NewSystemMetadataEnumOption` struct L283-287 — `{ metadata_definition_id: Uuid, value: String, position: i32 }` — Insert for [`SystemMetadataEnumOption`].
- pub `SystemMetadataEnumOptionChangeset` struct L292-295 — `{ value: Option<String>, position: Option<i32> }` — Partial update for [`SystemMetadataEnumOption`].
- pub `SystemTemplateMetadata` struct L308-314 — `{ id: Uuid, template_id: Uuid, metadata_definition_id: Uuid, default_value: Opti...` — Template-to-metadata association for system defaults
- pub `NewSystemTemplateMetadata` struct L319-324 — `{ template_id: Uuid, metadata_definition_id: Uuid, default_value: Option<String>...` — Insert for [`SystemTemplateMetadata`].
- pub `SystemTemplateMetadataChangeset` struct L330-333 — `{ default_value: Option<Option<String>>, required: Option<bool> }` — Partial update for [`SystemTemplateMetadata`].
- pub `SystemBoardDefault` struct L345-353 — `{ id: Uuid, board_level: BoardLevel, columns: String, transitions: String }` — Default board configuration per flight level
- pub `NewSystemBoardDefault` struct L358-363 — `{ board_level: BoardLevel, columns: String, transitions: String }` — Insert for [`SystemBoardDefault`].
- pub `SystemBoardDefaultChangeset` struct L368-373 — `{ board_level: Option<BoardLevel>, columns: Option<String>, transitions: Option<...` — Partial update for [`SystemBoardDefault`].
-  `User` type L93-98 — `= User` — the tenant `search_path` pinned on it.
-  `NewServiceAccountUser` type L124-144 — `= NewServiceAccountUser` — the tenant `search_path` pinned on it.
...
-  `insert_user` function L53-65 — `(conn: &mut PgConnection, external_id: &str, email: &str, name: &str) -> Uuid` — off-board ADRs, orphan documents, and unknown ids resolving to None
-  `board_id_by_slug` function L68-74 — `(conn: &mut PgConnection, slug: &str) -> Uuid` — The board with this slug in the current tenant schema.
-  `first_column` function L77-84 — `(conn: &mut PgConnection, board: Uuid) -> Uuid` — The first column of a board (position order) — items need a placement.
-  `activity_details` function L87-95 — `(conn: &mut PgConnection, action: ActivityAction, entity: Uuid) -> Vec<String>` — All `activity_log.details` values for (action, entity_id), in order.
-  `assert_check` function L99-117 — `( conn: &mut PgConnection, board: Uuid, user: Uuid, granted: &str, required: &st...` — Assert the SQL check agrees with the pure core matcher for one stored
-  `abac_capability_lifecycle` function L120-625 — `()` — off-board ADRs, orphan documents, and unknown ids resolving to None
-  `team_membership_implies_delivery_capabilities` function L632-761 — `()` — KAIROS-T-0072 (A-0006 amendment): membership of a board's owning team
-  `TEAM_SCRATCH_DB` variable L635 — `: &str` — off-board ADRs, orphan documents, and unknown ids resolving to None
-  `archived_items_resolve_the_same_capabilities_as_live_ones` function L772-931 — `()` — KAIROS-T-0153 / [KAIROS-A-0020]: archiving is a visibility default, not a
-  `ARCHIVE_SCRATCH_DB` variable L778 — `: &str` — off-board ADRs, orphan documents, and unknown ids resolving to None
...
-  `insert_user` function L39-52 — `(conn: &mut PgConnection, external_id: &str, email: &str, name: &str) -> Uuid` — own scratch database like the other kairos-db integration tests.
-  `seed_team` function L54-76 — `(conn: &mut PgConnection, name: &str, slug: &str, actor: Uuid) -> (Uuid, Uuid)` — own scratch database like the other kairos-db integration tests.
-  `column_name` function L78-85 — `(conn: &mut PgConnection, column_id: Uuid) -> String` — own scratch database like the other kairos-db integration tests.
-  `activity_details` function L87-95 — `(conn: &mut PgConnection, item: Uuid) -> Vec<String>` — own scratch database like the other kairos-db integration tests.
-  `move_task_between_delivery_boards` function L98-246 — `()` — own scratch database like the other kairos-db integration tests.
...
-  `board_rules_lifecycle` function L180-756 — `()` — removed

#### crates/kairos-db/tests/embeddings.rs

-  `DEFAULT_DATABASE_URL` variable L29 — `: &str` — `angreal test integration`), per KAIROS-A-0012: the database is never mocked.
-  `SCRATCH_DB` variable L30 — `: &str` — `angreal test integration`), per KAIROS-A-0012: the database is never mocked.
-  `DIM` variable L34 — `: usize` — The width the local model produces (KAIROS-T-0189), so the fixtures exercise
-  `admin_database_url` function L36-38 — `() -> String` — `angreal test integration`), per KAIROS-A-0012: the database is never mocked.
-  `with_database` function L40-43 — `(url: &str, db_name: &str) -> String` — `angreal test integration`), per KAIROS-A-0012: the database is never mocked.
-  `unit` function L46-50 — `(seed: u32) -> Vec<f32>` — A deterministic unit vector, so assertions can be exact.
-  `model` function L52-58 — `() -> StoredModel` — `angreal test integration`), per KAIROS-A-0012: the database is never mocked.
-  `Count` struct L61-64 — `{ count: i64 }` — `angreal test integration`), per KAIROS-A-0012: the database is never mocked.
-  `count_rows` function L66-71 — `(conn: &mut PgConnection, sql: &str) -> i64` — `angreal test integration`), per KAIROS-A-0012: the database is never mocked.
-  `embedding_store_lifecycle` function L74-585 — `()` — `angreal test integration`), per KAIROS-A-0012: the database is never mocked.
-  `Stamp` struct L367-370 — `{ updated_at: chrono::DateTime<chrono::Utc> }` — `angreal test integration`), per KAIROS-A-0012: the database is never mocked.
...
-  `insert_user` function L58-70 — `(conn: &mut PgConnection, external_id: &str, email: &str, name: &str) -> Uuid` — (`idx_item_relationships_source` / `idx_item_relationships_target`)
-  `board_id_by_slug` function L73-79 — `(conn: &mut PgConnection, slug: &str) -> Uuid` — The board with this slug in the current tenant schema.
-  `relationship_activity` function L83-91 — `(conn: &mut PgConnection, action: ActivityAction) -> Vec<String>` — All `activity_log.details` for relationship actions (`entity_id` is
-  `edge_count` function L94-107 — `( conn: &mut PgConnection, source: Uuid, target: Uuid, relationship: Relationshi...` — How many edges `(source, target, relationship)` exist.
-  `assert_rule_rejected` function L111-137 — `( conn: &mut PgConnection, source: Uuid, target: Uuid, relationship: Relationshi...` — Assert a link attempt is rejected by the type-rule matrix with the
-  `ExplainRow` struct L142-144 — `{ line: String }` — One EXPLAIN output line.
-  `ExplainRow` type L146-154 — `= ExplainRow` — (`idx_item_relationships_source` / `idx_item_relationships_target`)
-  `build` function L147-153 — `( row: &impl diesel::row::NamedRow<'a, diesel::pg::Pg>, ) -> diesel::deserialize...` — (`idx_item_relationships_source` / `idx_item_relationships_target`)
-  `explain` function L160-168 — `(conn: &mut PgConnection, sql: &str) -> String` — The EXPLAIN plan for `sql`, one string.
-  `relationship_graph_service` function L171-738 — `()` — (`idx_item_relationships_source` / `idx_item_relationships_target`)
-  `children_progress_rollups` function L745-923 — `()` — KAIROS-T-0080: the children-progress rollups — per-parent grouping,
-  `PROGRESS_DB` variable L746 — `: &str` — (`idx_item_relationships_source` / `idx_item_relationships_target`)
-  `focal_subgraph_contract` function L942-1197 — `()` — `item_subgraph`: depth bounding with min-depth per node, cross-links
-  `SUBGRAPH_DB` variable L943 — `: &str` — (`idx_item_relationships_source` / `idx_item_relationships_target`)
-  `archived_children_are_listed_marked_but_never_counted` function L1220-1371 — `()` — The case that motivated KAIROS-T-0158: **an initiative with one live
-  `NEIGHBOUR_DB` variable L1221 — `: &str` — (`idx_item_relationships_source` / `idx_item_relationships_target`)
...
-  `insert_user` function L140-152 — `(conn: &mut PgConnection, external_id: &str, email: &str, name: &str) -> Uuid` — by construction; each resolves only within its own tenant, never cross.
-  `board_id_by_slug` function L154-160 — `(conn: &mut PgConnection, slug: &str) -> Uuid` — by construction; each resolves only within its own tenant, never cross.
-  `first_column` function L162-169 — `(conn: &mut PgConnection, board: Uuid) -> Uuid` — by construction; each resolves only within its own tenant, never cross.
-  `metadata_def` function L171-177 — `(conn: &mut PgConnection, slug: &str) -> Uuid` — by construction; each resolves only within its own tenant, never cross.
-  `CountRow` struct L180-183 — `{ n: i64 }` — by construction; each resolves only within its own tenant, never cross.
-  `count_where_id` function L187-195 — `(conn: &mut PgConnection, table: &str, column: &str, id: Uuid) -> i64` — `count(*)` of `sql_fragment` (a full `SELECT ...
-  `directory_id_by_code` function L202-215 — `(conn: &mut PgConnection, short_code: &str) -> Option<Uuid>` — The id `entity_directory` resolves for `short_code` in the CURRENT tenant
-  `IdRow` struct L204-207 — `{ id: Uuid }` — by construction; each resolves only within its own tenant, never cross.
-  `Seed` struct L219-232 — `{ marker: String, strategy: Uuid, initiative: Uuid, task: Uuid, document: Uuid, ...` — Every seeded item in one tenant, with the short codes services assigned
-  `COLLIDE_CODE` variable L235 — `: &str` — The short code deliberately shared, byte-for-byte, across both tenants.
-  `seed` function L242-398 — `(conn: &mut PgConnection, slug: &str, user: Uuid) -> Seed` — Seed one tenant (connection already pinned to it).
-  `run_search` function L404-408 — `(conn: &mut PgConnection, request: serde_json::Value) -> SearchResults` — by construction; each resolves only within its own tenant, never cross.
-  `all_result_ids` function L410-418 — `(results: &SearchResults) -> Vec<Uuid>` — by construction; each resolves only within its own tenant, never cross.
-  `result_count` function L420-422 — `(results: &SearchResults) -> usize` — by construction; each resolves only within its own tenant, never cross.
-  `CHECKSUM_TABLES` variable L432-443 — `: [&str; 10]` — The tables whose contents must be byte-identical before and after an
-  `table_fingerprint` function L446-460 — `(conn: &mut PgConnection, table: &str) -> String` — `count:md5` fingerprint of one table in the CURRENT tenant schema.
-  `Fp` struct L448-451 — `{ fp: String }` — by construction; each resolves only within its own tenant, never cross.
-  `fingerprint_all` function L464-469 — `(conn: &mut PgConnection) -> Vec<(String, String)>` — Fingerprint every [`CHECKSUM_TABLES`] table (connection pinned to the
-  `cross_read_visibility_sweep` function L476-673 — `()` — by construction; each resolves only within its own tenant, never cross.
-  `DB` variable L477 — `: &str` — by construction; each resolves only within its own tenant, never cross.
-  `cross_write_battery` function L680-796 — `()` — by construction; each resolves only within its own tenant, never cross.
-  `DB` variable L681 — `: &str` — by construction; each resolves only within its own tenant, never cross.
-  `short_code_collision` function L803-870 — `()` — by construction; each resolves only within its own tenant, never cross.
-  `DB` variable L804 — `: &str` — by construction; each resolves only within its own tenant, never cross.
-  `pool_stress` module L881-1040 — `-` — Barrier-synchronized concurrent rounds over ONE small pool, alternating
-  `ROUNDS` variable L896 — `: usize` — by construction; each resolves only within its own tenant, never cross.
-  `TitleRow` struct L899-902 — `{ title: String }` — by construction; each resolves only within its own tenant, never cross.
-  `TenantFixture` struct L907-910 — `{ board: Uuid, column: Uuid }` — Per-tenant board + first column, resolved once (sync) for the async
-  `pool_reuse_stress` function L913-1039 — `()` — by construction; each resolves only within its own tenant, never cross.
-  `DB` variable L914 — `: &str` — by construction; each resolves only within its own tenant, never cross.

#### crates/kairos-db/tests/local_auth.rs

-  `DEFAULT_DATABASE_URL` variable L24 — `: &str` — Runs against the real compose Postgres (KAIROS-A-0012, never mocked).
-  `admin_url` function L26-28 — `() -> String` — Runs against the real compose Postgres (KAIROS-A-0012, never mocked).
-  `with_database` function L30-33 — `(url: &str, db: &str) -> String` — Runs against the real compose Postgres (KAIROS-A-0012, never mocked).
-  `setup` function L37-47 — `(db: &str) -> PgConnection` — A scratch database with public migrations applied.
-  `teardown` function L49-52 — `(db: &str)` — Runs against the real compose Postgres (KAIROS-A-0012, never mocked).
-  `make_user` function L54-65 — `(conn: &mut PgConnection, email: &str) -> User` — Runs against the real compose Postgres (KAIROS-A-0012, never mocked).
-  `mint` function L67-78 — `(conn: &mut PgConnection, user_id: Uuid, hash: &str, ttl_days: i64) -> Uuid` — Runs against the real compose Postgres (KAIROS-A-0012, never mocked).
-  `a_session_round_trips_and_expiry_and_revocation_are_honored` function L81-149 — `()` — Runs against the real compose Postgres (KAIROS-A-0012, never mocked).
-  `changing_a_password_revokes_every_session` function L152-211 — `()` — Runs against the real compose Postgres (KAIROS-A-0012, never mocked).
-  `looking_a_user_up_does_not_filter_out_oidc_only_accounts` function L214-236 — `()` — Runs against the real compose Postgres (KAIROS-A-0012, never mocked).
-  `deleting_a_user_takes_their_sessions_with_them` function L239-258 — `()` — Runs against the real compose Postgres (KAIROS-A-0012, never mocked).
-  `expired_sessions_can_be_swept` function L261-287 — `()` — Runs against the real compose Postgres (KAIROS-A-0012, never mocked).
-  `adding_a_password_to_an_oidc_identity_does_not_fork_the_person` function L290-340 — `()` — Runs against the real compose Postgres (KAIROS-A-0012, never mocked).
-  `the_bootstrap_admin_is_single_use` function L343-400 — `()` — Runs against the real compose Postgres (KAIROS-A-0012, never mocked).
...
-  `models_round_trip` function L146-530 — `()` — KAIROS-T-0016 adversarial suite.
-  `DB` variable L147 — `: &str` — KAIROS-T-0016 adversarial suite.
-  `pool_isolation_interleaved` function L533-671 — `()` — KAIROS-T-0016 adversarial suite.
-  `DB` variable L534 — `: &str` — KAIROS-T-0016 adversarial suite.
-  `team_names` function L556-563 — `(conn: &mut kairos_db::TenantConnection) -> Vec<String>` — KAIROS-T-0016 adversarial suite.

#### crates/kairos-db/tests/proposals.rs

-  `DEFAULT_DATABASE_URL` variable L22 — `: &str` — under its own output, and that a decided proposal cannot be decided twice.
-  `SCRATCH_DB` variable L23 — `: &str` — under its own output, and that a decided proposal cannot be decided twice.
-  `admin_database_url` function L25-27 — `() -> String` — under its own output, and that a decided proposal cannot be decided twice.
-  `with_database` function L29-32 — `(url: &str, db: &str) -> String` — under its own output, and that a decided proposal cannot be decided twice.
-  `edge_proposal_lifecycle` function L35-317 — `()` — under its own output, and that a decided proposal cannot be decided twice.
-  `Count` struct L202-205 — `{ count: i64 }` — under its own output, and that a decided proposal cannot be decided twice.
...
-  `EXPECTED_TABLES` variable L28-41 — `: [&str; 10]` — The public-schema tables: the 8 defined by KAIROS-S-0004 plus the
-  `admin_database_url` function L43-45 — `() -> String` — touches the dev `kairos` database or interferes with other tests.
-  `with_database` function L48-53 — `(url: &str, db_name: &str) -> String` — Replace the database name (final path segment) in a postgres URL.
-  `TableName` struct L56-59 — `{ table_name: String }` — touches the dev `kairos` database or interferes with other tests.
-  `public_base_tables` function L61-76 — `(conn: &mut PgConnection) -> Vec<String>` — touches the dev `kairos` database or interferes with other tests.
-  `assert_database_error_kind` function L79-88 — `( result: Result<T, DieselError>, kind: DatabaseErrorKind, context: &str, )` — Expect a database error of `kind` from `result`.
-  `public_migrations_from_empty_database` function L91-157 — `()` — touches the dev `kairos` database or interferes with other tests.
...
-  `insert_user` function L78-90 — `(conn: &mut PgConnection, external_id: &str, email: &str, name: &str) -> Uuid` — scheduler test, no database)
-  `board_id_by_slug` function L92-98 — `(conn: &mut PgConnection, slug: &str) -> Uuid` — scheduler test, no database)
-  `at` function L100-102 — `(y: i32, mo: u32, d: u32, h: u32, mi: u32, s: u32) -> DateTime<Utc>` — scheduler test, no database)
-  `seed_strategy` function L105-139 — `( conn: &mut PgConnection, board: Uuid, name: &str, versions: i32, actor: Uuid, ...` — Create a strategy with `versions` content versions (create + updates).
-  `backdate_history` function L143-153 — `(conn: &mut PgConnection, item: Uuid, version: i32, ts: DateTime<Utc>)` — Fabricate a snapshot's age (the KAIROS-T-0015 injected-clock seeding:
-  `history_versions` function L156-163 — `(conn: &mut PgConnection, item: Uuid) -> Vec<i32>` — Remaining history versions of an item, ascending.
-  `total_history_rows` function L165-170 — `(conn: &mut PgConnection) -> i64` — scheduler test, no database)
-  `sweep_details` function L173-180 — `(conn: &mut PgConnection) -> Vec<String>` — `retention_sweep` audit rows' details, in `occurred_at` order.
-  `old_activity_count` function L182-188 — `(conn: &mut PgConnection, cutoff: DateTime<Utc>) -> i64` — scheduler test, no database)
-  `counts` function L190-196 — `(archived: u64, pruned: u64, warnings: u64) -> TableCounts` — scheduler test, no database)
-  `read_ndjson` function L199-206 — `(path: &std::path::Path) -> Vec<serde_json::Value>` — Parse an NDJSON archive file into one `serde_json::Value` per line.
-  `json_ts` function L208-214 — `(value: &serde_json::Value, key: &str) -> DateTime<Utc>` — scheduler test, no database)
-  `retention_sweeper_lifecycle` function L217-575 — `()` — scheduler test, no database)
-  `retention_loop_ticks_until_break` function L582-598 — `()` — The in-process scheduler (KAIROS-A-0004 "in-process scheduled task"):
...
-  `insert_user` function L70-82 — `(conn: &mut PgConnection, external_id: &str, email: &str, name: &str) -> Uuid` — within the stated budget with exactly one task-hydration query
-  `board_id_by_slug` function L85-91 — `(conn: &mut PgConnection, slug: &str) -> Uuid` — The board with this slug in the current tenant schema.
-  `metadata_definition` function L95-116 — `(conn: &mut PgConnection, name: &str, slug: &str) -> Uuid` — The tenant's metadata definition with this slug, creating it if the
-  `set_metadata` function L118-127 — `(conn: &mut PgConnection, item_id: Uuid, definition_id: Uuid, value: &str)` — within the stated budget with exactly one task-hydration query
-  `run` function L131-136 — `(conn: &mut PgConnection, request: serde_json::Value) -> (SearchResults, SearchS...` — Parse a request from its JSON shape (the S-0005 wire format) and run it
-  `ids` function L138-140 — `(rows: &[T], id_of: impl Fn(&T) -> Uuid) -> HashSet<Uuid>` — within the stated budget with exactly one task-hydration query
-  `task_ids` function L142-144 — `(results: &SearchResults) -> HashSet<Uuid>` — within the stated budget with exactly one task-hydration query
-  `all_ids` function L146-154 — `(results: &SearchResults) -> HashSet<Uuid>` — within the stated budget with exactly one task-hydration query
-  `returned_row_count` function L156-162 — `(results: &SearchResults) -> usize` — within the stated budget with exactly one task-hydration query
-  `unified_search_pipeline` function L165-1068 — `()` — within the stated budget with exactly one task-hydration query
...
-  `DEFAULT_DATABASE_URL` variable L37 — `: &str` — Same default as `.angreal/task_db.py`'s `DATABASE_URL`.
-  `SCRATCH_DB` variable L39 — `: &str` — `system_board_defaults` rows are seeded.
-  `EXPECTED_TABLES` variable L44-81 — `: [&str; 33]` — The tenant tables (sorted): the 21 from the KAIROS-S-0004 DDL plus
-  `EXPECTED_VIEWS` variable L83 — `: [&str; 2]` — `system_board_defaults` rows are seeded.
-  `EXPECTED_SEQUENCES` variable L85-91 — `: [&str; 5]` — `system_board_defaults` rows are seeded.
-  `EXPECTED_INDEXES` variable L100-124 — `: [&str; 23]` — Every named index a freshly provisioned tenant carries: the S-0004
-  `admin_database_url` function L126-128 — `() -> String` — `system_board_defaults` rows are seeded.
-  `with_database` function L131-136 — `(url: &str, db_name: &str) -> String` — Replace the database name (final path segment) in a postgres URL.
-  `NameRow` struct L139-142 — `{ name: String }` — `system_board_defaults` rows are seeded.
-  `CountRow` struct L145-148 — `{ count: i64 }` — `system_board_defaults` rows are seeded.
-  `names` function L150-160 — `(conn: &mut PgConnection, query: &str, param: &str) -> Vec<String>` — `system_board_defaults` rows are seeded.
-  `schema_tables` function L162-170 — `(conn: &mut PgConnection, schema: &str) -> Vec<String>` — `system_board_defaults` rows are seeded.
-  `schema_views` function L172-178 — `(conn: &mut PgConnection, schema: &str) -> Vec<String>` — `system_board_defaults` rows are seeded.
-  `schema_sequences` function L180-187 — `(conn: &mut PgConnection, schema: &str) -> Vec<String>` — `system_board_defaults` rows are seeded.
-  `schema_indexes` function L189-195 — `(conn: &mut PgConnection, schema: &str) -> Vec<String>` — `system_board_defaults` rows are seeded.
-  `schema_shape` function L209-254 — `(conn: &mut PgConnection, schema: &str) -> Vec<String>` — Everything about a tenant schema's shape that a migration can change, as one
-  `Row` struct L223-226 — `{ line: String }` — `system_board_defaults` rows are seeded.
-  `schema_exists` function L256-263 — `(conn: &mut PgConnection, schema: &str) -> bool` — `system_board_defaults` rows are seeded.
-  `count` function L265-270 — `(conn: &mut PgConnection, sql: &str) -> i64` — `system_board_defaults` rows are seeded.
-  `board_columns` function L273-285 — `(conn: &mut PgConnection, board_slug: &str) -> Vec<String>` — Board columns (ordered by position) for a board slug in `org_acme`.
-  `board_transitions` function L288-303 — `(conn: &mut PgConnection, board_slug: &str) -> BTreeSet<String>` — Transition pairs `"From -> To"` for a board slug in `org_acme`.
-  `transitions` function L305-307 — `(pairs: &[(&str, &str)]) -> BTreeSet<String>` — `system_board_defaults` rows are seeded.
-  `tenant_provisioning_lifecycle` function L310-943 — `()` — `system_board_defaults` rows are seeded.
...
-  `insert_user` function L74-86 — `(conn: &mut PgConnection, external_id: &str, email: &str, name: &str) -> Uuid` — records ONE activity row with the cascade count + short codes
-  `board_id_by_slug` function L89-95 — `(conn: &mut PgConnection, slug: &str) -> Uuid` — The board with this slug in the current tenant schema.
-  `first_column` function L98-105 — `(conn: &mut PgConnection, board: Uuid) -> Uuid` — The first column of a board (position order).
-  `history_of` function L109-120 — `(conn: &mut PgConnection, item: Uuid) -> Vec<(i32, String, String)>` — All history snapshots for an item as `(version, title, content)`, in
-  `activity_details` function L123-131 — `(conn: &mut PgConnection, action: ActivityAction, entity: Uuid) -> Vec<String>` — All `activity_log.details` values for (action, entity_id), in order.
-  `CountRow` struct L134-137 — `{ n: i64 }` — records ONE activity row with the cascade count + short codes
-  `view_count` function L147-155 — `(conn: &mut PgConnection, view: &str, id: Uuid) -> i64` — How many LIVE rows of `searchable_items` / `entity_directory` carry
-  `view_count_any` function L161-167 — `(conn: &mut PgConnection, view: &str, id: Uuid) -> i64` — How many rows of the view carry this id REGARDLESS of liveness — the
-  `code_number` function L170-175 — `(code: &str) -> i64` — The numeric part of a `{PREFIX}-{L}-{NNNN}` short code.
-  `write_path_lifecycle` function L178-831 — `()` — records ONE activity row with the cascade count + short codes
-  `WRITERS` variable L374 — `: usize` — records ONE activity row with the cascade count + short codes

### crates/kairos-embed/src/bin

> *Semantic summary to be generated by AI agent.*

#### crates/kairos-embed/src/bin/fetch-model.rs

-  `main` function L22-64 — `() -> std::process::ExitCode` — Idempotent: a populated cache makes this a load and a probe.

### crates/kairos-embed/src

> *Semantic summary to be generated by AI agent.*

#### crates/kairos-embed/src/config.rs

- pub `ProviderKind` enum L34-43 — `Local | Remote | Deterministic | None` — Which provider a deployment wants.
- pub `EmbedConfig` struct L47-56 — `{ kind: ProviderKind, local: crate::local::LocalConfig, remote: Option<crate::re...` — Everything needed to build a provider.
- pub `ConfigError` enum L60-82 — `UnknownProvider | RemoteWithoutUrl | RemoteWithoutModel | BadNumber | ProviderNo...` — A configuration that could not be read.
- pub `from_env` function L86-88 — `() -> Result<Self, ConfigError>` — Read the configuration from the process environment.
- pub `from_vars` function L93-157 — `(get: impl Fn(&str) -> Option<String>) -> Result<Self, ConfigError>` — Read from an arbitrary lookup, so this is testable without touching the
- pub `build` function L164-190 — `(&self) -> Result<Option<Arc<dyn EmbeddingProvider>>, EmbedError>` — Build the provider, or `None` for [`ProviderKind::None`].
-  `EmbedConfig` type L84-191 — `= EmbedConfig` — outbound calls — says so, and gets search that still works.
-  `tests` module L194-315 — `-` — outbound calls — says so, and gets search that still works.
-  `vars` function L198-204 — `(pairs: &[(&str, &str)]) -> impl Fn(&str) -> Option<String> + use<>` — outbound calls — says so, and gets search that still works.
-  `the_default_is_local_with_nothing_set` function L208-216 — `()` — Rule 1 is only satisfied if the default needs no configuration.
-  `a_url_alone_selects_remote` function L221-235 — `()` — Setting a URL is saying what you want; it should not also require saying
-  `remote_without_a_url_or_model_is_refused` function L238-247 — `()` — outbound calls — says so, and gets search that still works.
-  `an_unknown_provider_is_refused_by_name` function L250-257 — `()` — outbound calls — says so, and gets search that still works.
-  `none_builds_no_provider_and_is_not_an_error` function L261-267 — `()` — Switching embeddings off is a choice, not a failure.
-  `deterministic_builds_without_any_other_configuration` function L270-279 — `()` — outbound calls — says so, and gets search that still works.
-  `a_bad_timeout_names_the_variable` function L282-293 — `()` — outbound calls — says so, and gets search that still works.
-  `blank_values_are_treated_as_unset` function L296-302 — `()` — outbound calls — says so, and gets search that still works.
-  `the_cache_directory_and_download_flag_are_read` function L306-314 — `()` — outbound calls — says so, and gets search that still works.

#### crates/kairos-embed/src/deterministic.rs

- pub `DeterministicProvider` struct L20-22 — `{ id: ModelId }` — Hash-derived embeddings: same text, same vector, always.
- pub `new` function L29-33 — `(dimension: usize) -> Self` — A provider producing `dimension`-wide vectors.
-  `DeterministicProvider` type L24-34 — `= DeterministicProvider` — model, and KAIROS-T-0189's own measurements are where that happens.
-  `DeterministicProvider` type L36-43 — `impl Default for DeterministicProvider` — model, and KAIROS-T-0189's own measurements are where that happens.
-  `default` function L40-42 — `() -> Self` — 384 wide, matching the local default (`bge-small-en-v1.5-q`), so swapping
-  `DeterministicProvider` type L45-85 — `impl EmbeddingProvider for DeterministicProvider` — model, and KAIROS-T-0189's own measurements are where that happens.
-  `model_id` function L46-48 — `(&self) -> &ModelId` — model, and KAIROS-T-0189's own measurements are where that happens.
-  `embed` function L50-84 — `(&self, texts: &[String]) -> Result<Vec<Embedding>, EmbedError>` — model, and KAIROS-T-0189's own measurements are where that happens.
-  `tests` module L88-167 — `-` — model, and KAIROS-T-0189's own measurements are where that happens.
-  `texts` function L92-94 — `(items: &[&str]) -> Vec<String>` — model, and KAIROS-T-0189's own measurements are where that happens.
-  `the_same_text_always_gives_the_same_vector` function L98-107 — `()` — The property the whole thing exists for.
-  `a_vector_does_not_depend_on_its_company` function L112-122 — `()` — And it does not depend on what else was in the batch — the failure that
-  `order_is_preserved` function L125-131 — `()` — model, and KAIROS-T-0189's own measurements are where that happens.
-  `different_texts_are_not_similar` function L134-142 — `()` — model, and KAIROS-T-0189's own measurements are where that happens.
-  `vectors_are_unit_length_like_a_real_provider` function L145-151 — `()` — model, and KAIROS-T-0189's own measurements are where that happens.
-  `any_width_is_reachable` function L154-160 — `()` — model, and KAIROS-T-0189's own measurements are where that happens.
-  `an_empty_batch_is_an_empty_result_not_an_error` function L163-166 — `()` — model, and KAIROS-T-0189's own measurements are where that happens.

#### crates/kairos-embed/src/lib.rs

- pub `local` module L42 — `-` — re-embedding in response is KAIROS-T-0190's.
- pub `remote` module L44 — `-` — re-embedding in response is KAIROS-T-0190's.
- pub `Embedding` type L52 — `= Vec<f32>` — One embedding: a dense vector in whichever space its [`ModelId`] names.
- pub `ModelId` struct L61-70 — `{ provider: String, model: String, dimension: usize }` — Which model produced a vector, and how wide it is.
- pub `new` function L74-80 — `(provider: impl Into<String>, model: impl Into<String>, dimension: usize) -> Sel...` — A new identity.
- pub `mismatch` function L89-109 — `(&self, stored: &ModelId) -> Option<Mismatch>` — How `stored` differs from `self`, or `None` if they agree.
- pub `Mismatch` enum L120-159 — `Dimension | Model | Provider` — A disagreement between the configured model and a stored vector's model.
- pub `is_fatal` function L165-167 — `(&self) -> bool` — Whether this mismatch makes stored vectors unusable rather than merely
- pub `EmbedError` enum L172-199 — `Unavailable | Failed | CountMismatch | WidthMismatch` — Why embedding failed.
- pub `EmbeddingProvider` interface L217-232 — `{ fn model_id(), fn embed(), fn embed_one() }` — Turns text into vectors.
- pub `cosine` function L267-278 — `(a: &[f32], b: &[f32]) -> Option<f32>` — Cosine similarity between two vectors in the same space.
-  `config` module L46 — `-` — re-embedding in response is KAIROS-T-0190's.
-  `deterministic` module L47 — `-` — re-embedding in response is KAIROS-T-0190's.
-  `ModelId` type L72-110 — `= ModelId` — re-embedding in response is KAIROS-T-0190's.
-  `ModelId` type L112-116 — `= ModelId` — re-embedding in response is KAIROS-T-0190's.
-  `fmt` function L113-115 — `(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result` — re-embedding in response is KAIROS-T-0190's.
-  `Mismatch` type L161-168 — `= Mismatch` — re-embedding in response is KAIROS-T-0190's.
-  `embed_one` function L228-231 — `(&self, text: &str) -> Result<Embedding, EmbedError>` — Embed one text.
-  `validate_batch` function L241-259 — `( id: &ModelId, texts: &[String], vectors: Vec<Embedding>, ) -> Result<Vec<Embed...` — Check a provider's own output before it reaches a caller: one vector per
-  `tests` module L281-365 — `-` — re-embedding in response is KAIROS-T-0190's.
-  `id` function L284-286 — `(provider: &str, model: &str, dim: usize) -> ModelId` — re-embedding in response is KAIROS-T-0190's.
-  `matching_identities_do_not_mismatch` function L289-292 — `()` — re-embedding in response is KAIROS-T-0190's.
-  `a_width_difference_outranks_a_model_difference` function L296-306 — `()` — Width first, because it is the one that cannot be compared at all.
-  `same_width_different_model_is_fatal` function L311-317 — `()` — The trap this exists to catch: same width, different space.
-  `same_model_different_provider_is_reported_but_not_fatal` function L322-327 — `()` — Same model and width through a different provider is reported but not
-  `cosine_is_none_across_widths_and_on_zero_vectors` function L330-334 — `()` — re-embedding in response is KAIROS-T-0190's.
-  `cosine_is_one_for_identical_and_zero_for_orthogonal` function L337-342 — `()` — re-embedding in response is KAIROS-T-0190's.
-  `a_short_batch_is_refused_rather_than_misaligned` function L345-350 — `()` — re-embedding in response is KAIROS-T-0190's.
-  `a_wrong_width_is_refused` function L353-364 — `()` — re-embedding in response is KAIROS-T-0190's.

#### crates/kairos-embed/src/local.rs

- pub `LocalConfig` struct L87-98 — `{ cache_dir: PathBuf, allow_download: bool }` — How to start the local provider.
- pub `LocalProvider` struct L110-118 — `{ model: Mutex<TextEmbedding>, id: ModelId }` — ONNX inference in-process.
- pub `new` function L125-170 — `(config: &LocalConfig) -> Result<Self, EmbedError>` — Load the model from `config.cache_dir`.
-  `MODEL` variable L58 — `: EmbeddingModel` — The model this provider uses.
-  `MODEL_NAME` variable L63 — `: &str` — Stable name recorded with every vector.
-  `DIMENSION` variable L66 — `: usize` — Vector width, asserted against the model at startup rather than trusted.
-  `MODEL_CACHE_NEEDLE` variable L70 — `: &str` — What a cache directory for this model is called, once separators are
-  `normalise` function L74-76 — `(name: &str) -> String` — Lowercase and drop the separators that differ between the model's several
-  `BATCH` variable L83 — `: usize` — How many texts go to the model at once.
-  `LocalConfig` type L100-107 — `impl Default for LocalConfig` — reason.
-  `default` function L101-106 — `() -> Self` — reason.
-  `LocalProvider` type L120-194 — `= LocalProvider` — reason.
-  `is_cached` function L186-193 — `(config: &LocalConfig) -> bool` — Whether `cache_dir` already holds something for this model.
-  `LocalProvider` type L196-215 — `impl EmbeddingProvider for LocalProvider` — reason.
-  `model_id` function L197-199 — `(&self) -> &ModelId` — reason.
-  `embed` function L201-214 — `(&self, texts: &[String]) -> Result<Vec<Embedding>, EmbedError>` — reason.
-  `tests` module L218-291 — `-` — reason.
-  `a_cache_miss_explains_itself_instead_of_downloading` function L224-241 — `()` — The cache-miss path is the one an operator meets, so its message is worth
-  `is_cached_is_false_for_a_missing_directory` function L244-249 — `()` — reason.
-  `the_real_cache_directory_name_is_recognised` function L254-262 — `()` — The real directory name fastembed creates must read as a hit.
-  `is_cached_is_true_for_a_directory_holding_the_model` function L266-276 — `()` — And end to end through `is_cached`, against a directory on disk.
-  `is_cached_is_false_for_an_empty_directory` function L281-290 — `()` — An empty directory is a miss, not a hit — otherwise an image whose model

#### crates/kairos-embed/src/remote.rs

- pub `RemoteConfig` struct L37-48 — `{ base_url: String, model: String, api_key: Option<String>, timeout: Duration }` — How to reach an OpenAI-compatible embeddings endpoint.
- pub `new` function L52-59 — `(base_url: impl Into<String>, model: impl Into<String>) -> Self` — A configuration with the default timeout.
- pub `RemoteProvider` struct L63-67 — `{ client: reqwest::blocking::Client, config: RemoteConfig, id: ModelId }` — Embeddings from an OpenAI-compatible HTTP endpoint.
- pub `new` function L89-126 — `(config: RemoteConfig) -> Result<Self, EmbedError>` — Connect, and learn the vector width by embedding one probe text.
-  `RemoteConfig` type L50-60 — `= RemoteConfig` — the vectors are placed by their `index` rather than taken in arrival order.
-  `EmbeddingsResponse` struct L70-72 — `{ data: Vec<EmbeddingDatum> }` — the vectors are placed by their `index` rather than taken in arrival order.
-  `EmbeddingDatum` struct L75-80 — `{ index: usize, embedding: Vec<f32> }` — the vectors are placed by their `index` rather than taken in arrival order.
-  `RemoteProvider` type L82-177 — `= RemoteProvider` — the vectors are placed by their `index` rather than taken in arrival order.
-  `request` function L129-176 — `(&self, texts: &[String]) -> Result<Vec<Embedding>, EmbedError>` — One HTTP round trip, vectors restored to input order.
-  `RemoteProvider` type L179-191 — `impl EmbeddingProvider for RemoteProvider` — the vectors are placed by their `index` rather than taken in arrival order.
-  `model_id` function L180-182 — `(&self) -> &ModelId` — the vectors are placed by their `index` rather than taken in arrival order.
-  `embed` function L184-190 — `(&self, texts: &[String]) -> Result<Vec<Embedding>, EmbedError>` — the vectors are placed by their `index` rather than taken in arrival order.
-  `tests` module L194-253 — `-` — the vectors are placed by their `index` rather than taken in arrival order.
-  `the_url_is_built_without_a_double_slash` function L198-204 — `()` — the vectors are placed by their `index` rather than taken in arrival order.
-  `an_unreachable_endpoint_names_itself` function L207-221 — `()` — the vectors are placed by their `index` rather than taken in arrival order.
-  `vectors_are_placed_by_index_not_arrival_order` function L226-241 — `()` — Out-of-order responses are the failure that would silently attach every
-  `a_response_missing_an_input_is_detected` function L244-252 — `()` — the vectors are placed by their `index` rather than taken in arrival order.

### crates/kairos-embed/tests

> *Semantic summary to be generated by AI agent.*

#### crates/kairos-embed/tests/local_model.rs

-  `PROVIDER` variable L31-39 — `: LazyLock<LocalProvider>` — ONE provider, shared by every test in this file.
-  `provider` function L41-43 — `() -> &'static LocalProvider` — an integration test rather than a unit one.
-  `texts` function L45-47 — `(items: &[&str]) -> Vec<String>` — an integration test rather than a unit one.
-  `the_local_model_embeds_and_advertises_itself_honestly` function L50-66 — `()` — an integration test rather than a unit one.
-  `the_same_text_gives_the_same_vector_whatever_it_travels_with` function L75-100 — `()` — **The property the model was chosen for.** A dynamically quantized model
-  `related_text_is_closer_than_unrelated_text` function L106-130 — `()` — And the vectors mean something, which the deterministic provider cannot show.
-  `batched_and_unbatched_agree` function L135-151 — `()` — Batching must not change the answer.
-  `an_empty_batch_costs_nothing` function L156-159 — `()` — The empty batch is a real case — a backfill whose page happens to be fully
...
-  `list_adrs` function L76-115 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Qu...` — `ITEM_NOT_ON_BOARD` (T-0010's typed error).
-  `get_adr` function L128-140 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Pa...` — `ITEM_NOT_ON_BOARD` (T-0010's typed error).
-  `create_adr` function L155-196 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — `ITEM_NOT_ON_BOARD` (T-0010's typed error).
-  `update_adr` function L213-250 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — `ITEM_NOT_ON_BOARD` (T-0010's typed error).
-  `delete_adr` function L265-288 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — `ITEM_NOT_ON_BOARD` (T-0010's typed error).
-  `transition_adr` function L306-326 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — `ITEM_NOT_ON_BOARD` (T-0010's typed error).
...
-  `list_documents` function L155-194 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Qu...` — pre-checks make a link failure after create unreachable in practice.
-  `get_document` function L207-219 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Pa...` — pre-checks make a link failure after create unreachable in practice.
-  `create_document` function L236-296 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — pre-checks make a link failure after create unreachable in practice.
-  `update_document` function L313-351 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — pre-checks make a link failure after create unreachable in practice.
-  `delete_document` function L366-390 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — pre-checks make a link failure after create unreachable in practice.
...
-  `list_initiatives` function L84-123 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Qu...` — T-0018 handler pattern.
-  `get_initiative` function L136-148 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Pa...` — T-0018 handler pattern.
-  `create_initiative` function L163-205 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — T-0018 handler pattern.
-  `update_initiative` function L222-260 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — T-0018 handler pattern.
-  `delete_initiative` function L276-299 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — T-0018 handler pattern.
-  `transition_initiative` function L316-343 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — T-0018 handler pattern.

#### crates/kairos-server/src/api/local_accounts.rs

- pub `router` function L57-68 — `() -> Router<AppState>` — nothing here invents a third.
- pub `CreateLocalAccountRequest` struct L72-84 — `{ email: String, display_name: Option<String>, password: String, role: Option<St...` — `POST /api/local-accounts` body.
- pub `LocalAccountView` struct L88-98 — `{ user_id: String, email: String, display_name: String, created: bool, membershi...` — `POST /api/local-accounts` 201 body.
- pub `SetPasswordRequest` struct L102-104 — `{ password: String }` — `PUT /api/local-accounts/{user_id}/password` body.
- pub `SessionView` struct L108-120 — `{ id: String, created_at: String, expires_at: String, last_used_at: Option<Strin...` — One session, for the audit listing.
- pub `SessionListResponse` struct L124-127 — `{ items: Vec<SessionView>, total: i64 }` — `GET /api/local-accounts/{user_id}/sessions` envelope.
- pub `create_local_account` function L154-228 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — nothing here invents a third.
- pub `set_local_password` function L245-277 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — nothing here invents a third.
- pub `list_sessions` function L291-325 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — nothing here invents a third.
- pub `revoke_sessions` function L344-371 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — nothing here invents a third.
-  `MANAGE` variable L55 — `: &str` — The pseudo-capability named in 403s.
-  `rfc3339` function L129-131 — `(at: chrono::DateTime<chrono::Utc>) -> String` — nothing here invents a third.
-  `hash_or_reject` function L135-139 — `(password: &str) -> Result<String, ApiError>` — Map a too-short password onto a 422.
-  `require_member_of` function L381-398 — `( conn: &mut diesel::pg::PgConnection, org_id: Uuid, user_id: Uuid, ) -> Result<...` — The target must be a member of THIS organization.
...
- pub `local_accounts` module L37 — `-` — aggregation endpoint is KAIROS-T-0023.
- pub `meta` module L38 — `-` — aggregation endpoint is KAIROS-T-0023.
- pub `org` module L39 — `-` — aggregation endpoint is KAIROS-T-0023.
- pub `proposals` module L40 — `-` — aggregation endpoint is KAIROS-T-0023.
- pub `search` module L41 — `-` — aggregation endpoint is KAIROS-T-0023.
- pub `strategies` module L42 — `-` — aggregation endpoint is KAIROS-T-0023.
- pub `tasks` module L43 — `-` — aggregation endpoint is KAIROS-T-0023.
- pub `openapi` module L45 — `-` — aggregation endpoint is KAIROS-T-0023.
- pub `router` function L64-73 — `() -> Router<AppState>` — All five entity family routers, merged (mounted behind the full
- pub `clamp_pagination` function L85-92 — `(pagination: &dto::Pagination) -> (i64, i64)` — Clamp raw S-0005 pagination params to `(limit, offset)`.
- pub `clamp_list` function L100-111 — `(query: &dto::ListQuery) -> (i64, i64, Liveness)` — Clamp an entity-family list query to `(limit, offset, liveness)`.
- pub `parse_uuid` function L119-122 — `(value: &str, field: &str) -> Result<Uuid, ApiError>` — Parse a UUID body field (`422 VALIDATION` on malformed input — the DTO
- pub `parse_opt_uuid` function L125-127 — `(value: Option<&str>, field: &str) -> Result<Option<Uuid>, ApiError>` — [`parse_uuid`] over an optional field.
- pub `board_id_by_ref` function L146-161 — `(conn: &mut PgConnection, reference: &str) -> Result<Uuid, ApiError>` — Resolve a board reference — **slug or UUID** — to a live board's id
- pub `opt_board_id_by_ref` function L164-169 — `( conn: &mut PgConnection, reference: Option<&str>, ) -> Result<Option<Uuid>, Ap...` — [`board_id_by_ref`] for an optional reference.
- pub `parse_enum` function L173-184 — `(value: &str, field: &str, allowed: &[T]) -> Result<T, ApiError>` — Parse a TEXT-backed enum body field (`task_type`, `complexity`,
- pub `require_capability` function L195-212 — `( conn: &mut PgConnection, slug: &str, board_id: Option<Uuid>, user_id: Uuid, ca...` — The A-0006 write gate: org admins bypass; otherwise the caller needs a
- pub `Liveness` enum L235-241 — `LiveOnly | IncludeArchived` — Whether a lookup may return work that has been archived.
- pub `resolve_short_code` function L253-286 — `( conn: &mut PgConnection, short_code: &str, liveness: Liveness, ) -> Result<Opt...` — Resolve a short code to `(id, entity_type)` across all five entity
- pub `resolve_item_type` function L294-308 — `(conn: &mut PgConnection, id: Uuid) -> Result<Option<ItemType>, ApiError>` — The type of a live item by id (documents included), via the same
- pub `short_code_not_found` function L316-318 — `(entity_type: &str, short_code: &str) -> ApiError` — The 404 for `/{short_code}` path segments that resolve to nothing.
- pub `map_abac_error` function L327-329 — `(e: AbacError) -> ApiError` — [`AbacError`] never carries a client mistake on the check path (grants
- pub `map_item_error` function L335-377 — `(e: ItemError) -> ApiError` — [`ItemError`] → HTTP.
- pub `map_board_error` function L383-458 — `(e: BoardError) -> ApiError` — [`BoardError`] → HTTP, for the transition endpoints: invalid moves are
- pub `map_graph_error` function L463-475 — `(e: GraphError) -> ApiError` — [`GraphError`] → HTTP, for the document-create `supports` edge: rule
-  `DEFAULT_LIMIT` variable L80 — `: i64` — Default page size when `?limit=` is omitted.
-  `MAX_LIMIT` variable L82 — `: i64` — Hard cap on `?limit=`.
-  `DirectoryRow` struct L219-224 — `{ id: Uuid, entity_type: String }` — aggregation endpoint is KAIROS-T-0023.

#### crates/kairos-server/src/api/openapi.rs

- pub `spec` function L231-233 — `() -> utoipa::openapi::OpenApi` — The aggregated OpenAPI document (also consumed by `tests/openapi.rs`,
- pub `router` function L243-250 — `(dev_ui: bool) -> Router<AppState>` — Build the module's routes.
-  `ApiDoc` struct L227 — `-` — there.
-  `SPEC_JSON` variable L236-238 — `: LazyLock<String>` — The serialized spec, built once per process.
-  `openapi_json` function L263-268 — `() -> impl IntoResponse` — there.
-  `whoami` function L296 — `()` — there.
-  `spa_config` function L321 — `()` — there.
-  `token_relay` function L345 — `()` — there.
-  `swagger_ui` function L353-355 — `() -> Html<&'static str>` — The dev-only Swagger UI page (`KAIROS_DEV_UI=true`).
-  `SWAGGER_UI_HTML` variable L361-382 — `: &str` — Kept minimal on purpose: the page is behind the same auth → tenant

#### crates/kairos-server/src/api/proposals.rs

- pub `router` function L24-29 — `() -> Router<AppState>` — already looking at it.
-  `map_proposal_error` function L32-45 — `(e: kairos_db::proposals::ProposalError) -> ApiError` — [`kairos_db::proposals::ProposalError`] → HTTP-shaped API error.
-  `to_dto` function L51-69 — `( conn: &mut diesel::pg::PgConnection, p: kairos_db::proposals::EdgeProposal, ) ...` — Map a stored proposal to its DTO, resolving both ends to short codes.
-  `list` function L83-101 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Pa...` — already looking at it.
-  `confirm` function L117-124 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Ex...` — already looking at it.
-  `reject` function L139-146 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Ex...` — already looking at it.
-  `decide` function L148-168 — `( state: AppState, tenant: TenantContext, actor: Uuid, id: Uuid, confirming: boo...` — already looking at it.

#### crates/kairos-server/src/api/search.rs

- pub `router` function L52-57 — `() -> Router<AppState>` — DTO carries `archived_at`, and it is non-null exactly for those rows.
-  `related` function L84-144 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, ax...` — DTO carries `archived_at`, and it is non-null exactly for those rows.
-  `RelatedQuery` struct L148-151 — `{ limit: Option<usize> }` — Query parameters for [`related`].
-  `search` function L168-214 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Js...` — DTO carries `archived_at`, and it is non-null exactly for those rows.
-  `field_invalid` function L221-224 — `(field: &str, message: impl Into<String>) -> ApiError` — 400 `VALIDATION` naming the offending field in `details.field`.
-  `uuid_field` function L227-230 — `(value: &str, field: &str) -> Result<Uuid, ApiError>` — Parse a UUID-carrying field.
-  `timestamp_field` function L233-242 — `(value: &str, field: &str) -> Result<DateTime<Utc>, ApiError>` — Parse an RFC 3339 timestamp field.
-  `enum_field` function L246-257 — `( value: &str, field: &str, allowed: &str, ) -> Result<T, ApiError>` — Parse a closed-vocabulary field through the core model's serde
-  `to_core` function L262-275 — `(request: &dto_search::SearchRequest) -> Result<core_search::SearchRequest, ApiE...` — Convert the wire request into the typed `kairos_core::search` request.
-  `filter_to_core` function L277-352 — `( filter: &dto_search::SearchFilter, ) -> Result<core_search::SearchFilter, ApiE...` — DTO carries `archived_at`, and it is non-null exactly for those rows.
-  `traverse_to_core` function L354-385 — `( traverse: &dto_search::SearchTraverse, ) -> Result<core_search::Traverse, ApiE...` — DTO carries `archived_at`, and it is non-null exactly for those rows.
-  `sort_to_core` function L387-396 — `(sort: &dto_search::SearchSort) -> Result<core_search::Sort, ApiError>` — DTO carries `archived_at`, and it is non-null exactly for those rows.
-  `map_validation_error` function L406-437 — `(e: SearchValidationError) -> ApiError` — [`SearchValidationError`] → 400 `VALIDATION`.
-  `map_search_error` function L440-448 — `(e: SearchError) -> ApiError` — [`SearchError`] → HTTP (module docs).
-  `into_response` function L456-481 — `(results: SearchResults) -> dto_search::SearchResponse` — Convert the pipeline's typed results into the S-0005 response shape
...
-  `list_strategies` function L83-122 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Qu...` — T-0018 handler pattern; see [`super`] for the shared conventions.
-  `get_strategy` function L135-147 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Pa...` — T-0018 handler pattern; see [`super`] for the shared conventions.
-  `create_strategy` function L161-193 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — T-0018 handler pattern; see [`super`] for the shared conventions.
-  `update_strategy` function L210-247 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — T-0018 handler pattern; see [`super`] for the shared conventions.
-  `delete_strategy` function L262-285 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — T-0018 handler pattern; see [`super`] for the shared conventions.
-  `transition_strategy` function L302-329 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — T-0018 handler pattern; see [`super`] for the shared conventions.
...
-  `list_tasks` function L148-190 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Qu...` — handler pattern.
-  `get_task` function L203-216 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Pa...` — handler pattern.
-  `create_task` function L231-286 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — handler pattern.
-  `update_task` function L303-343 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — handler pattern.
-  `delete_task` function L358-381 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — handler pattern.
-  `set_work_class` function L399-420 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — handler pattern.
-  `set_repository` function L439-467 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — handler pattern.
-  `move_task` function L487-511 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — handler pattern.
-  `transition_task` function L529-549 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — handler pattern.
...
-  `get_metadata` function L53-67 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Pa...` — history/activity rows.
-  `update_metadata` function L89-163 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — history/activity rows.
...
- pub `validated_metadata_ops` function L295-341 — `( conn: &mut PgConnection, item_type: ItemType, values: &std::collections::BTree...` — Resolve and validate every entry of a metadata write, writing nothing.
- pub `item_metadata_response` function L345-371 — `( conn: &mut PgConnection, item_id: Uuid, short_code: &str, ) -> Result<dto::Ite...` — An item's metadata values hydrated with their definitions, ordered by
...
- pub `router` function L45-77 — `() -> Router<AppState>` — capability writes require `administer_members`.
-  `CONFIGURE` variable L41 — `: &str` — The A-0006 capability for board configuration writes.
-  `ADMINISTER_MEMBERS` variable L43 — `: &str` — The A-0006 capability for board membership/capability administration.
...
-  `list_boards` function L222-254 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Qu...` — capability writes require `administer_members`.
-  `BoardDetailQuery` struct L258-268 — `{ include_removed_columns: bool }` — Query of `GET /api/boards/{id}`.
-  `get_board` function L282-297 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Pa...` — capability writes require `administer_members`.
-  `create_board` function L314-362 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — capability writes require `administer_members`.
-  `update_board` function L379-430 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — capability writes require `administer_members`.
-  `delete_board` function L447-498 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — capability writes require `administer_members`.
...
-  `board_items` function L538-728 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Pa...` — capability writes require `administer_members`.
-  `list_columns` function L745-762 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Pa...` — capability writes require `administer_members`.
-  `add_column` function L779-800 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — capability writes require `administer_members`.
-  `update_column` function L820-869 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — capability writes require `administer_members`.
-  `remove_column` function L890-914 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — capability writes require `administer_members`.
-  `list_transitions` function L931-948 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Pa...` — capability writes require `administer_members`.
-  `add_transition` function L965-995 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — capability writes require `administer_members`.
-  `remove_transition` function L1012-1047 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — capability writes require `administer_members`.
-  `capabilities_of` function L1054-1067 — `( conn: &mut PgConnection, board_id: Uuid, user_id: Uuid, ) -> Result<Vec<String...` — The `capabilities` a user holds on a board, sorted.
-  `board_member_view` function L1070-1082 — `( conn: &mut PgConnection, board_id: Uuid, user_id: Uuid, ) -> Result<dto::Board...` — One user's `BoardMember` view (joins `public.users` for identity).
-  `list_board_members` function L1096-1151 — `( State(state): State<AppState>, Extension(tenant): Extension<TenantContext>, Pa...` — capability writes require `administer_members`.
-  `add_board_member` function L1169-1195 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — capability writes require `administer_members`.
-  `replace_capabilities` function L1216-1253 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — capability writes require `administer_members`.
-  `remove_board_member` function L1272-1304 — `( State(state): State<AppState>, Extension(auth): Extension<AuthContext>, Extens...` — capability writes require `administer_members`.
...
-  `vocabulary_tests` module L343-429 — `-` — the tenant middleware, gated by `KAIROS_DEPLOYMENT_ADMINS`.
-  `server_sources` function L360-375 — `() -> Vec<std::path::PathBuf>` — Every `.rs` file under the server's `src/`, where request handling lives.
-  `walk` function L361-370 — `(dir: &std::path::Path, out: &mut Vec<std::path::PathBuf>)` — the tenant middleware, gated by `KAIROS_DEPLOYMENT_ADMINS`.
-  `every_grantable_capability_is_consulted_somewhere` function L378-402 — `()` — the tenant middleware, gated by `KAIROS_DEPLOYMENT_ADMINS`.
-  `the_two_capabilities_that_authorised_nothing_are_gone` function L405-417 — `()` — the tenant middleware, gated by `KAIROS_DEPLOYMENT_ADMINS`.
-  `the_configure_glob_still_covers_board_configuration` function L420-428 — `()` — the tenant middleware, gated by `KAIROS_DEPLOYMENT_ADMINS`.
...
- pub `AppState` struct L27-54 — `{ config: Arc<AppConfig>, pool: TenantPool, blocking: BlockingTenantPool, auth: ...` — Shared state behind every request: config, the tenant-pinning pool, and
- pub `BuildError` enum L58-65 — `Pool | Oidc` — Why [`build_state`] failed (startup-time, fail-fast).
- pub `build_state` function L73-101 — `(config: AppConfig) -> Result<AppState, BuildError>` — Build the shared state: connect the pool and resolve the OIDC issuer
- pub `state_with` function L142-159 — `(config: AppConfig, pool: TenantPool, auth: Arc<Authenticator>) -> AppState` — Build a state from an existing pool and a pre-built [`Authenticator`]
- pub `router` function L163-316 — `(state: AppState) -> Router` — The production router: `/healthz` open; everything under `/api` behind
- pub `WhoamiTeam` struct L320-327 — `{ id: Uuid, slug: String, name: String }` — A team the caller belongs to (from the tenant schema's `team_members`).
- pub `WhoamiBoardCapabilities` struct L337-345 — `{ board_id: Uuid, board_slug: String, grants: Vec<String> }` — One board on which the caller holds explicit capability grants
- pub `WhoamiResponse` struct L350-368 — `{ user: WhoamiUser, organization: WhoamiOrganization, teams: Vec<WhoamiTeam>, ca...` — `GET /api/whoami` — the S-0006 whoami precursor: proves the full
- pub `WhoamiRepository` struct L372-379 — `{ id: Uuid, slug: String, forge: String, repo_full_name: String, team_slug: Stri...` — One repository of [`WhoamiResponse::repositories`].
- pub `WhoamiUser` struct L383-392 — `{ id: Uuid, external_id: String, email: String, display_name: String }` — The `user` object of [`WhoamiResponse`].
- pub `WhoamiOrganization` struct L396-403 — `{ id: Uuid, slug: String, role: &'static str }` — The `organization` object of [`WhoamiResponse`].
- pub `serve` function L511-556 — `(config: AppConfig) -> Result<(), String>` — Run the server: build state, bind `KAIROS_BIND_ADDR`, serve with
-  `POOL_SIZE` variable L69 — `: u32` — Pool size for the server.
-  `build_embedding_service` function L113-136 — `() -> Option<Arc<crate::embedding::EmbeddingService>>` — Build the embedding service, or `None`.
-  `whoami` function L406-506 — `( Extension(auth): Extension<AuthContext>, Extension(tenant): Extension<TenantCo...` — The probe endpoint behind the full middleware stack (KAIROS-T-0017).
-  `bootstrap_first_admin` function L569-640 — `(state: &AppState) -> Result<(), String>` — Create the first-boot admin if `KAIROS_BOOTSTRAP_ADMIN` is set and the
-  `shutdown_signal` function L643-649 — `()` — Resolves when ctrl-c (SIGINT) arrives.

#### crates/kairos-server/src/blocking.rs

- pub `BlockingTenantPool` struct L24-26 — `{ pool: Pool<ConnectionManager<PgConnection>> }` — The sync pool.
- pub `new` function L41-48 — `(database_url: &str, max_size: u32) -> Self` — Build a lazy pool of at most `max_size` connections against
- pub `run` function L66-115 — `( &self, slug: &str, f: F, ) -> impl std::future::Future<Output = Result<T, ApiE...` — Run `f` on a sync connection pinned to `slug`'s tenant schema
- pub `run_public` function L124-154 — `( &self, f: F, ) -> impl std::future::Future<Output = Result<T, ApiError>> + Sen...` — Run `f` on a sync connection pinned to `search_path = public` (no
- pub `pool_state` function L159-162 — `(&self) -> (u32, u32)` — Point-in-time `(total_connections, idle_connections)` for the r2d2
-  `BlockingTenantPool` type L28-34 — `= BlockingTenantPool` — reset after the closure is defense-in-depth).
-  `fmt` function L29-33 — `(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result` — reset after the closure is defense-in-depth).
-  `BlockingTenantPool` type L36-163 — `= BlockingTenantPool` — reset after the closure is defense-in-depth).
...
- pub `AppConfig` struct L78-254 — `{ database_url: String, bind_addr: SocketAddr, oidc_issuer_url: Option<String>, ...` — Everything the server needs to run, resolved once at startup.
- pub `from_env` function L259-261 — `() -> Result<Self, ConfigError>` — Read configuration from the process environment, failing fast on
- pub `from_lookup` function L265-512 — `(lookup: impl Fn(&str) -> Option<String>) -> Result<Self, ConfigError>` — Testable core of [`Self::from_env`]: resolve from any lookup
-  `ApiBearer` type L43-52 — `= ApiBearer` — variants directly instead of mutating process environment.
-  `AppConfig` type L256-513 — `= AppConfig` — variants directly instead of mutating process environment.
-  `parse_num` function L518-530 — `( get: &impl Fn(&str) -> Option<String>, var: &'static str, default: T, ) -> Res...` — Parse an optional numeric variable, or fail naming it.
-  `tests` module L533-995 — `-` — variants directly instead of mutating process environment.
-  `lookup` function L537-543 — `(vars: &[(&str, &str)]) -> impl Fn(&str) -> Option<String>` — variants directly instead of mutating process environment.
-  `MINIMAL` variable L545-549 — `: &[(&str, &str)]` — variants directly instead of mutating process environment.
-  `minimal_config_applies_defaults` function L552-576 — `()` — variants directly instead of mutating process environment.
-  `local_auth_makes_the_issuer_optional` function L582-591 — `()` — variants directly instead of mutating process environment.
-  `without_local_auth_the_issuer_is_still_required` function L594-606 — `()` — variants directly instead of mutating process environment.
-  `half_an_issuer_is_refused` function L609-627 — `()` — variants directly instead of mutating process environment.
-  `local_auth_and_an_issuer_together_are_valid` function L630-637 — `()` — variants directly instead of mutating process environment.
-  `a_zero_session_lifetime_is_refused_when_local_auth_is_on` function L640-646 — `()` — variants directly instead of mutating process environment.
-  `missing_required_var_names_it` function L649-652 — `()` — variants directly instead of mutating process environment.
-  `empty_value_is_treated_as_unset` function L655-660 — `()` — variants directly instead of mutating process environment.
-  `invalid_bind_addr_and_log_format_are_rejected` function L663-683 — `()` — variants directly instead of mutating process environment.
-  `api_bearer_id_token_is_parsed` function L686-692 — `()` — variants directly instead of mutating process environment.
-  `optional_vars_are_carried_through` function L695-721 — `()` — variants directly instead of mutating process environment.
-  `tracing_is_off_by_default_and_samples_everything_when_on` function L728-739 — `()` — variants directly instead of mutating process environment.
-  `a_bad_sample_ratio_fails_at_boot_rather_than_silently` function L742-767 — `()` — variants directly instead of mutating process environment.
-  `the_sampling_bounds_are_inclusive` function L770-777 — `()` — variants directly instead of mutating process environment.
-  `repo_root` function L806-811 — `() -> std::path::PathBuf` — variants directly instead of mutating process environment.
-  `read_repo` function L813-816 — `(rel: &str) -> String` — variants directly instead of mutating process environment.
-  `read_by_binary` function L820-867 — `() -> BTreeSet<String>` — Every variable the binary asks for, across all three config types and
-  `env_keys_in` function L872-888 — `(yaml: &str) -> BTreeSet<String>` — Environment keys a YAML `environment:`/`data:` block sets, by the shape
-  `documented_in_env_example` function L893-907 — `() -> BTreeSet<String>` — Variables `.env.example` tells an operator to set (`NAME=` at column 0,
-  `NOT_THE_BINARYS` variable L911-923 — `: &[&str]` — Consumed by the deployment machinery itself rather than by the binary,
-  `deployments_set_only_variables_the_binary_reads` function L926-956 — `()` — variants directly instead of mutating process environment.
-  `compose_forwards_every_variable_the_env_example_documents` function L959-978 — `()` — variants directly instead of mutating process environment.
-  `the_variables_google_workspace_requires_reach_the_container` function L981-994 — `()` — variants directly instead of mutating process environment.

#### crates/kairos-server/src/embedding.rs

- pub `RefreshError` enum L48-58 — `Store | Provider | UnknownEntityType` — Errors from a refresh.
- pub `RefreshOutcome` struct L62-69 — `{ primary_embedded: bool, chunks_embedded: usize, chunks_total: usize }` — What a refresh actually did.
- pub `did_work` function L73-75 — `(&self) -> bool` — Whether anything at all was sent to the model.
- pub `embedded` function L78-80 — `(&self) -> usize` — How many texts were embedded.
- pub `BatchOutcome` struct L85-92 — `{ items_seen: usize, items_changed: usize, texts_embedded: usize }` — What a batch did.
- pub `REFRESH_BATCH` variable L100 — `: i64` — How many items a background sweep looks at per tenant per tick.
- pub `EmbeddingService` struct L103-106 — `{ provider: Arc<dyn EmbeddingProvider>, model: StoredModel }` — Keeps an item's vectors in line with its text.
- pub `new` function L129-137 — `(provider: Arc<dyn EmbeddingProvider>) -> Self` — Wrap a provider.
- pub `model` function L140-142 — `(&self) -> &StoredModel` — The model every stored row is compared against.
- pub `model_display` function L145-150 — `(&self) -> String` — The model, for a log line an operator will read.
- pub `refresh_item` function L156-241 — `( &self, conn: &mut diesel::pg::PgConnection, item: &PendingItem, metadata: &[(S...` — Bring one item's vectors up to date.
- pub `refresh_batch` function L248-254 — `( &self, conn: &mut diesel::pg::PgConnection, limit: i64, ) -> Result<BatchOutco...` — Refresh up to `limit` items, oldest short code first.
- pub `refresh_page` function L264-292 — `( &self, conn: &mut diesel::pg::PgConnection, limit: i64, offset: i64, ) -> Resu...` — One page of a sweep, starting at `offset`.
- pub `run_refresher` function L321-391 — `( blocking: crate::blocking::BlockingTenantPool, service: Arc<EmbeddingService>,...` — Run a refresh sweep across every tenant, for ever.
- pub `RelatedWork` struct L439-447 — `{ short_code: String, proposals: Vec<kairos_core::retrieval::Proposal>, sources:...` — The answer to "what is related to this?".
- pub `related_work` function L467-522 — `( &self, conn: &mut diesel::pg::PgConnection, short_code: &str, config: &kairos_...` — Find work related to `short_code`, as bounded proposals.
-  `RefreshOutcome` type L71-81 — `= RefreshOutcome` — directly from a handler.
-  `EmbeddingService` type L108-114 — `= EmbeddingService` — directly from a handler.
-  `fmt` function L109-113 — `(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result` — directly from a handler.
-  `parse_entity_type` function L116-125 — `(value: &str) -> Result<ItemType, RefreshError>` — directly from a handler.
-  `EmbeddingService` type L127-293 — `= EmbeddingService` — directly from a handler.
-  `tests` module L394-431 — `-` — directly from a handler.
-  `every_entity_type_parses_and_nothing_else_does` function L398-410 — `()` — directly from a handler.
-  `an_outcome_reports_what_it_sent_to_the_model` function L413-430 — `()` — directly from a handler.
-  `CANDIDATE_DEPTH` variable L454 — `: i64` — How many candidates each source contributes before fusion.
-  `EmbeddingService` type L456-523 — `= EmbeddingService` — directly from a handler.
-  `blank` function L525-538 — `(n: &kairos_db::embeddings::Neighbour) -> kairos_core::retrieval::Candidate` — directly from a handler.
...
- pub `too_many_requests` function L60-67 — `(retry_after_secs: u64) -> Self` — 429 `TOO_MANY_REQUESTS` — the auth throttle is holding this caller off
- pub `membership_required` function L72-82 — `(slug: &str) -> Self` — 403 `MEMBERSHIP_REQUIRED` — authenticated but not a member of the
- pub `forbidden` function L85-87 — `(message: impl Into<String>) -> Self` — 403 `FORBIDDEN` — authenticated and a member, but not allowed.
- pub `tenant_not_found` function L91-93 — `(message: impl Into<String>) -> Self` — 404 `TENANT_NOT_FOUND` — no organization for the resolved slug (or
- pub `not_found` function L96-98 — `(message: impl Into<String>) -> Self` — 404 `NOT_FOUND` — generic missing resource.
- pub `conflict` function L103-105 — `(message: impl Into<String>) -> Self` — 409 `CONFLICT` — KAIROS-A-0004 optimistic-concurrency rejection.
- pub `unprocessable` function L109-111 — `(code: &'static str, message: impl Into<String>) -> Self` — 422 with a caller-chosen code — the semantic-rejection family
- pub `validation` function L116-118 — `(message: impl Into<String>) -> Self` — 422 `VALIDATION` — a body/reference the request names is malformed
- pub `capability_required` function L123-132 — `(capability: &str, board_id: Option<uuid::Uuid>) -> Self` — 403 `FORBIDDEN` naming the missing KAIROS-A-0006 capability in
- pub `internal` function L136-143 — `(context: impl std::fmt::Display) -> Self` — 500 `INTERNAL` — the message is logged; the envelope carries a
-  `ApiError` type L27-144 — `= ApiError` — middleware layer and handler in this crate (KAIROS-T-0017).
-  `ApiError` type L146-157 — `impl IntoResponse for ApiError` — middleware layer and handler in this crate (KAIROS-T-0017).
-  `into_response` function L147-156 — `(self) -> Response` — middleware layer and handler in this crate (KAIROS-T-0017).
-  `tests` module L160-186 — `-` — middleware layer and handler in this crate (KAIROS-T-0017).
-  `envelope_matches_s0005_shape` function L164-176 — `()` — middleware layer and handler in this crate (KAIROS-T-0017).
-  `membership_required_names_the_org_and_access_path` function L179-185 — `()` — middleware layer and handler in this crate (KAIROS-T-0017).
...
- pub `embedding` module L20 — `-` — - [`web`] — GUI serving + SPA auth support (A-0015, KAIROS-T-0039)
- pub `error` module L21 — `-` — - [`web`] — GUI serving + SPA auth support (A-0015, KAIROS-T-0039)
- pub `forge` module L22 — `-` — - [`web`] — GUI serving + SPA auth support (A-0015, KAIROS-T-0039)
- pub `local_auth` module L23 — `-` — - [`web`] — GUI serving + SPA auth support (A-0015, KAIROS-T-0039)
- pub `login` module L24 — `-` — - [`web`] — GUI serving + SPA auth support (A-0015, KAIROS-T-0039)
- pub `metrics` module L25 — `-` — - [`web`] — GUI serving + SPA auth support (A-0015, KAIROS-T-0039)
- pub `middleware` module L26 — `-` — - [`web`] — GUI serving + SPA auth support (A-0015, KAIROS-T-0039)
- pub `rate_limit` module L27 — `-` — - [`web`] — GUI serving + SPA auth support (A-0015, KAIROS-T-0039)
- pub `ws` module L28 — `-` — - [`web`] — GUI serving + SPA auth support (A-0015, KAIROS-T-0039)
- pub `mcp` module L30 — `-` — - [`web`] — GUI serving + SPA auth support (A-0015, KAIROS-T-0039)
- pub `scim` module L32 — `-` — - [`web`] — GUI serving + SPA auth support (A-0015, KAIROS-T-0039)
- pub `service_accounts` module L34 — `-` — - [`web`] — GUI serving + SPA auth support (A-0015, KAIROS-T-0039)
- pub `web` module L36 — `-` — - [`web`] — GUI serving + SPA auth support (A-0015, KAIROS-T-0039)
- pub `TracingGuard` struct L50-52 — `{ provider: Option<opentelemetry_sdk::trace::SdkTracerProvider> }` — Held by `main` for the process's life; flushes pending spans on drop.
- pub `init_tracing` function L76-126 — `(config: &AppConfig) -> TracingGuard` — Initialize `tracing-subscriber` per KAIROS-A-0013: `KAIROS_LOG_LEVEL` filter
-  `TracingGuard` type L54-64 — `impl Drop for TracingGuard` — - [`web`] — GUI serving + SPA auth support (A-0015, KAIROS-T-0039)
-  `drop` function L55-63 — `(&mut self)` — - [`web`] — GUI serving + SPA auth support (A-0015, KAIROS-T-0039)
-  `tracer_provider` function L129-155 — `( endpoint: &str, sample_ratio: f64, ) -> Result<opentelemetry_sdk::trace::SdkTr...` — Build the OTLP/HTTP exporter and a batching provider around it.

#### crates/kairos-server/src/local_auth.rs

- pub `SESSION_PREFIX` variable L39 — `: &str` — The fixed session-token prefix.
- pub `SESSION_SECRET_LEN` variable L42 — `: usize` — 32 random bytes → 64 hex characters, matching the API-key secret length.
- pub `PasswordError` enum L72-80 — `Hash | MalformedStoredHash` — Why a password could not be hashed or verified.
- pub `hash_password` function L90-98 — `(password: &str) -> Result<String, PasswordError>` — Hash a password for storage.
- pub `verify_password` function L104-110 — `(password: &str, stored: &str) -> Result<bool, PasswordError>` — Verify a password against a stored PHC string.
- pub `verify_against_dummy` function L119-125 — `(password: &str) -> bool` — Spend the same work as a real verification, and return false.
- pub `MIN_PASSWORD_LEN` variable L134 — `: usize` — The shortest password Kairos will store (KAIROS-T-0204).
- pub `validate_password` function L145-154 — `(password: &str) -> Result<(), PasswordTooShort>` — Check a password is long enough, counting CHARACTERS rather than bytes.
- pub `PasswordTooShort` struct L163-168 — `{ minimum: usize, got: usize }` — A password below [`MIN_PASSWORD_LEN`].
- pub `generate_session_token` function L186-190 — `() -> String` — Mint a session token: `kairos_ss_<64-hex>`.
- pub `hash_session_token` function L194-196 — `(token: &str) -> String` — The value stored at rest: hex SHA-256 of the full token.
- pub `parse_session_token` function L203-210 — `(token: &str) -> Option<&str>` — The secret part of a presented token, or `None` if it is not shaped like one.
- pub `is_session_token` function L215-217 — `(token: &str) -> bool` — True if the bearer looks like a session token, so `require_auth` takes the
-  `ARGON2_M_COST` variable L61 — `: u32` — argon2id parameters (KAIROS-T-0201).
-  `ARGON2_T_COST` variable L62 — `: u32` — tested without a database.
-  `ARGON2_P_COST` variable L63 — `: u32` — tested without a database.
-  `hasher` function L82-86 — `() -> Argon2<'static>` — tested without a database.
-  `DUMMY` variable L122-123 — `: &str` — tested without a database.
-  `hex` function L170-172 — `(bytes: &[u8]) -> String` — tested without a database.
-  `tests` module L220-359 — `-` — tested without a database.
-  `a_password_verifies_against_its_own_hash_and_nothing_else` function L225-230 — `()` — tested without a database.
-  `the_stored_hash_carries_its_parameters` function L233-239 — `()` — tested without a database.
-  `the_same_password_hashes_differently_every_time` function L242-250 — `()` — tested without a database.
-  `a_malformed_stored_hash_is_an_error_not_a_wrong_password` function L253-265 — `()` — tested without a database.
-  `the_dummy_verification_does_work_and_always_fails` function L268-276 — `()` — tested without a database.
-  `the_password_floor_counts_characters_not_bytes` function L279-293 — `()` — tested without a database.
-  `the_floor_message_is_specific_because_the_setter_knows_the_password` function L296-301 — `()` — tested without a database.
-  `whitespace_is_part_of_the_password` function L304-315 — `()` — tested without a database.
-  `a_session_token_round_trips` function L318-324 — `()` — tested without a database.
-  `session_tokens_are_not_api_keys_and_api_keys_are_not_sessions` function L327-335 — `()` — tested without a database.
-  `a_malformed_session_token_does_not_parse` function L338-350 — `()` — tested without a database.
-  `hashing_a_token_is_stable_and_one_way` function L353-358 — `()` — tested without a database.

#### crates/kairos-server/src/login.rs

- pub `router` function L58-62 — `() -> Router<AppState>` — `/api/login` + `/api/logout`.
- pub `LoginRequest` struct L66-70 — `{ email: String, password: String }` — `POST /api/login` body.
- pub `LoginResponse` struct L75-83 — `{ token: String, expires_at: String, user: LoginUser }` — `POST /api/login` 200 body.
- pub `LoginUser` struct L87-91 — `{ id: String, email: String, display_name: String }` — The `user` object of [`LoginResponse`].
- pub `login` function L132-218 — `(State(state): State<AppState>, req: Request) -> Result<Response, ApiError>` — thread that is tens of milliseconds during which that thread serves nobody.
- pub `logout` function L231-251 — `(State(state): State<AppState>, req: Request) -> Result<StatusCode, ApiError>` — thread that is tens of milliseconds during which that thread serves nobody.
- pub `authenticate_session` function L258-303 — `(state: &AppState, token: &str) -> Result<AuthContext, ApiError>` — Authenticate a session bearer, for `require_auth`'s third branch.
- pub `expiry_from` function L307-309 — `(now: DateTime<Utc>, ttl_secs: u64) -> DateTime<Utc>` — When a session minted now would expire — exposed for tests and for the CLI's
-  `bad_login` function L98-100 — `() -> ApiError` — The ONE failure for every way a login can fail.
-  `bad_session` function L103-105 — `() -> ApiError` — The ONE failure for every way a session bearer can fail.
-  `tests` module L312-351 — `-` — thread that is tens of milliseconds during which that thread serves nobody.
-  `every_login_failure_carries_the_same_message` function L316-331 — `()` — thread that is tens of milliseconds during which that thread serves nobody.
-  `a_session_failure_says_nothing_about_which_kind` function L334-341 — `()` — thread that is tens of milliseconds during which that thread serves nobody.
-  `the_default_session_lifetime_is_a_fortnight` function L344-350 — `()` — thread that is tens of milliseconds during which that thread serves nobody.

#### crates/kairos-server/src/main.rs

-  `connect_and_migrate_public` function L51-72 — `() -> Result<PgConnection, String>` — Establish the migration connection from `DATABASE_URL` and apply pending
-  `flag_value` function L75-83 — `(args: &[String], flag: &str) -> Result<Option<String>, String>` — Value of `--flag <value>` in `args`, if present.
-  `has_flag` function L85-87 — `(args: &[String], flag: &str) -> bool` — version and exits 0.
-  `create_tenant` function L89-108 — `(conn: &mut PgConnection, args: &[String]) -> Result<(), String>` — version and exits 0.
-  `drop_tenant` function L110-120 — `(conn: &mut PgConnection, args: &[String]) -> Result<(), String>` — version and exits 0.
-  `password_arg` function L127-144 — `(args: &[String], what: &str) -> Result<String, String>` — Read a password from `--password`, or from stdin when it is absent.
-  `set_password` function L157-206 — `(conn: &mut PgConnection, args: &[String]) -> Result<(), String>` — `set-password --email <email> [--password <pw>]` (KAIROS-T-0204).
-  `hash_password_cmd` function L214-223 — `(args: &[String]) -> Result<(), String>` — `hash-password [--password <pw>]` (KAIROS-T-0204).
-  `migrate_tenants` function L225-249 — `(conn: &mut PgConnection) -> Result<(), String>` — version and exits 0.
-  `list_tenants` function L251-267 — `(conn: &mut PgConnection) -> Result<(), String>` — version and exits 0.
-  `seed_demo` function L274-313 — `(conn: &mut PgConnection, args: &[String]) -> Result<(), String>` — The `seed-demo` subcommand (KAIROS-T-0035, KAIROS-A-0012 fixtures):
-  `serve` function L318-343 — `() -> Result<(), String>` — The `serve` subcommand (KAIROS-T-0017): fail-fast config, tracing init
-  `embed_backfill` function L362-473 — `(conn: &mut PgConnection, args: &[String]) -> Result<(), String>` — `embed-backfill [--tenant <slug>] [--batch N] [--max-batches N] [--pause-ms N]`
-  `flag` function L365-370 — `(args: &[String], name: &str) -> Option<String>` — version and exits 0.
-  `number` function L371-378 — `(args: &[String], name: &str, default: u64) -> Result<u64, String>` — version and exits 0.
-  `embed_index` function L482-536 — `(conn: &mut PgConnection, args: &[String]) -> Result<(), String>` — `embed-index [--tenant <slug>]` (KAIROS-T-0190): pin the vector columns to
-  `run` function L538-575 — `() -> Result<bool, String>` — version and exits 0.
-  `main` function L577-589 — `() -> ExitCode` — version and exits 0.
-  `tests` module L592-627 — `-` — version and exits 0.
-  `smoke` function L596-598 — `()` — version and exits 0.
-  `args` function L600-602 — `(list: &[&str]) -> Vec<String>` — version and exits 0.
-  `flag_value_parses_pairs` function L605-613 — `()` — version and exits 0.
-  `flag_value_rejects_missing_value` function L616-619 — `()` — version and exits 0.
-  `has_flag_detects_presence` function L622-626 — `()` — version and exits 0.

#### crates/kairos-server/src/metrics.rs

- pub `Metrics` struct L88-93 — `{ http: Mutex<BTreeMap<HttpKey, Histogram>>, tenants: Mutex<BTreeMap<String, u64...` — A per-router metrics registry (held as `Arc<Metrics>` in
- pub `new` function L97-99 — `() -> Self` — Fresh, empty registry.
- pub `record_lockout` function L116-123 — `(&self, kind: &str)` — Increment the lockout counter for a subject kind (`"identity"` or
- pub `track_metrics` function L210-277 — `(State(state): State<AppState>, req: Request, next: Next) -> Response` — The outer HTTP-metrics layer (KAIROS-A-0013): times every request, then
- pub `metrics_handler` function L283-314 — `(State(state): State<AppState>) -> Response` — `GET /metrics` (KAIROS-A-0013): the Prometheus scrape endpoint.
- pub `readyz` function L320-358 — `(State(state): State<AppState>) -> Response` — `GET /readyz` (KAIROS-A-0013): readiness = database connectivity AND no
-  `BUCKETS` variable L52-54 — `: &[f64]` — Histogram bucket upper bounds (`le`), in seconds — the Prometheus client
-  `Histogram` struct L60-67 — `{ buckets: [u64; BUCKETS.len()], sum: f64, count: u64 }` — One HTTP histogram: per-bucket cumulative counts (each entry counts
-  `Histogram` type L69-79 — `= Histogram` — are non-`/api` so they are outside the S-0005 surface entirely.
-  `observe` function L70-78 — `(&mut self, value: f64)` — are non-`/api` so they are outside the S-0005 surface entirely.
-  `HttpKey` type L82 — `= (String, String, String)` — The label triple for an HTTP histogram series.
-  `Metrics` type L95-201 — `= Metrics` — are non-`/api` so they are outside the S-0005 surface entirely.
-  `record_http` function L102-110 — `(&self, method: &str, route: &str, status: u16, secs: f64)` — Record one completed HTTP request into the duration histogram.
-  `record_tenant` function L126-133 — `(&self, tenant: &str)` — Increment the per-tenant request counter.
-  `render_http_and_tenants` function L138-200 — `(&self, out: &mut String)` — Render the accumulated HTTP histograms and per-tenant counters into
-  `tests` module L361-400 — `-` — are non-`/api` so they are outside the S-0005 surface entirely.
-  `histogram_buckets_are_cumulative` function L365-379 — `()` — are non-`/api` so they are outside the S-0005 surface entirely.
-  `render_emits_expected_families` function L382-399 — `()` — are non-`/api` so they are outside the S-0005 surface entirely.

#### crates/kairos-server/src/rate_limit.rs

- pub `Subject` enum L59-64 — `Identity | Source` — What is being counted against.
- pub `kind` function L69-74 — `(&self) -> &'static str` — The metric/log label for this kind of subject.
- pub `ThrottleConfig` struct L80-87 — `{ max_failures: u32, window: Duration, lockout: Duration }` — Thresholds.
- pub `AuthThrottle` struct L116-119 — `{ config: ThrottleConfig, buckets: Mutex<HashMap<Subject, Bucket>> }` — The throttle.
- pub `LockedOut` struct L123-126 — `{ retry_after_secs: u64 }` — A subject is locked out, and for how much longer.
- pub `new` function L129-134 — `(config: ThrottleConfig) -> Self` — flaky.
- pub `check` function L141-157 — `(&self, subjects: &[Subject], now: Instant) -> Result<(), LockedOut>` — May these subjects attempt authentication?
- pub `record_failure` function L161-191 — `(&self, subjects: &[Subject], now: Instant) -> Option<LockedOut>` — Record a failed attempt, and lock out any subject that has now had too
- pub `record_success` function L198-209 — `(&self, subjects: &[Subject], now: Instant)` — Record a success, clearing the subjects' counters.
- pub `evict_expired` function L217-226 — `(&self, now: Instant) -> usize` — Drop entries that can no longer affect a decision.
- pub `tracked` function L229-231 — `(&self) -> usize` — How many subjects are being tracked (for the metric, and for tests).
- pub `from_config` function L236-244 — `(config: &crate::config::AppConfig) -> Option<AuthThrottle>` — Build the throttle this deployment's configuration asks for, or `None` when
- pub `client_addr` function L269-285 — `(req: &Request, trusted_proxy: bool) -> Option<IpAddr>` — The client address to count against, or `None` when nothing trustworthy is
- pub `locked_out_response` function L294-300 — `(locked: LockedOut) -> Response` — The 429 for a locked-out caller, with `Retry-After`.
- pub `Attempt` struct L313-318 — `{ throttle: Option<&'a AuthThrottle>, metrics: &'a crate::metrics::Metrics, subj...` — One authentication attempt, from the throttle's point of view.
- pub `begin` function L327-345 — `( state: &'a crate::app::AppState, source: Option<IpAddr>, identity: Option<Stri...` — Begin an attempt against `identity` (the thing being authenticated *as*,
- pub `refuse_if_locked_out` function L353-365 — `(&self) -> Option<Response>` — The 429 to return instead of checking the credential, if this caller is
- pub `failed` function L368-390 — `(&self)` — Report that the credential did not verify.
- pub `succeeded` function L393-397 — `(&self)` — Report that the credential verified, clearing the counters.
-  `Subject` type L66-75 — `= Subject` — flaky.
-  `ThrottleConfig` type L89-103 — `impl Default for ThrottleConfig` — flaky.
-  `default` function L90-102 — `() -> Self` — flaky.
-  `Bucket` struct L106-111 — `{ failures: u32, window_started: Instant, locked_until: Option<Instant> }` — flaky.
-  `AuthThrottle` type L128-232 — `= AuthThrottle` — flaky.
-  `EVICT_ABOVE` variable L305 — `: usize` — Above this many tracked subjects, a recorded failure also sweeps the expired
-  `tests` module L401-559 — `-` — flaky.
-  `subjects` function L404-409 — `() -> Vec<Subject>` — flaky.
-  `cfg` function L411-417 — `() -> ThrottleConfig` — flaky.
-  `failures_below_the_threshold_do_not_lock_out` function L420-427 — `()` — flaky.
-  `the_threshold_locks_out_and_the_lockout_expires` function L430-444 — `()` — flaky.
-  `failures_decay_so_a_slow_typist_is_never_locked_out` function L447-457 — `()` — flaky.
-  `identity_and_source_are_counted_separately` function L460-478 — `()` — flaky.
-  `a_success_clears_the_count_but_not_a_live_lockout` function L481-498 — `()` — flaky.
-  `eviction_drops_only_what_cannot_matter` function L501-515 — `()` — flaky.
-  `a_locked_out_subject_is_not_evicted_early` function L518-538 — `()` — flaky.
-  `retry_after_rounds_up` function L541-558 — `()` — flaky.

#### crates/kairos-server/src/web.rs

- pub `router` function L137-150 — `(state: &AppState) -> Router<AppState>` — The web routes: `/api/config` + `/api/auth/token`.
- pub `spa_fallback` function L318-336 — `(State(state): State<AppState>, method: Method, uri: Uri) -> Response` — The router fallback (mounted in [`crate::app::router`]): reserved
...
-  `WebAuth` struct L82-95 — `{ issuer: Option<String>, client_id: String, api_bearer: crate::config::ApiBeare...` — Shared context for the two auth routes: issuer + client id from
-  `WebAuth` type L97-129 — `= WebAuth` — tooling).
-  `endpoints` function L106-128 — `(&self) -> Result<&IdpEndpoints, ApiError>` — Discovery document fetch (`{issuer}/.well-known/openid-configuration`),
-  `idp_unreachable` function L131-133 — `(message: String) -> ApiError` — tooling).
-  `SpaConfig` struct L154-168 — `{ issuer: Option<String>, client_id: String, authorization_endpoint: Option<Stri...` — `GET /api/config` response body (mirrored by `kairos-web::auth`).
-  `spa_config` function L172-191 — `( State(state): State<AppState>, Extension(web_auth): Extension<Arc<WebAuth>>, )...` — `GET /api/config` — see module docs.
-  `TokenRelayForm` struct L196-202 — `{ grant_type: String, code: Option<String>, redirect_uri: Option<String>, code_v...` — What the SPA may relay.
-  `relay_params` function L209-252 — `( client_id: &'a str, client_secret: Option<&'a str>, form: &'a TokenRelayForm, ...` — Assemble the whitelisted form parameters to relay to the issuer's token
-  `token_relay` function L259-286 — `( Extension(web_auth): Extension<Arc<WebAuth>>, form: Result<Form<TokenRelayForm...` — `POST /api/auth/token` — the same-origin token relay (module docs).
-  `missing` function L289-291 — `(field: &'static str) -> impl FnOnce() -> ApiError` — Builder for the "required form field is missing" error.
-  `is_reserved` function L298-305 — `(path: &str) -> bool` — Is this path owned by an API surface (never SPA-fallback material)?
-  `wants_index_fallback` function L310-314 — `(rel: &str) -> bool` — Should a missing file fall back to `index.html`? Yes for route-like
-  `serve_from_dir` function L339-360 — `(dist: &std::path::Path, rel: &str) -> Response` — Dev serving: files out of `KAIROS_WEB_DIST`, with the SPA fallback.
-  `serve_embedded_or_placeholder` function L364-379 — `(rel: &str) -> Response` — Release serving: assets embedded by the `embed-web` feature.
-  `Assets` struct L368 — `-` — tooling).
-  `serve_embedded_or_placeholder` function L384-406 — `(_rel: &str) -> Response` — No dist dir, no embedded assets: an honest placeholder so plain
-  `PLACEHOLDER` variable L385-396 — `: &str` — tooling).
-  `asset_response` function L411-427 — `(name: &str, bytes: Bytes) -> Response` — An asset body with content type + cache policy.
-  `tests` module L430-554 — `-` — tooling).
-  `auth_code_form` function L433-441 — `() -> TokenRelayForm` — tooling).
-  `relay_includes_client_secret_only_when_configured` function L446-460 — `()` — The relay appends `client_secret` iff configured (KAIROS-T-0056), and
-  `relay_secret_applies_to_refresh_grant` function L465-476 — `()` — The secret is applied to the refresh grant too (Google refreshes are
-  `relay_rejects_unknown_grant` function L480-489 — `()` — An unsupported grant is rejected even with a secret configured.
-  `reserved_prefixes_match_paths_not_strings` function L492-519 — `()` — tooling).
-  `index_fallback_only_for_route_like_paths` function L522-530 — `()` — tooling).
-  `asset_responses_carry_mime_and_cache_policy` function L533-553 — `()` — tooling).
...
-  `protected_resource_metadata` function L134-154 — `( State(state): State<AppState>, headers: HeaderMap, ) -> Json<Value>` — RFC 9728 protected-resource metadata: the `/mcp` resource identifier
...
- pub `ProposeEdgeParams` struct L126-136 — `{ source: String, target: String, relationship: String, why: String }` — Parameters for `propose_edge` (KAIROS-T-0192).
- pub `RelatedWorkParams` struct L141-147 — `{ short_code: String, limit: Option<usize> }` — Parameters for `related_work` (KAIROS-T-0191).
- pub `SearchParams` struct L151-166 — `{ q: Option<String>, filter: Option<SearchFilterParams>, traverse: Option<Search...` — `activity_log` writes as the API path).
- pub `SearchFilterParams` struct L170-199 — `{ entity_type: Option<Vec<String>>, board_id: Option<String>, column_id: Option<...` — `activity_log` writes as the API path).
- pub `SearchTraverseParams` struct L203-212 — `{ from: String, relationships: Vec<String>, direction: String, depth: Option<u32...` — `activity_log` writes as the API path).
- pub `SearchSortParams` struct L216-221 — `{ field: String, order: String }` — `activity_log` writes as the API path).
- pub `CreateItemParams` struct L225-273 — `{ item_type: String, title: String, board: Option<String>, parent: Option<String...` — `activity_log` writes as the API path).
- pub `UpdateItemParams` struct L277-287 — `{ short_code: String, title: Option<String>, content: String, version: i32 }` — `activity_log` writes as the API path).
- pub `EditItemParams` struct L291-302 — `{ short_code: String, search: String, replace: String, replace_all: bool }` — `activity_log` writes as the API path).
- pub `TransitionItemParams` struct L306-312 — `{ short_code: String, to_column: String }` — `activity_log` writes as the API path).
- pub `MoveItemParams` struct L316-322 — `{ short_code: String, to_board: String }` — `activity_log` writes as the API path).
- pub `LinkItemsParams` struct L326-333 — `{ source: String, target: String, relationship: String }` — `activity_log` writes as the API path).
- pub `UnlinkItemsParams` struct L337-344 — `{ source: String, target: String, relationship: String }` — `activity_log` writes as the API path).
- pub `SetMetadataParams` struct L348-355 — `{ short_code: String, values: BTreeMap<String, Option<String>> }` — `activity_log` writes as the API path).
- pub `RestoreItemParams` struct L359-362 — `{ short_code: String }` — `activity_log` writes as the API path).
- pub `DeleteItemParams` struct L366-372 — `{ short_code: String, confirm: bool }` — `activity_log` writes as the API path).
- pub `whoami` function L395-480 — `( &self, context: RequestContext<RoleServer>, ) -> Result<CallToolResult, ErrorD...` — `activity_log` writes as the API path).
- pub `list_repositories` function L485-528 — `( &self, Parameters(params): Parameters<ListRepositoriesParams>, context: Reques...` — `activity_log` writes as the API path).
- pub `get_repository` function L533-602 — `( &self, Parameters(params): Parameters<GetRepositoryParams>, context: RequestCo...` — `activity_log` writes as the API path).
- pub `my_boards` function L607-679 — `( &self, Parameters(params): Parameters<MyBoardsParams>, context: RequestContext...` — `activity_log` writes as the API path).
- pub `board_items` function L684-746 — `( &self, Parameters(params): Parameters<BoardItemsParams>, context: RequestConte...` — `activity_log` writes as the API path).
- pub `get_item` function L751-875 — `( &self, Parameters(params): Parameters<GetItemParams>, context: RequestContext<...` — `activity_log` writes as the API path).
- pub `get_history` function L880-953 — `( &self, Parameters(params): Parameters<GetHistoryParams>, context: RequestConte...` — `activity_log` writes as the API path).
- pub `propose_edge` function L958-997 — `( &self, Parameters(params): Parameters<ProposeEdgeParams>, context: RequestCont...` — `activity_log` writes as the API path).
- pub `related_work` function L1002-1029 — `( &self, Parameters(params): Parameters<RelatedWorkParams>, context: RequestCont...` — `activity_log` writes as the API path).
- pub `search` function L1034-1073 — `( &self, Parameters(params): Parameters<SearchParams>, context: RequestContext<R...` — `activity_log` writes as the API path).
- pub `create_item` function L1078-1090 — `( &self, Parameters(params): Parameters<CreateItemParams>, context: RequestConte...` — `activity_log` writes as the API path).
- pub `update_item` function L1095-1120 — `( &self, Parameters(params): Parameters<UpdateItemParams>, context: RequestConte...` — `activity_log` writes as the API path).
- pub `edit_item` function L1125-1181 — `( &self, Parameters(params): Parameters<EditItemParams>, context: RequestContext...` — `activity_log` writes as the API path).
- pub `move_item` function L1186-1242 — `( &self, Parameters(params): Parameters<MoveItemParams>, context: RequestContext...` — `activity_log` writes as the API path).
- pub `transition_item` function L1247-1308 — `( &self, Parameters(params): Parameters<TransitionItemParams>, context: RequestC...` — `activity_log` writes as the API path).
- pub `link_items` function L1313-1342 — `( &self, Parameters(params): Parameters<LinkItemsParams>, context: RequestContex...` — `activity_log` writes as the API path).
- pub `unlink_items` function L1347-1376 — `( &self, Parameters(params): Parameters<UnlinkItemsParams>, context: RequestCont...` — `activity_log` writes as the API path).
- pub `set_metadata` function L1381-1459 — `( &self, Parameters(params): Parameters<SetMetadataParams>, context: RequestCont...` — `activity_log` writes as the API path).
- pub `delete_item` function L1464-1496 — `( &self, Parameters(params): Parameters<DeleteItemParams>, context: RequestConte...` — `activity_log` writes as the API path).
- pub `restore_item` function L1501-1546 — `( &self, Parameters(params): Parameters<RestoreItemParams>, context: RequestCont...` — `activity_log` writes as the API path).
-  `KairosMcp` type L379-1547 — `= KairosMcp` — `activity_log` writes as the API path).
-  `run_tool` function L382-390 — `(&self, tenant: &TenantContext, f: F) -> Result<CallToolResult, ErrorData>` — Run one closure on a tenant-pinned sync connection (the T-0018
-  `ItemView` struct L1554-1580 — `{ id: Uuid, item_type: ItemType, short_code: String, title: String, content: Str...` — A uniform projection of any live item, whatever its table.
-  `load_item` function L1589-1764 — `( conn: &mut PgConnection, short_code: &str, liveness: Liveness, ) -> Result<Ite...` — Resolve a short code and load its [`ItemView`]; 404 `NOT_FOUND`
-  `authorize_item_write` function L1769-1784 — `( conn: &mut PgConnection, slug: &str, user: Uuid, item: &ItemView, ) -> Result<...` — The A-0006 write gate for an item: `manage_<type>` on the item's
-  `require_capability_explained` function L1791-1834 — `( conn: &mut PgConnection, slug: &str, board_id: Option<Uuid>, user: Uuid, capab...` — `require_capability`, but when the caller is a cross-team filer — no
-  `board_by_ref` function L1837-1852 — `(conn: &mut PgConnection, reference: &str) -> Result<Board, ApiError>` — Resolve a board by UUID or slug; 404 `NOT_FOUND` otherwise.
-  `team_by_ref` function L1855-1874 — `( conn: &mut PgConnection, reference: &str, ) -> Result<kairos_db::models::teams...` — Resolve a team by UUID or slug; 422 otherwise (a filter value).
-  `board_by_id` function L1877-1884 — `(conn: &mut PgConnection, board_id: Uuid) -> Result<Board, ApiError>` — A board row by id (must exist — callers hold a FK to it).
-  `board_columns` function L1889-1891 — `(conn: &mut PgConnection, board_id: Uuid) -> Result<Vec<BoardColumn>, ApiError>` — A board's LIVE columns in position order — what the board is now, so
-  `board_columns_including_removed` function L1900-1917 — `( conn: &mut PgConnection, board_id: Uuid, liveness: Liveness, ) -> Result<Vec<B...` — A board's columns in position order, removed ones included when the
-  `column_label` function L1925-1934 — `(conn: &mut PgConnection, column_id: Uuid) -> Result<String, ApiError>` — The name of ANY column, removed ones included — the audit answer, not
-  `resolve_column` function L1938-1955 — `(columns: &[BoardColumn], reference: &str) -> Result<Uuid, ApiError>` — Resolve a column reference (UUID or case-insensitive name) against a
-  `BoardItemRow` struct L1958-1970 — `{ column_id: Uuid, short_code: String, title: String, repository_id: Option<Uuid...` — One compact row of a board listing.
-  `BoardStrategySelect` type L1977 — `= (Uuid, String, String, Option<DateTime<Utc>>)` — What each family's board listing selects.
-  `BoardInitiativeSelect` type L1978 — `= (Uuid, String, String, bool, Option<DateTime<Utc>>)` — `activity_log` writes as the API path).
-  `BoardTaskSelect` type L1979-1987 — `= ( Uuid, String, String, TaskType, WorkClass, Option<Uuid>, Option<DateTime<Utc...` — `activity_log` writes as the API path).
-  `BoardAdrSelect` type L1988 — `= (Option<Uuid>, String, String, Option<DateTime<Utc>>)` — `activity_log` writes as the API path).
-  `board_item_rows` function L1999-2135 — `( conn: &mut PgConnection, board_id: Uuid, repository: Option<Uuid>, liveness: L...` — Every item placed on a board (strategies, initiatives, tasks, and
-  `column_item_counts` function L2140-2149 — `( conn: &mut PgConnection, board_id: Uuid, ) -> Result<HashMap<Uuid, i64>, ApiEr...` — Per-column LIVE item counts for one board — what `list_boards` prints
-  `repo_slug_map` function L2153-2168 — `( conn: &mut PgConnection, ids: &[Uuid], ) -> Result<BTreeMap<Uuid, String>, Api...` — Slugs for a set of repository ids, one query (KAIROS-T-0111): what the
-  `repo_label` function L2171-2188 — `(conn: &mut PgConnection, repository_id: Option<Uuid>) -> Result<String, ApiErro...` — `slug (owner team)` for one task's repository, or `(none)`.
-  `require_live_typed` function L2192-2200 — `( conn: &mut PgConnection, short_code: &str, field: &str, ) -> Result<(Uuid, Ite...` — A live item by short code WITH its type (the edge-permission check needs
-  `metadata_lines` function L2204-2217 — `(conn: &mut PgConnection, item_id: Uuid) -> Result<String, ApiError>` — An item's metadata values as compact `- slug: value` lines (ordered by
-  `ChainRow` struct L2220-2229 — `{ id: Uuid, short_code: String, title: String, deleted_at: Option<DateTime<Utc>>...` — `activity_log` writes as the API path).
-  `parent_chain` function L2242-2266 — `(conn: &mut PgConnection, item_id: Uuid) -> Result<Vec<ChainRow>, ApiError>` — The item's ancestors via incoming `parent` edges, nearest first
-  `relationship_lines` function L2277-2335 — `(conn: &mut PgConnection, item_id: Uuid) -> Result<String, ApiError>` — Agent-oriented relationship lines for `get_item`: parent chain,
-  `line` function L2285-2292 — `(neighbor: &kairos_db::graph::Neighbor) -> String` — One neighbour line, tagged when the neighbour is archived.
-  `map_update_error` function L2346-2374 — `( conn: &mut PgConnection, item: &ItemView, e: items::ItemError, ) -> Result<Api...` — Map an [`items::ItemError`] from a content update to the S-0006 tool
-  `map_link_error` function L2384-2399 — `(e: GraphError) -> ApiError` — [`GraphError`] → the same codes the REST relationship endpoints emit:
-  `level_of` function L2406-2414 — `(item_type: ItemType) -> BoardLevel` — The board level whose boards host this item type.
-  `default_board_for` function L2418-2440 — `(conn: &mut PgConnection, level: BoardLevel) -> Result<Board, ApiError>` — The tenant's single live board of `level`, or a 422 asking the agent to
-  `resolve_template` function L2443-2474 — `(conn: &mut PgConnection, reference: &str) -> Result<Uuid, ApiError>` — Resolve a template reference (UUID, slug, or name) to its id.
-  `reject_field` function L2478-2490 — `( field: &str, value: Option<&String>, item_type: ItemType, applies_to: &str, ) ...` — Reject a type-specific field supplied for the wrong item type (agents
-  `create_item_impl` function L2495-2811 — `( conn: &mut PgConnection, tenant: &TenantContext, user: Uuid, params: &CreateIt...` — The create_item body: resolve the target board (or parent, for
-  `field_invalid` function L2819-2821 — `(field: &str, message: impl Into<String>) -> ApiError` — A field-level 422 `VALIDATION` for the search input (the tool-error
-  `uuid_field` function L2823-2826 — `(value: &str, field: &str) -> Result<Uuid, ApiError>` — `activity_log` writes as the API path).
-  `timestamp_field` function L2828-2837 — `(value: &str, field: &str) -> Result<DateTime<Utc>, ApiError>` — `activity_log` writes as the API path).
-  `enum_field` function L2841-2852 — `( value: &str, field: &str, allowed: &str, ) -> Result<T, ApiError>` — Parse a closed-vocabulary value through the core model's serde
-  `render_related_work` function L2863-2884 — `(found: &crate::embedding::RelatedWork) -> String` — Convert the tool input into the typed `kairos_core::search` request and
-  `search_to_core` function L2886-3020 — `(params: &SearchParams) -> Result<core_search::SearchRequest, ApiError>` — `activity_log` writes as the API path).
-  `map_search_error` function L3024-3032 — `(e: SearchError) -> ApiError` — [`SearchError`] → tool error (validation was pre-checked, so this is
-  `archived_marker` function L3038-3044 — `(deleted_at: Option<DateTime<Utc>>) -> &'static str` — `" [archived]"` for a row that has been put away, empty otherwise
-  `render_search_results` function L3048-3120 — `(results: &SearchResults, repo_slugs: &BTreeMap<Uuid, String>) -> String` — Compact REQ-1.6 rendering: results grouped by type, one line per item
-  `create_item_parity_tests` module L3123-3263 — `-` — `activity_log` writes as the API path).
-  `struct_fields` function L3142-3161 — `(source: &str, name: &str) -> BTreeSet<String>` — Field names declared by a `pub struct X { ..
-  `read` function L3163-3166 — `(rel: &str) -> String` — `activity_log` writes as the API path).
-  `DELIBERATELY_ABSENT` variable L3170-3199 — `: &[(&str, &str)]` — REST fields `create_item` deliberately does not take, each with the reason
-  `create_item_covers_every_rest_create_field` function L3202-3232 — `()` — `activity_log` writes as the API path).
-  `the_two_fields_an_agent_could_not_reach_are_reachable` function L3235-3242 — `()` — `activity_log` writes as the API path).
-  `the_absence_of_column_is_documented_not_merely_true` function L3245-3262 — `()` — `activity_log` writes as the API path).
...
- pub `TokenClaims` struct L67-87 — `{ sub: String, email: Option<String>, name: Option<String>, email_verified: Opti...` — The token claims this crate consumes.
- pub `DiscoveryError` enum L109-118 — `Discovery` — Why building an [`Authenticator`] failed (startup-time, fail-fast).
- pub `VerifyError` enum L122-158 — `MissingToken | UnknownKey | Invalid | MissingEmail | KeyFetch | NoIssuer` — Why a token was rejected (or could not be checked).
- pub `Authenticator` struct L189-207 — `{ issuer: String, audiences: Vec<String>, jwks_uri: Option<String>, http: reqwes...` — Validates bearer tokens against one OIDC issuer: JWKS cache keyed by
- pub `discover` function L234-279 — `(issuer: &str, audience: &str) -> Result<Self, DiscoveryError>` — Resolve the issuer's discovery document, prime the JWKS cache, and
- pub `disabled` function L292-302 — `() -> Self` — Test constructor: fixed keys, no JWKS endpoint (refresh-on-unknown-
- pub `is_enabled` function L305-307 — `(&self) -> bool` — Whether this deployment has an issuer at all.
- pub `with_static_keys` function L309-323 — `( issuer: &str, audience: &str, keys: impl IntoIterator<Item = (String, Decoding...` — mode, and an effectively-empty value is a startup error.
- pub `verify` function L388-406 — `(&self, token: &str) -> Result<TokenClaims, VerifyError>` — Validate `token` (RS256 signature, `iss`, `aud`, `exp`) and return
- pub `require_auth` function L561-621 — `( State(state): State<AppState>, mut req: Request, next: Next, ) -> Result<Respo...` — The auth layer: validate the bearer token, JIT-upsert the user, insert
-  `lenient_bool` function L91-105 — `(deserializer: D) -> Result<Option<bool>, D::Error>` — `true`/`false`, or the strings `"true"`/`"false"` (case-insensitively).
-  `ApiError` type L160-167 — `= ApiError` — mode, and an effectively-empty value is a startup error.
-  `from` function L161-166 — `(err: VerifyError) -> Self` — mode, and an effectively-empty value is a startup error.
-  `DiscoveryDoc` struct L170-172 — `{ jwks_uri: String }` — mode, and an effectively-empty value is a startup error.
-  `JwksDoc` struct L175-177 — `{ keys: Vec<Jwk> }` — mode, and an effectively-empty value is a startup error.
-  `Jwk` struct L180-185 — `{ kty: String, kid: Option<String>, n: Option<String>, e: Option<String> }` — mode, and an effectively-empty value is a startup error.
-  `Authenticator` type L209-217 — `= Authenticator` — mode, and an effectively-empty value is a startup error.
-  `fmt` function L210-216 — `(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result` — mode, and an effectively-empty value is a startup error.
-  `parse_audiences` function L222-228 — `(raw: &str) -> Vec<String>` — Parse `OIDC_AUDIENCE`: a comma-separated allow-list (KAIROS-T-0055).
-  `Authenticator` type L230-407 — `= Authenticator` — mode, and an effectively-empty value is a startup error.
-  `refresh_keys` function L326-365 — `(&self) -> Result<(), VerifyError>` — Fetch the JWKS and replace the cache with its RSA keys.
-  `key_for` function L369-384 — `(&self, kid: &str) -> Result<Option<DecodingKey>, VerifyError>` — The decoding key for `kid`, refreshing the JWKS once (behind the
-  `bearer_token` function L410-418 — `(req: &Request) -> Result<&str, VerifyError>` — `Authorization: Bearer <token>` or [`VerifyError::MissingToken`].
-  `jit_upsert_user` function L435-549 — `(pool: &TenantPool, claims: &TokenClaims) -> Result<User, ApiError>` — mode, and an effectively-empty value is a startup error.
-  `tests` module L624-952 — `-` — mode, and an effectively-empty value is a startup error.
-  `TEST_RSA_PRIVATE_PEM` variable L638-665 — `: &str` — Throwaway RSA keypair for minting test tokens.
-  `TEST_RSA_PUBLIC_PEM` variable L667-675 — `: &str` — mode, and an effectively-empty value is a startup error.
-  `ISSUER` variable L677 — `: &str` — mode, and an effectively-empty value is a startup error.
-  `AUDIENCE` variable L678 — `: &str` — mode, and an effectively-empty value is a startup error.
-  `KID` variable L679 — `: &str` — mode, and an effectively-empty value is a startup error.
-  `authenticator` function L681-684 — `() -> Authenticator` — mode, and an effectively-empty value is a startup error.
-  `MintClaims` struct L687-694 — `{ iss: &'a str, aud: &'a str, sub: &'a str, exp: i64, email: &'a str, name: &'a ...` — mode, and an effectively-empty value is a startup error.
-  `mint` function L696-709 — `(iss: &str, aud: &str, exp: i64, kid: Option<&str>) -> String` — mode, and an effectively-empty value is a startup error.
-  `future_exp` function L711-713 — `() -> i64` — mode, and an effectively-empty value is a startup error.
-  `chrono_now` function L715-720 — `() -> u64` — mode, and an effectively-empty value is a startup error.
-  `valid_token_yields_claims` function L723-731 — `()` — mode, and an effectively-empty value is a startup error.
-  `expired_token_is_rejected` function L734-745 — `()` — mode, and an effectively-empty value is a startup error.
-  `wrong_audience_is_rejected` function L748-758 — `()` — mode, and an effectively-empty value is a startup error.
-  `any_listed_audience_validates_and_unlisted_is_rejected` function L765-788 — `()` — KAIROS-T-0055: `OIDC_AUDIENCE` as a comma-separated list — a token
-  `audience_parsing_handles_lists_and_blanks` function L795-803 — `()` — KAIROS-T-0055: single-value backward compat is the default test
-  `wrong_issuer_is_rejected` function L806-816 — `()` — mode, and an effectively-empty value is a startup error.
-  `unknown_kid_is_rejected` function L819-825 — `()` — mode, and an effectively-empty value is a startup error.
-  `garbage_token_is_rejected` function L828-834 — `()` — mode, and an effectively-empty value is a startup error.
-  `google_shaped_id_token_validates_and_opaque_access_token_is_rejected` function L844-866 — `()` — KAIROS-T-0054: the middleware is issuer/token-kind agnostic — it
-  `GOOGLE_ISSUER` variable L845 — `: &str` — mode, and an effectively-empty value is a startup error.
-  `GOOGLE_CLIENT_ID` variable L846 — `: &str` — mode, and an effectively-empty value is a startup error.
-  `verify_errors_map_to_401_envelope` function L869-873 — `()` — mode, and an effectively-empty value is a startup error.
-  `claims_from` function L887-889 — `(json: serde_json::Value) -> Result<TokenClaims, serde_json::Error>` — mode, and an effectively-empty value is a startup error.
-  `a_literal_true_is_the_only_thing_that_verifies_an_email` function L892-927 — `()` — mode, and an effectively-empty value is a startup error.
-  `an_absent_email_verified_is_not_a_verified_email` function L930-937 — `()` — mode, and an effectively-empty value is a startup error.
-  `an_unparseable_email_verified_does_not_reject_the_token` function L940-951 — `()` — mode, and an effectively-empty value is a startup error.
...
- pub `TenantContext` struct L40-47 — `{ org_id: Uuid, slug: String, role: OrgRole }` — The resolved tenant, inserted as a request extension for every request
- pub `TenantDb` struct L52-55 — `{ pool: TenantPool, slug: String }` — A tenant-pinned pool handle: connections checked out through it have
- pub `conn` function L59-64 — `(&self) -> Result<TenantConnection, ApiError>` — Check out a connection pinned to this tenant's schema.
- pub `slug` function L67-69 — `(&self) -> &str` — The tenant slug this handle is pinned to.
- pub `resolve_slug` function L83-111 — `(config: &AppConfig, headers: &HeaderMap) -> Result<String, ApiError>` — Resolve the tenant slug for a request per the module-level order.
- pub `require_tenant` function L115-197 — `( State(state): State<AppState>, mut req: Request, next: Next, ) -> Result<Respo...` — The tenant layer: resolve the slug, load the organization, require
-  `TenantDb` type L57-70 — `= TenantDb` — [`TenantDb`] handle whose connections are pinned to the tenant schema.
-  `host_without_port` function L74-79 — `(host: &str) -> &str` — The host part of a `Host` header value (port stripped; IPv6 literals
-  `tests` module L200-294 — `-` — [`TenantDb`] handle whose connections are pinned to the tenant schema.
-  `config` function L204-235 — `(single_tenant: Option<&str>, base_domain: Option<&str>) -> AppConfig` — [`TenantDb`] handle whose connections are pinned to the tenant schema.
-  `headers` function L237-246 — `(pairs: &[(&str, &str)]) -> HeaderMap` — [`TenantDb`] handle whose connections are pinned to the tenant schema.
-  `single_tenant_mode_wins_over_everything` function L249-253 — `()` — [`TenantDb`] handle whose connections are pinned to the tenant schema.
-  `host_subdomain_resolves_against_base_domain` function L256-262 — `()` — [`TenantDb`] handle whose connections are pinned to the tenant schema.
-  `non_matching_hosts_fall_through_to_x_tenant` function L265-278 — `()` — [`TenantDb`] handle whose connections are pinned to the tenant schema.
-  `x_tenant_is_the_fallback_without_base_domain` function L281-285 — `()` — [`TenantDb`] handle whose connections are pinned to the tenant schema.
-  `unresolvable_requests_are_tenant_not_found` function L288-293 — `()` — [`TenantDb`] handle whose connections are pinned to the tenant schema.
...
-  `user_schema` function L43-85 — `() -> Value` — The User schema resource: the attribute subset this server round-trips.
-  `group_schema` function L88-114 — `() -> Value` — The Group schema resource.
-  `schemas` function L117-122 — `() -> Response` — `GET /scim/v2/Schemas` (RFC 7643 §7): ListResponse of the two schemas.
-  `resource_types` function L125-152 — `() -> Response` — `GET /scim/v2/ResourceTypes` (RFC 7643 §6).
...
-  `user_resource` function L37-53 — `(user: &User, active: bool) -> Value` — Render one `public.users` row as a SCIM User resource.
-  `UserPayload` struct L60-69 — `{ user_name: String, external_id: Option<String>, display_name: Option<String>, ...` — The inbound attribute subset of a POST/PUT User payload.
-  `parse_scim_bool` function L72-79 — `(value: &Value) -> Option<bool>` — A SCIM boolean: JSON bool, or Entra's `"True"`/`"False"` strings.
-  `first_email` function L83-96 — `(body: &Value) -> Option<String>` — The first usable email value: primary first, then first entry; entries
-  `parse_user_payload` function L98-133 — `(body: &Value) -> Result<UserPayload, ScimError>` — the identity-join and deprovision contracts).
-  `resolve_or_create_user` function L146-225 — `( conn: &mut PgConnection, payload: &UserPayload, ) -> Result<User, ScimError>` — The identity-join contract (module docs): externalId → external_id, then
-  `revoke_membership` function L230-256 — `( conn: &mut PgConnection, ctx: &ScimContext, membership: &OrganizationMember, u...` — Revoke the org membership (deprovision): LAST_ADMIN-guarded, activity-
-  `load_member` function L260-273 — `( conn: &mut PgConnection, org_id: Uuid, user_id: Uuid, ) -> Result<(Organizatio...` — Load the member (membership + user) or 404 — the resource set is the
-  `create_user` function L281-319 — `( State(state): State<AppState>, Extension(ctx): Extension<ScimContext>, body: B...` — `POST /scim/v2/Users` — provision: link-or-create the `public.users`
-  `list_users` function L323-387 — `( State(state): State<AppState>, Extension(ctx): Extension<ScimContext>, Query(p...` — `GET /scim/v2/Users` — list the org's members; supports
-  `UserFilter` enum L332-335 — `UserName | ExternalId` — the identity-join and deprovision contracts).
-  `get_user` function L390-402 — `( State(state): State<AppState>, Extension(ctx): Extension<ScimContext>, Path(id...` — `GET /scim/v2/Users/{id}`.
-  `UserChange` enum L405-408 — `Active | DisplayName` — One parsed User PATCH change.
-  `parse_user_patch` function L411-495 — `(body: &Value) -> Result<Vec<UserChange>, ScimError>` — Parse the RFC 7644 §3.5.2 PatchOp subset for Users (module docs).
-  `patch_user` function L499-548 — `( State(state): State<AppState>, Extension(ctx): Extension<ScimContext>, Path(id...` — `PATCH /scim/v2/Users/{id}` — `active: false` deprovisions immediately
-  `replace_user` function L560-634 — `( State(state): State<AppState>, Extension(ctx): Extension<ScimContext>, Path(id...` — `PUT /scim/v2/Users/{id}` — replace the writable profile subset.
-  `delete_user` function L638-652 — `( State(state): State<AppState>, Extension(ctx): Extension<ScimContext>, Path(id...` — `DELETE /scim/v2/Users/{id}` — revoke the membership (LAST_ADMIN-
...
-  `typed_error_mapping_roundtrip` function L93-421 — `()` — discipline).

#### crates/kairos-server/tests/embedding_service.rs

-  `DEFAULT_DATABASE_URL` variable L30 — `: &str` — right thing — that is KAIROS-T-0189's measurements, against a real model.
-  `SCRATCH_DB` variable L31 — `: &str` — right thing — that is KAIROS-T-0189's measurements, against a real model.
-  `document` function L34-40 — `(status: &str) -> String` — A document with a heading per section, so chunking produces a known count.
-  `admin_database_url` function L42-44 — `() -> String` — right thing — that is KAIROS-T-0189's measurements, against a real model.
-  `with_database` function L46-49 — `(url: &str, db_name: &str) -> String` — right thing — that is KAIROS-T-0189's measurements, against a real model.
-  `refreshing_embeds_only_what_changed` function L52-225 — `()` — right thing — that is KAIROS-T-0189's measurements, against a real model.
-  `VersionRow` struct L228-231 — `{ version: i32 }` — right thing — that is KAIROS-T-0189's measurements, against a real model.
-  `a_sweep_reaches_items_beyond_the_first_page` function L241-335 — `()` — A sweep must reach past its first page (KAIROS-T-0193).
-  `SCRATCH` variable L242 — `: &str` — right thing — that is KAIROS-T-0189's measurements, against a real model.
-  `PAGE` variable L282 — `: i64` — right thing — that is KAIROS-T-0189's measurements, against a real model.
-  `the_refresher_picks_up_work_created_after_it_started` function L346-432 — `()` — The background refresher embeds work that arrives after it started
-  `SCRATCH` variable L347 — `: &str` — right thing — that is KAIROS-T-0189's measurements, against a real model.
...
-  `entity_endpoints_against_live_stack` function L183-1378 — `()` — admin API for grants is KAIROS-T-0019.
...
#### crates/kairos-server/tests/local_accounts.rs

-  `common` module L8 — `-` — creating an account, resetting a password, and inspecting and ending sessions —
-  `SCRATCH_DB` variable L34 — `: &str` — Runs against the LIVE compose Postgres (`angreal services up`).
-  `ADMIN_PASSWORD` variable L35 — `: &str` — Runs against the LIVE compose Postgres (`angreal services up`).
-  `send` function L37-52 — `(router: &Router, request: Request<Body>) -> (StatusCode, Value)` — Runs against the LIVE compose Postgres (`angreal services up`).
-  `as_user` function L54-77 — `( router: &Router, method: Method, uri: &str, token: &str, body: Option<Value>, ...` — Runs against the LIVE compose Postgres (`angreal services up`).
-  `login` function L80-92 — `(router: &Router, email: &str, password: &str) -> String` — Log in and return the session bearer.
-  `member` function L94-116 — `(conn: &mut PgConnection, email: &str, org_id: Uuid, role: OrgRole) -> User` — Runs against the LIVE compose Postgres (`angreal services up`).
-  `an_org_admin_manages_local_accounts` function L119-392 — `()` — Runs against the LIVE compose Postgres (`angreal services up`).
-  `the_break_glass_subcommand_needs_no_login` function L395-511 — `()` — Runs against the LIVE compose Postgres (`angreal services up`).

#### crates/kairos-server/tests/local_login.rs

-  `common` module L10 — `-` — `POST /api/login` mints a session bearer, `require_auth` accepts it as its
-  `PASSWORD` variable L37 — `: &str` — deployment with local auth OFF is unchanged.
-  `send` function L39-54 — `(router: &Router, request: Request<Body>) -> (StatusCode, Value)` — deployment with local auth OFF is unchanged.
-  `login` function L60-72 — `(router: &Router, email: &str, password: &str) -> (StatusCode, Value, StdDuratio...` — `POST /api/login`, with how long it took.
-  `whoami` function L74-83 — `(router: &Router, token: &str) -> (StatusCode, Value)` — deployment with local auth OFF is unchanged.
-  `logout` function L85-93 — `(router: &Router, token: &str) -> StatusCode` — deployment with local auth OFF is unchanged.
-  `make_user` function L95-119 — `(conn: &mut PgConnection, email: &str, org_id: Uuid, with_password: bool) -> Use...` — deployment with local auth OFF is unchanged.
-  `fixture` function L122-134 — `(scratch_db: &str) -> (PgConnection, String, Uuid)` — Scratch DB + provisioned `acme`, and the org id to hang members off.
-  `teardown` function L138-142 — `(conn: PgConnection, scratch_db: &str)` — Drop the scratch database.
-  `logging_in_with_a_password_works_and_every_failure_looks_the_same` function L145-274 — `()` — deployment with local auth OFF is unchanged.
-  `a_burst_of_password_guesses_is_refused` function L277-312 — `()` — deployment with local auth OFF is unchanged.
-  `with_local_auth_off_there_is_no_login_endpoint` function L315-355 — `()` — deployment with local auth OFF is unchanged.
-  `a_service_account_cannot_log_in_with_a_password` function L358-396 — `()` — deployment with local auth OFF is unchanged.
-  `a_deployment_with_no_issuer_still_lets_people_in` function L399-473 — `()` — deployment with local auth OFF is unchanged.
...
-  `mcp_endpoint_against_live_stack` function L343-1278 — `()` — calls (asserted straight from the scratch database).
...
-  `openapi_endpoint_against_live_stack` function L366-504 — `()` — services), uploaded by the workflow's "OpenAPI artifact" step.
...
-  `org_and_admin_endpoints_against_live_stack` function L75-1078 — `()` — - `bob`   — org member used for the capability grant/revoke lifecycle.

#### crates/kairos-server/tests/rate_limit.rs

-  `common` module L16 — `-` — a burst of bad credentials from one source is actually refused, the refusal
-  `SCRATCH_DB` variable L41 — `: &str` — needed: the API-key path never validates a JWT.
-  `MAX_FAILURES` variable L46 — `: u32` — After this many failures the throttle locks the source out.
-  `whoami_from` function L54-92 — `( router: &Router, key: &str, peer: &str, extra: &[(&str, &str)], ) -> (StatusCo...` — A `/api/whoami` request from a specific socket peer.
-  `metrics_text` function L94-109 — `(router: &Router) -> String` — needed: the API-key path never validates a JWT.
-  `a_burst_of_failed_authentications_is_refused` function L112-258 — `()` — needed: the API-key path never validates a JWT.
-  `throttling_off_lets_a_burst_through` function L261-289 — `()` — needed: the API-key path never validates a JWT.

#### crates/kairos-server/tests/related_work.rs

-  `DEFAULT_DATABASE_URL` variable L29 — `: &str` — distribution in KAIROS-T-0190.
-  `SCRATCH_DB` variable L30 — `: &str` — distribution in KAIROS-T-0190.
-  `admin_database_url` function L32-34 — `() -> String` — distribution in KAIROS-T-0190.
-  `with_database` function L36-39 — `(url: &str, db: &str) -> String` — distribution in KAIROS-T-0190.
-  `doc` function L48-54 — `(unique: &str, shared: &str) -> String` — A document whose MIDDLE section is the part shared between fixtures.
-  `related_work_proposes_and_degrades` function L57-320 — `()` — distribution in KAIROS-T-0190.
-  `show_real_proposals_for_the_demo_tenant` function L337-363 — `()` — Eyeball real retrieval against the seeded `demo` tenant, with the **real**
...
-  `count_users_with_email` function L125-131 — `(conn: &mut PgConnection, email: &str) -> i64` — How many `public.users` rows carry this email (KAIROS-T-0197: the bug was a
-  `acme_role` function L134-143 — `(conn: &mut PgConnection, org_id: Uuid, user_id: Uuid) -> Option<String>` — The member's role in acme, if any.
-  `CountRow` struct L146-149 — `{ count: i64 }` — - `dave`  — never logs in: exists only through SCIM (created user row).
-  `activity_count` function L152-162 — `(conn: &mut PgConnection, entity_type: &str, details_like: &str) -> i64` — Count acme activity rows matching an entity type and details pattern.
-  `scim_provisioning_against_live_stack` function L165-1200 — `()` — - `dave`  — never logs in: exists only through SCIM (created user row).
...
-  `search_endpoint_against_live_stack` function L199-924 — `()` — and every archived hit comes back marked with `archived_at`
...
-  `web_surfaces_against_live_stack` function L68-244 — `()` — integration suite here.
...
-  `ws_events_against_live_stack` function L203-689 — `()` — disconnect/reconnect — every await timeout-bounded.
...
- pub `base_config` function L184-227 — `(scratch_url: &str) -> AppConfig` — The standard test config: multi-tenant with `kairos.test` as the base
- pub `request` function L232-268 — `( router: &Router, method: Method, uri: &str, token: Option<&str>, headers: &[(&...` — One in-process request against the production router: any method/URI,
- pub `error_code` function L272-276 — `(body: &Value) -> &str` — The `error.code` of an S-0005 error envelope (panics on any other
- pub `TestServer` struct L282-287 — `{ addr: std::net::SocketAddr, base_url: String }` — A live instance of the production router on an ephemeral local port —
- pub `spawn_server` function L290-302 — `(router: Router) -> TestServer` — Serve `router` on an ephemeral 127.0.0.1 port.
- pub `client` function L307-309 — `(&self, token: &str, tenant: &str) -> KairosClient` — A typed client for this server: fixed bearer token, tenant via the
- pub `client_untenanted` function L313-315 — `(&self, token: &str) -> KairosClient` — A typed client WITHOUT tenant resolution (cross-tenant
...
-  `TestServer` type L304-316 — `= TestServer` — different subset, so unused-item lints are expected noise here.
...
- pub `BrandMark` function L312-320 — `() -> impl IntoView` — The Kairos brand mark — app-supplied (aurora ships no branding), drawn
...
-  `RedirectToIssuer` function L199-253 — `() -> impl IntoView` — The unauthenticated fallback: kick off the PKCE redirect (remembering where the
-  `NavLink` function L259-269 — `(#[prop(into)] href: String, #[prop(into)] label: String) -> impl IntoView` — One left-nav entry.
-  `WhoamiBadge` function L275-291 — `(whoami: LocalResource<Result<api::Whoami, ApiError>>) -> impl IntoView` — Who am I, which org, which role — the A-0015 whoami display.
-  `LogoutButton` function L296-307 — `() -> impl IntoView` — Drop the in-memory session; the shell guard (which sees the explicit
...
- pub `LOGIN_PATH` variable L53 — `: &str` — The local password-login endpoint (KAIROS-T-0203).
- pub `ApiBearer` enum L76-82 — `AccessToken | IdToken` — Which token the SPA presents as the `/api` bearer (`api_bearer` in
- pub `AuthConfig` struct L86-112 — `{ issuer: Option<String>, client_id: String, authorization_endpoint: Option<Stri...` — What `GET /api/config` returns (mirror of `kairos-server::web`).
- pub `can_sso` function L121-123 — `(&self) -> bool` — Whether an SSO redirect can actually be performed.
- pub `TokenResponse` struct L128-139 — `{ access_token: String, id_token: Option<String>, refresh_token: Option<String>,...` — A successful token response (authorization_code or refresh_token grant).
- pub `Session` struct L159-164 — `{ access_token: String, refresh_token: Option<String> }` — The in-memory session (A-0015: never persisted).
- pub `Auth` struct L169-182 — `{ session: RwSignal<Option<Session>>, config: RwSignal<Option<AuthConfig>>, sign...` — Reactive auth state, provided at the app root ([`provide_auth`]) and
- pub `provide_auth` function L189-206 — `() -> Auth` — Create the auth state and put it into context.
- pub `use_auth` function L225-227 — `() -> Auth` — The app-root [`Auth`] (panics outside the app tree — a bug by
- pub `is_authenticated` function L231-233 — `(&self) -> bool` — Reactive: is there a live session?
- pub `token` function L236-239 — `(&self) -> Option<String>` — Current bearer token (reactive).
- pub `signed_out` function L244-246 — `(&self) -> bool` — Reactive: did the user explicitly log out (vs.
- pub `restoring` function L250-252 — `(&self) -> bool` — Reactive: is the boot-time session restore (KAIROS-T-0071) still in
- pub `logout` function L258-263 — `(&self)` — Drop the in-memory session AND the stored refresh token (a logout
- pub `expire` function L270-274 — `(&self)` — Drop the session WITHOUT marking an explicit sign-out (refresh
- pub `config_cached` function L360-367 — `(&self) -> Result<AuthConfig, String>` — `/api/config`, fetched once and cached in the signal.
- pub `fetch_auth_config` function L372-384 — `() -> Result<AuthConfig, String>` — `GET /api/config` — the SPA's discovery endpoint (decision documented
- pub `begin_login` function L389-430 — `(auth: Auth, return_to: &str) -> Result<(), String>` — Start the PKCE flow: stash verifier/state/return-path in
- pub `complete_login` function L434-479 — `(auth: Auth) -> Result<String, String>` — Handle `/callback`: verify `state`, exchange the code through the
- pub `take_return_to` function L486-496 — `() -> String` — Where to go after a successful sign-in, consuming the stashed value.
- pub `stash_return_to` function L504-511 — `(path: &str)` — Remember where an unauthenticated visitor was headed before sending them to the
- pub `password_login` function L529-568 — `(auth: Auth, email: &str, password: &str) -> Result<(), String>` — `POST /api/login` — a local password login (KAIROS-T-0205, KAIROS-T-0203).
-  `SCOPES` variable L57 — `: &str` — Scopes requested at login.
-  `REFRESH_MARGIN_SECS` variable L60 — `: f64` — Refresh the session this many seconds before the access token expires.
-  `KEY_VERIFIER` variable L64 — `: &str` — Workspace (KAIROS-T-0054).
-  `KEY_STATE` variable L65 — `: &str` — Workspace (KAIROS-T-0054).
-  `KEY_RETURN_TO` variable L66 — `: &str` — Workspace (KAIROS-T-0054).
-  `KEY_REFRESH` variable L69 — `: &str` — The refresh token (KAIROS-T-0071): per-tab reload survival.
-  `AuthConfig` type L114-124 — `= AuthConfig` — Workspace (KAIROS-T-0054).
-  `TokenResponse` type L141-155 — `= TokenResponse` — Workspace (KAIROS-T-0054).
-  `bearer_for` function L146-154 — `(&self, kind: ApiBearer) -> String` — The token to use as the `/api` bearer for the given selection.
-  `restore_session` function L213-221 — `(auth: Auth)` — Boot-time session restore (KAIROS-T-0071): run the refresh grant with
-  `Auth` type L229-368 — `= Auth` — Workspace (KAIROS-T-0054).
-  `install` function L282-318 — `(&self, tokens: TokenResponse)` — Install a token response and schedule the silent refresh.
-  `refresh` function L323-331 — `(self)` — The silent-refresh grant through the relay, from the live session's
-  `refresh_with` function L337-354 — `(self, refresh_token: &str)` — One refresh grant with an explicit token — shared by the in-session
-  `LoginRequest` struct L531-534 — `{ email: &'a str, password: &'a str }` — Workspace (KAIROS-T-0054).
-  `LoginResponse` struct L536-538 — `{ token: String }` — Workspace (KAIROS-T-0054).
-  `login_error_message` function L575-586 — `(status: u16, body: &str) -> String` — The message to show for a failed login.
-  `post_token` function L589-608 — `(body: &str) -> Result<TokenResponse, String>` — POST a form body to the token relay and parse the token response.
-  `window` function L612-614 — `() -> web_sys::Window` — Workspace (KAIROS-T-0054).
-  `origin` function L616-621 — `() -> Result<String, String>` — Workspace (KAIROS-T-0054).
-  `session_storage` function L623-629 — `() -> Result<web_sys::Storage, String>` — Workspace (KAIROS-T-0054).
-  `clear_stored_refresh` function L632-636 — `()` — Remove the stored refresh token (logout / dead token).
-  `url_encode` function L638-640 — `(value: &str) -> String` — Workspace (KAIROS-T-0054).
-  `random_urlsafe` function L644-652 — `(len: usize) -> Result<String, String>` — `len` random bytes from WebCrypto, base64url-encoded (RFC 7636 §4.1
-  `s256_base64url` function L655-668 — `(verifier: &str) -> Result<String, String>` — The S256 code challenge: `BASE64URL(SHA256(verifier))` (RFC 7636 §4.2).
-  `base64url` function L672-692 — `(bytes: &[u8]) -> String` — Base64url without padding (RFC 4648 §5).
-  `ALPHABET` variable L673 — `: &[u8; 64]` — Workspace (KAIROS-T-0054).
-  `form_encode` function L695-701 — `(pairs: &[(&str, &str)]) -> String` — `application/x-www-form-urlencoded` body from pairs.
-  `form_component` function L704-715 — `(value: &str) -> String` — Percent-encode one form value (conservative: everything but unreserved).
-  `tests` module L718-832 — `-` — Workspace (KAIROS-T-0054).
-  `tokens` function L721-728 — `(access: &str, id: Option<&str>) -> TokenResponse` — Workspace (KAIROS-T-0054).
-  `bearer_selection_follows_api_bearer` function L731-737 — `()` — Workspace (KAIROS-T-0054).
-  `id_token_mode_falls_back_when_absent` function L740-745 — `()` — Workspace (KAIROS-T-0054).
-  `api_bearer_defaults_to_access_token_when_absent` function L748-763 — `()` — Workspace (KAIROS-T-0054).
-  `a_deployment_with_no_issuer_parses_and_offers_no_sso` function L766-777 — `()` — Workspace (KAIROS-T-0054).
-  `both_paths_at_once_is_representable` function L780-789 — `()` — Workspace (KAIROS-T-0054).
-  `a_pre_t0208_server_payload_still_parses` function L792-801 — `()` — Workspace (KAIROS-T-0054).
-  `base64url_matches_rfc4648_vectors` function L805-813 — `()` — RFC 4648 §10 test vectors, translated to base64url-no-padding.
-  `base64url_uses_urlsafe_alphabet` function L816-821 — `()` — Workspace (KAIROS-T-0054).
-  `form_encoding_escapes_reserved_characters` function L824-831 — `()` — Workspace (KAIROS-T-0054).
...
- pub `LoginPage` function L40-83 — `() -> impl IntoView` — Explicit sign-in page: logout lands here, and so does any unauthenticated
- pub `CallbackPage` function L239-257 — `() -> impl IntoView` — PKCE redirect target: exchanges the code (via the server relay), then
- pub `admin` module L298 — `-` — later tasks should keep.
- pub `NotFoundPage` function L309-319 — `() -> impl IntoView` — Router fallback.
-  `ProviderButton` function L87-120 — `() -> impl IntoView` — The "sign in with your identity provider" button: starts PKCE.
-  `PasswordForm` function L137-234 — `() -> impl IntoView` — The email/password form, for a deployment with local accounts
-  `copy_link` module L263 — `-` — later tasks should keep.
-  `boards` module L267 — `-` — later tasks should keep.
-  `item` module L272 — `-` — later tasks should keep.
-  `search` module L277 — `-` — later tasks should keep.
-  `editor` module L284 — `-` — later tasks should keep.
-  `teams` module L286 — `-` — later tasks should keep.
-  `repositories` module L293 — `-` — later tasks should keep.
-  `activity` module L304 — `-` — later tasks should keep.
...
-  `ItemLoaded` function L178-331 — `( item: ItemDetail, family: Family, on_saved: Callback<i32>, on_moved: Callback<...` — The loaded page: header + actions, the editor column, and the facts /
-  `manage_capability` function L338-346 — `(family: Family) -> &'static str` — The capability that putting an item BACK asks for: the same
-  `restore_power` function L354-375 — `( family: Family, board: LocalResource<Result<Option<api::BoardInfo>, ApiError>>...` — May the signed-in user restore THIS item? The shared whoami mirror
-  `put_away_when` function L384-392 — `(rfc3339: &str) -> String` — `2026-09-23T11:30:07.479107Z` → `2026-09-23 11:30 UTC` (display only;
-  `ArchivedBanner` function L407-460 — `( family: Family, #[prop(into)] code: String, /// The entity UUID — the activi...` — The unmistakable marker on an archived item (KAIROS-T-0164, ADR-20).
-  `RestoreControl` function L467-559 — `( family: Family, #[prop(into)] code: String, can_restore: Memo<bool>, on_restor...` — The Restore action (KAIROS-T-0160's endpoint): visible only to someone
-  `lifecycle_color` function L564-570 — `(state: &str) -> &'static str` — The badge color of a document lifecycle state (KAIROS-T-0078):
-  `LifecyclePanel` function L577-638 — `( #[prop(into)] code: String, current: String, /// The document is archived in t...` — The lifecycle control (KAIROS-T-0078, documents only): a
-  `ChildrenProgressBar` function L652-691 — `(family: Family, #[prop(into)] code: String) -> impl IntoView` — The children rollup under the header (KAIROS-T-0080): a segmented bar
-  `TypeFacts` function L695-734 — `(item: ItemDetail) -> impl IntoView` — The type-specific facts as pills (each family's extra columns).
-  `BoardPanel` function L742-845 — `( family: Family, #[prop(into)] code: String, /// The page's shared board read (...` — Board/column display: documents never sit on boards; ADRs may not; the
-  `board_power` function L852-873 — `( board_slug: String, team_id: Option<String>, kind: Option<boards::data::Entity...` — One board power as a memo over the shell's shared whoami identity
-  `RepositoryControl` function L881-1022 — `( code: String, /// The board's slug — for the client-side capability mirror. ...` — The task's repository binding (KAIROS-T-0109, A-0019): pick one of the
-  `THIS_BOARD` variable L1027 — `: &str` — The board picker's "stay put" option — the default, so a board move is
-  `MoveBoardControl` function L1041-1144 — `( code: String, /// The board the task sits on now — the source half of the ru...` — Move a task to another DELIVERY board (KAIROS-I-0012 D2): a team
-  `MoveControl` function L1153-1301 — `( family: Family, code: String, board: api::BoardInfo, column_id: Option<String>...` — The keyboard-accessible transition path (KAIROS-T-0075) plus the lane
-  `EdgeProposalsPanel` function L1313-1413 — `(#[prop(into)] code: String) -> impl IntoView` — Edge proposals waiting on this item (KAIROS-A-0021 rule 6, KAIROS-T-0192).
-  `RelatedWorkPanel` function L1437-1537 — `(#[prop(into)] code: String) -> impl IntoView` — "Possibly related" — the human side of retrieval (KAIROS-T-0195).
-  `RelationshipsPanel` function L1546-1605 — `(family: Family, #[prop(into)] code: String) -> impl IntoView` — Relationships summary: both directions, grouped, every neighbor linked
-  `link_state_color` function L1610-1617 — `(state: &str) -> &'static str` — The accent for a forge link's state (KAIROS-T-0100).
-  `DevelopmentPanel` function L1623-1694 — `(family: Family, #[prop(into)] code: String) -> impl IntoView` — Branches and pull/merge requests for this item (KAIROS-T-0100).
-  `relationship_label` function L1701-1720 — `(relationship: &str, outgoing: bool) -> String` — The human name of one relationship group, read from THIS item's side —
-  `RelationshipGroupView` function L1724-1764 — `( group: RelationshipGroup, #[prop(into)] direction: String, ) -> impl IntoView` — One direction of one relationship type, neighbors linked.
-  `tests` module L1767-1820 — `-` — warning ([`delete`], A-0001).
-  `tab_hrefs_never_emit_an_empty_code` function L1774-1779 — `()` — KAIROS-T-0124 #2: the tab anchors always carry the code — an empty
-  `put_away_when_reads_as_a_moment` function L1785-1795 — `()` — The banner says WHEN, in something a person reads (KAIROS-T-0164):
-  `restore_asks_for_the_archive_capability` function L1800-1806 — `()` — Restoring asks for the same `manage_<family>` the archive asked
-  `relationship_labels_read_from_the_items_side` function L1811-1819 — `()` — The summary reads from THIS item's side: an outgoing parent edge
...
-  `TeamBody` function L280-551 — `(view_model: TeamView, on_changed: Callback<()>) -> impl IntoView` — The loaded team detail — panels in the fixed v1 order.
-  `link_state_color` function L555-562 — `(state: &str) -> &'static str` — The accent for a forge link's state — same mapping as the item
-  `AnnouncementsPanel` function L573-710 — `( team_id: String, announcements: Vec<api::Announcement>, on_changed: Callback<(...` — Pinned-first feed with a member/org-admin post box and an author-or-
-  `DocTree` function L720-730 — `(pages: Vec<api::TeamPageNode>, team_slug: String) -> impl IntoView` — The page-tree navigator: root nodes except the charter (it has its own
-  `render_tree_level` function L735-785 — `( pages: &[api::TeamPageNode], parent: Option<&str>, team_slug: &str, path_prefi...` — One nesting level: `parent`'s children in position-then-title order
...
- pub `EditorState` struct L107-119 — `{ full_access: RwSignal<bool>, manage_all: RwSignal<bool>, configure_all: RwSign...` — Reactive editor state: one signal per toggle (signals are Copy, so the
- pub `new` function L129-131 — `() -> Self` — Fresh editor, everything off (whitelist model: no grant by default).
- pub `from_capabilities` function L134-150 — `(capabilities: &[String]) -> Self` — Editor prefilled from a member's stored grants.
- pub `flags` function L166-177 — `(&self) -> SelectionFlags` — Snapshot the toggles into pure flags (untracked — call at submit).
- pub `selection` function L181-183 — `(&self) -> Vec<String>` — The capability list to send (empty means "nothing selected" — the
- pub `CapabilityEditor` function L204-296 — `(state: EditorState) -> impl IntoView` — The grant editor (see module docs for the presentation design).
- pub `CapabilityPills` function L301-319 — `(capabilities: Vec<String>) -> impl IntoView` — A member's stored grants as pills: `*` gold, family globs violet,
-  `SINGLES` variable L37-46 — `: &[&str]` — Everything the editor can emit besides the globs.
-  `SelectionFlags` type L58-78 — `= SelectionFlags` — is host-unit-tested; the component is a thin binding over it.
-  `EditorState` type L121-125 — `impl Default for EditorState` — is host-unit-tested; the component is a thin binding over it.
-  `default` function L122-124 — `() -> Self` — is host-unit-tested; the component is a thin binding over it.
-  `EditorState` type L127-184 — `= EditorState` — is host-unit-tested; the component is a thin binding over it.
-  `single_signal` function L152-163 — `(&self, name: &str) -> RwSignal<bool>` — is host-unit-tested; the component is a thin binding over it.
-  `CapabilityRow` function L189-200 — `( checked: RwSignal<bool>, #[prop(into)] label: String, #[prop(into)] name: Stri...` — One capability toggle row: switch + human label + the raw capability
-  `tests` module L322-393 — `-` — is host-unit-tested; the component is a thin binding over it.
-  `caps` function L325-327 — `(list: &[&str]) -> Vec<String>` — is host-unit-tested; the component is a thin binding over it.
-  `full_access_wins` function L331-336 — `()` — Full access always composes to exactly `["*"]`.
-  `family_glob_swallows_covered_singles` function L348-369 — `()` — Family globs travel as globs and swallow their covered singles.
-  `singles_round_trip` function L373-378 — `()` — Singles round-trip: stored grants → flags → the same set out.
-  `transition_glob_normalizes` function L382-385 — `()` — A stored `transition_*` glob normalizes to its only member.
-  `empty_selection_is_empty` function L390-392 — `()` — Nothing selected composes to the empty list (callers block the
...
-  `tests` module L84-160 — `-` — what the UI offers.
-  `me` function L88-108 — `(role: &str, grants: &[(&str, &[&str])]) -> Whoami` — what the UI offers.
-  `glob_semantics_mirror_a0006` function L111-124 — `()` — what the UI offers.
-  `org_admin_can_access_everything` function L127-131 — `()` — what the UI offers.
-  `plain_member_without_grants_is_denied` function L134-138 — `()` — what the UI offers.
-  `member_with_board_config_grant_can_access_but_is_not_admin` function L141-159 — `()` — what the UI offers.
...
- pub `EdgeProposal` struct L1254-1268 — `{ id: String, source: String, target: String, relationship: String, claim: Strin...` — One pending edge proposal, as the item page shows it (KAIROS-T-0192).
- pub `fetch_edge_proposals` function L1272-1274 — `(auth: Auth, code: String) -> Result<Vec<EdgeProposal>, ApiError>` — `GET /api/items/{short_code}/proposals` → edges an agent has suggested and
- pub `confirm_edge_proposal` function L1277-1279 — `(auth: Auth, id: String) -> Result<EdgeProposal, ApiError>` — `POST /api/proposals/{id}/confirm` — create the edge.
- pub `RelatedProposal` struct L1283-1298 — `{ short_code: String, title: String, entity_type: String, claim: String, score: ...` — One possibly-related item (KAIROS-T-0195).
- pub `RelatedWork` struct L1302-1311 — `{ proposals: Vec<RelatedProposal>, vector: bool, note: String }` — The answer to "what is related to this?" (KAIROS-T-0195).
- pub `fetch_related_work` function L1318-1320 — `(auth: Auth, code: String) -> Result<RelatedWork, ApiError>` — `GET /api/items/{short_code}/related` → what might be related, as proposals.
- pub `reject_edge_proposal` function L1323-1325 — `(auth: Auth, id: String) -> Result<EdgeProposal, ApiError>` — `POST /api/proposals/{id}/reject` — recorded, not erased.
...
-  `CreateForm` function L335-420 — `( team_id: String, parent_id: Option<String>, on_changed: Callback<()>, ) -> imp...` — The create surface inside a folder (KAIROS-T-0086): kind + slug +
-  `StructurePanel` function L428-591 — `( team: api::Team, pages: Vec<api::TeamPageNode>, node: api::TeamPageNode, on_ch...` — Rename / move / delete for an unprotected node (KAIROS-T-0086).

### docs/theme

> *Semantic summary to be generated by AI agent.*

#### docs/theme/mermaid.min.js

- pub `constructor` method L679 — `constructor()`
- pub `visitEndAnchor` method L679 — `visitEndAnchor(a)`
- pub `constructor` method L681 — `constructor()`
- pub `visitStartAnchor` method L681 — `visitStartAnchor(a)`
- pub `constructor` method L769 — `constructor(k,R,S,O,N,P=!1)`
- pub `file` method L769 — `file(k)`
- pub `from` method L769 — `from(k)`
- pub `fsPath` method L769 — `fsPath()`
- pub `isUri` method L769 — `isUri(k)`
- pub `parse` method L769 — `parse(k,R=!1)`
- pub `revive` method L769 — `revive(k)`
- pub `toJSON` method L769 — `toJSON()`
- pub `toString` method L769 — `toString(k=!1)`
- pub `with` method L769 — `with(k)`
-  `AS` function L1 — `function AS(t,e,r)`
-  `Axe` function L1 — `function Axe(t)`
-  `BF` function L1 — `function BF(t,e,r)`
-  `CS` function L1 — `function CS(t,e)`
-  `DS` function L1 — `function DS(t,e)`
-  `Dxe` function L1 — `function Dxe(t)`
-  `IF` function L1 — `function IF(t,e,r)`
-  `Ib` function L1-6 — `function Ib(t)`
-  `Ixe` function L1 — `function Ixe()`
-  `LF` function L1 — `function LF(t,e)`
-  `LS` function L1 — `function LS(t,e=kxe)`
-  `Lxe` function L1 — `function Lxe(t)`
-  `Mxe` function L1 — `function Mxe(t)`
-  `Nb` function L1 — `function Nb(t,e,r,n)`
-  `Nxe` function L1 — `function Nxe()`
-  `Oxe` function L1 — `function Oxe()`
-  `RF` function L1 — `function RF(t,e)`
-  `Rxe` function L1 — `function Rxe(t)`
-  `SS` function L1 — `function SS(t,e)`
-  `_S` function L1 — `function _S(t,e)`
-  `_xe` function L1 — `function _xe(t)`
-  `a` function L1 — `function a(s)`
-  `bxe` function L1 — `function bxe(t,e)`
-  `e` function L1 — `function e(f)`
-  `h` function L1 — `function h()`
-  `i` function L1 — `function i(f)`
-  `k` function L1 — `function k(S)`
-  `l` function L1 — `function l(f)`
-  `n` function L1 — `function n(f,d)`
-  `r` function L1 — `function r(f)`
-  `s` function L1 — `function s(l)`
-  `u` function L1 — `function u(f)`
-  `xxe` function L1 — `function xxe(t,e="defs")`
-  `y` function L1 — `function y(...v)`
-  `Et` function L7 — `function Et(zt)`
-  `vt` function L7 — `function vt()`
-  `Ar` function L12 — `function Ar(t,e)`
-  `Ge` function L12 — `function Ge()`
-  `Uf` function L12 — `function Uf(t)`
-  `Ws` function L12 — `function Ws(t)`
-  `Y1` function L12 — `function Y1(t,e)`
-  `cbe` function L12 — `function cbe(t)`
-  `iG` function L12-13 — `function iG()`
-  `or` function L12 — `function or(yt,Se,at)`
-  `r` function L12 — `function r()`
-  `ube` function L12 — `function ube(t)`
-  `$C` function L13 — `function $C(t)`
-  `B4e` function L13 — `function B4e(t,e,r)`
-  `BG` function L13 — `function BG(t)`
-  `Ct` function L13 — `function Ct(t)`
-  `EC` function L13 — `function EC(t,e)`
-  `F4e` function L13 — `function F4e(t)`
-  `G` function L13 — `function G(t,e,r,n,i,a)`
-  `Gbe` function L13 — `function Gbe(t)`
-  `JG` function L13 — `function JG(t,e)`
-  `Lbe` function L13 — `function Lbe(t)`
-  `Mbe` function L13 — `function Mbe(t)`
-  `PC` function L13 — `function PC(t)`
-  `Qbe` function L13 — `function Qbe(t,e)`
-  `RC` function L13 — `function RC(t,e,r)`
-  `VC` function L13 — `function VC(t)`
-  `Xf` function L13 — `function Xf(t)`
-  `Zbe` function L13 — `function Zbe(t)`
-  `_` function L13 — `function _()`
-  `cG` function L13 — `function cG(t)`
-  `e$` function L13 — `function e$(t,e)`
-  `ec` function L13 — `function ec(t)`
-  `jG` function L13 — `function jG(t)`
-  `k$` function L13 — `function k$(t,e,r)`
-  `le` function L13 — `function le(t,e)`
-  `n4` function L13 — `function n4(t,e)`
-  `pG` function L13 — `function pG(t,e,r,n,i)`
-  `r4e` function L13 — `function r4e(t)`
-  `uh` function L13 — `function uh(t,e,r)`
-  `vG` function L13 — `function vG(t)`
-  `w` function L13 — `function w()`
-  `w4` function L13 — `function w4(t,e)`
-  `x4` function L13 — `function x4(t)`
-  `xG` function L13 — `function xG(t)`
-  `yr` function L13 — `function yr(t,e)`
-  `n` function L267-268 — `function n()`
-  `_` function L268 — `function _(ke)`
-  `x3e` function L273 — `function x3e()`
-  `$` function L329 — `function $(H)`
-  `$3` function L329 — `function $3(t)`
-  `$4` function L329 — `function $4(t)`
-  `$5e` function L329 — `function $5e(t)`
-  `$6e` function L329 — `function $6e(t)`
-  `$7` function L329 — `function $7()`
-  `$A` function L329 — `function $A(t,e)`
-  `$U` function L329 — `function $U()`
-  `$ke` function L329 — `function $ke(t,e)`
-  `$p` function L329 — `function $p(t)`
-  `$we` function L329 — `function $we()`
-  `A` function L329 — `function A(H,q,Z)`
-  `A5e` function L329 — `function A5e()`
-  `A6e` function L329 — `function A6e(t)`
-  `A7` function L329 — `function A7(t)`
-  `ACe` function L329 — `function ACe(t)`
-  `AW` function L329 — `function AW(t)`
-  `Ake` function L329 — `function Ake(t,e)`
-  `Ay` function L329 — `function Ay(t,e)`
-  `B` function L329 — `function B(H)`
-  `B3` function L329 — `function B3(t,e)`
-  `B5e` function L329 — `function B5e(t,e,r,n,i,a,s)`
-  `B6e` function L329 — `function B6e(t)`
-  `B7` function L329 — `function B7(t,e,r)`
-  `Bke` function L329 — `function Bke(t,e)`
-  `Bp` function L329 — `function Bp(t,e,r)`
-  `Bwe` function L329 — `function Bwe(t)`
-  `By` function L329 — `function By(t)`
-  `C` function L329 — `function C(H,q,Z,ae)`
-  `C3` function L329 — `function C3()`
-  `C5e` function L329 — `function C5e(t)`
-  `CCe` function L329 — `function CCe(t,e,r)`
-  `CTe` function L329 — `function CTe(t)`
-  `CW` function L329 — `function CW(t,e)`
-  `Cke` function L329 — `function Cke(t,e)`
-  `Cy` function L329 — `function Cy(t,e)`
-  `D` function L329 — `function D(H,q,Z)`
-  `D3` function L329 — `function D3(t)`
-  `D4` function L329 — `function D4(t,e)`
-  `D6e` function L329 — `function D6e(t,e)`
-  `D7` function L329 — `function D7(t)`
-  `D8` function L329 — `function D8(t,e)`
-  `DA` function L329 — `function DA(t,e)`
-  `DTe` function L329 — `function DTe(t,e)`
-  `DV` function L329 — `function DV(t)`
-  `D_` function L329 — `function D_(t,e)`
-  `Dke` function L329 — `function Dke(t,e)`
-  `Do` function L329 — `function Do(t)`
-  `Dp` function L329 — `function Dp(t)`
-  `Dwe` function L329 — `function Dwe()`
-  `E` function L329 — `function E(H,q)`
-  `E5e` function L329 — `function E5e(t,e)`
-  `E6e` function L329 — `function E6e(t,e)`
-  `E7` function L329 — `function E7(t)`
-  `E8` function L329 — `function E8(t)`
-  `ECe` function L329 — `function ECe(t)`
-  `EH` function L329 — `function EH(t)`
-  `EU` function L329 — `function EU(t,e)`
-  `EW` function L329 — `function EW(t)`
-  `Eke` function L329 — `function Eke(t,e,r)`
-  `El` function L329 — `function El()`
-  `Ey` function L329 — `function Ey(t,e,r)`
-  `F` function L329 — `function F(H)`
-  `F3` function L329 — `function F3(t,e)`
-  `F5e` function L329 — `function F5e(t)`
-  `F6e` function L329 — `function F6e(t,e)`
-  `F7` function L329 — `function F7(t)`
-  `FA` function L329 — `function FA(t)`
-  `FU` function L329 — `function FU()`
-  `Fke` function L329 — `function Fke(t,e)`
-  `Fn` function L329 — `function Fn(t)`
-  `Fy` function L329 — `function Fy(t)`
-  `G3` function L329 — `function G3(t)`
-  `G4` function L329 — `function G4(t,e,r,n)`
-  `G5e` function L329 — `function G5e(t,e)`
-  `G6e` function L329 — `function G6e()`
-  `G7` function L329 — `function G7(t)`
-  `GA` function L329 — `function GA(t)`
-  `Gke` function L329 — `function Gke(t,e)`
-  `Gp` function L329 — `function Gp(t)`
-  `Gwe` function L329 — `function Gwe()`
-  `Gy` function L329 — `function Gy(t)`
-  `H3` function L329 — `function H3(t)`
-  `H5e` function L329 — `function H5e(t,e)`
-  `H7` function L329 — `function H7()`
-  `HA` function L329 — `function HA(t,e)`
-  `HCe` function L329 — `function HCe(t)`
-  `HV` function L329 — `function HV(t,e,r)`
-  `HW` function L329 — `function HW(t)`
-  `Hke` function L329 — `function Hke(t,e)`
-  `Hr` function L329 — `function Hr(t,e,r)`
-  `Hwe` function L329 — `function Hwe(t,e,r)`
-  `I` function L329 — `function I(D)`
-  `I3` function L329 — `function I3(t)`
-  `I5e` function L329 — `function I5e()`
-  `I6e` function L329 — `function I6e(t,e)`
-  `I7` function L329 — `function I7(t)`
-  `ICe` function L329 — `function ICe(t,e)`
-  `ITe` function L329 — `function ITe(t)`
-  `IU` function L329 — `function IU(t,e,r,n)`
-  `Ike` function L329 — `function Ike(t,e)`
-  `J` function L329 — `function J(H)`
-  `J4` function L329 — `function J4(t,e,r)`
-  `J5e` function L329 — `function J5e(t)`
-  `J7` function L329 — `function J7(t,e)`
-  `JA` function L329 — `function JA(t)`
-  `JEe` function L329 — `function JEe(t)`
-  `JV` function L329 — `function JV(t)`
-  `JW` function L329 — `function JW(t)`
-  `J_` function L329 — `function J_(t)`
-  `Jf` function L329 — `function Jf(t)`
-  `Jke` function L329 — `function Jke(t)`
-  `Js` function L329 — `function Js(t,e)`
-  `Jwe` function L329 — `function Jwe(t,e)`
-  `K` function L329 — `function K(H)`
-  `K5e` function L329 — `function K5e(t)`
-  `KA` function L329 — `function KA(t)`
-  `KEe` function L329 — `function KEe(t)`
-  `Ka` function L329 — `function Ka(t,e)`
-  `Ki` function L329 — `function Ki(t,e)`
-  `Kke` function L329 — `function Kke(t)`
-  `Ks` function L329 — `function Ks()`
-  `Kwe` function L329 — `function Kwe(t,e,r)`
-  `L` function L329 — `function L(H,q)`
-  `L4` function L329 — `function L4(t,e,r)`
-  `L5e` function L329 — `function L5e(t)`
-  `L6e` function L329 — `function L6e(t)`
-  `L7` function L329 — `function L7(t)`
-  `L8` function L329 — `function L8(t)`
-  `LA` function L329 — `function LA(t,e)`
-  `LSe` function L329 — `function LSe(t)`
-  `LV` function L329 — `function LV(t)`
-  `LW` function L329 — `function LW(t)`
-  `LY` function L329 — `function LY(t)`
-  `Lke` function L329 — `function Lke(t,e)`
-  `Lp` function L329 — `function Lp()`
-  `Lwe` function L329 — `function Lwe()`
-  `Ly` function L329 — `function Ly(t,e)`
-  `M6e` function L329 — `function M6e(t)`
-  `M8` function L329 — `function M8(t,e)`
-  `MTe` function L329 — `function MTe()`
-  `MX` function L329 — `function MX(t,e)`
-  `Mke` function L329 — `function Mke(t)`
-  `Mwe` function L329 — `function Mwe(t,e,r)`
-  `My` function L329 — `function My()`
-  `N` function L329 — `function N(H,q,Z)`
-  `N3` function L329 — `function N3(t,e,r,n,i,a,s)`
-  `N4` function L329 — `function N4(t,e)`
-  `N5e` function L329 — `function N5e(t)`
-  `NA` function L329 — `function NA(t,e)`
-  `NTe` function L329 — `function NTe(t,e,r)`
-  `NU` function L329 — `function NU(t)`
-  `NV` function L329 — `function NV(t,e)`
-  `Nke` function L329 — `function Nke(t,e)`
-  `O` function L329 — `function O(H,q,Z)`
-  `O3` function L329 — `function O3(t)`
-  `O4` function L329 — `function O4(t)`
-  `O5e` function L329 — `function O5e(t)`
-  `O6e` function L329 — `function O6e(t)`
-  `O7` function L329 — `function O7(t,e)`
-  `OCe` function L329 — `function OCe(t,e,r)`
-  `OW` function L329 — `function OW(t,e,r,n,i,a,s,l,u,h)`
-  `Oi` function L329 — `function Oi(t,e)`
-  `Oke` function L329 — `function Oke(t,e)`
-  `Op` function L329 — `function Op(t)`
-  `P` function L329 — `function P(H)`
-  `P3` function L329 — `function P3(t,e)`
-  `P5e` function L329 — `function P5e(t,e,r,n,i,a)`
-  `P6e` function L329 — `function P6e(t)`
-  `P7` function L329 — `function P7()`
-  `PCe` function L329 — `function PCe(t)`
-  `PEe` function L329 — `function PEe(t)`
-  `PU` function L329 — `function PU(t)`
-  `Pke` function L329 — `function Pke(t)`
-  `Pn` function L329 — `function Pn(t)`
-  `Pp` function L329 — `function Pp(t,e,r)`
-  `Pwe` function L329 — `function Pwe(t)`
-  `Py` function L329 — `function Py(t,e,r)`
-  `Q` function L329 — `function Q(H)`
-  `Q5e` function L329 — `function Q5e(t,e)`
-  `Q7` function L329 — `function Q7(t,e)`
-  `Q8` function L329 — `function Q8(t)`
-  `QA` function L329 — `function QA(t,e)`
-  `QV` function L329 — `function QV()`
-  `Q_` function L329 — `function Q_(t,e)`
-  `Qke` function L329 — `function Qke(t,e)`
-  `Qwe` function L329 — `function Qwe(t,e,r)`
-  `R` function L329 — `function R(H,q,Z)`
-  `R3` function L329 — `function R3()`
-  `R4` function L329 — `function R4(t,e,r)`
-  `R5e` function L329 — `function R5e()`
-  `R6e` function L329 — `function R6e()`
-  `R7` function L329 — `function R7()`
-  `R8` function L329 — `function R8(t,e,r,n)`
-  `RCe` function L329 — `function RCe(t)`
-  `RTe` function L329 — `function RTe(t,e,r)`
-  `RU` function L329 — `function RU(t)`
-  `RV` function L329 — `function RV(t,e)`
-  `Rke` function L329 — `function Rke(t,e)`
-  `Rwe` function L329 — `function Rwe(t)`
-  `S` function L329 — `function S(H,q,Z)`
-  `S5e` function L329 — `function S5e(t)`
-  `S7` function L329 — `function S7(t)`
-  `SA` function L329 — `function SA(t)`
-  `SCe` function L329 — `function SCe(t,e,r)`
-  `SH` function L329 — `function SH()`
-  `STe` function L329 — `function STe(t,e)`
-  `SW` function L329 — `function SW(t)`
-  `Sh` function L329 — `function Sh(t,e,r)`
-  `Ske` function L329 — `function Ske(t,e,r)`
-  `Sp` function L329 — `function Sp(t,e)`
-  `T` function L329 — `function T(E)`
-  `T6e` function L329 — `function T6e()`
-  `TA` function L329 — `function TA(t,e,r,n,i)`
-  `TCe` function L329 — `function TCe(t)`
-  `TTe` function L329 — `function TTe(t)`
-  `TU` function L329 — `function TU(t)`
-  `TW` function L329 — `function TW(t,e)`
-  `Tke` function L329 — `function Tke(t,e,r)`
-  `Ty` function L329 — `function Ty()`
-  `U3` function L329 — `function U3(t,e)`
-  `U4` function L329 — `function U4(t)`
-  `U5e` function L329 — `function U5e(t,e)`
-  `U6e` function L329 — `function U6e(t)`
-  `U7` function L329 — `function U7()`
-  `UA` function L329 — `function UA(t,e)`
-  `UEe` function L329 — `function UEe(t)`
-  `Uke` function L329 — `function Uke(t,e)`
-  `Up` function L329 — `function Up(t)`
-  `V3` function L329 — `function V3(t)`
-  `V5e` function L329 — `function V5e(t)`
-  `V6e` function L329 — `function V6e(t)`
-  `V7` function L329 — `function V7()`
-  `VCe` function L329 — `function VCe(t,e)`
-  `VEe` function L329 — `function VEe(t)`
-  `VW` function L329 — `function VW(t)`
-  `Vke` function L329 — `function Vke(t)`
-  `Vy` function L329 — `function Vy(t)`
-  `W` function L329 — `function W(H)`
-  `W5e` function L329 — `function W5e(t,e)`
-  `W6e` function L329 — `function W6e(t,e)`
-  `W7` function L329 — `function W7()`
-  `WA` function L329 — `function WA(t)`
-  `WCe` function L329 — `function WCe(t)`
-  `WW` function L329 — `function WW(t)`
-  `Wke` function L329 — `function Wke(t,e)`
-  `Wwe` function L329 — `function Wwe(t,e)`
-  `Wy` function L329 — `function Wy(t,e,r)`
-  `X$` function L329 — `function X$({_intern:t,_key:e},r)`
-  `X4` function L329 — `function X4(t,e,r,n,i,a)`
-  `X5e` function L329 — `function X5e(t,e,r)`
-  `X6e` function L329 — `function X6e(t,e,r)`
-  `X8` function L329 — `function X8(t)`
-  `XA` function L329 — `function XA(t)`
-  `XCe` function L329 — `function XCe(t)`
-  `XEe` function L329 — `function XEe(t)`
-  `Xke` function L329 — `function Xke(t,e)`
-  `Xwe` function L329 — `function Xwe(t)`
-  `Y4` function L329 — `function Y4(t,e)`
-  `Y5e` function L329 — `function Y5e(t,e)`
-  `Y7` function L329 — `function Y7(t)`
-  `Y8` function L329 — `function Y8(t)`
-  `YA` function L329 — `function YA(t)`
-  `YW` function L329 — `function YW(t)`
-  `Yke` function L329 — `function Yke(t,e)`
-  `Ywe` function L329 — `function Ywe(t,e,r)`
-  `Z5e` function L329 — `function Z5e(t,e)`
-  `Z6e` function L329 — `function Z6e(t,e)`
-  `Z7` function L329 — `function Z7(t)`
-  `ZA` function L329 — `function ZA()`
-  `ZV` function L329 — `function ZV()`
-  `Zf` function L329 — `function Zf(t,e)`
-  `Zke` function L329 — `function Zke(t,e)`
-  `Zwe` function L329 — `function Zwe(t,e,r)`
-  `_4` function L329 — `function _4(t,e,r)`
-  `_5e` function L329 — `function _5e()`
-  `_6e` function L329 — `function _6e(t)`
-  `_7` function L329 — `function _7(t)`
-  `_8` function L329 — `function _8(t)`
-  `_Te` function L329 — `function _Te(t)`
-  `_W` function L329 — `function _W(t,e)`
-  `_ke` function L329 — `function _ke(t,e)`
-  `_p` function L329 — `function _p(t,e,r)`
-  `_we` function L329 — `function _we()`
-  `a` function L329 — `function a(l,u,h=0,f=l.length)`
-  `a3` function L329 — `function a3(t)`
-  `a8` function L329 — `function a8(t)`
-  `aA` function L329 — `function aA(t,e)`
-  `aCe` function L329 — `function aCe(t,e)`
-  `aEe` function L329 — `function aEe()`
-  `aTe` function L329 — `function aTe(t,e)`
-  `aU` function L329 — `function aU(t,e,r,n)`
-  `aV` function L329 — `function aV(t,e,r)`
-  `aY` function L329 — `function aY(t,e)`
-  `a_` function L329 — `function a_(t)`
-  `awe` function L329 — `function awe()`
-  `b5e` function L329 — `function b5e(t,e)`
-  `b8` function L329 — `function b8(t,e)`
-  `bA` function L329 — `function bA(t)`
-  `bCe` function L329 — `function bCe(t)`
-  `bSe` function L329 — `function bSe()`
-  `bTe` function L329 — `function bTe(t)`
-  `bU` function L329 — `function bU(t)`
-  `bW` function L329 — `function bW(t,e,r)`
-  `b_` function L329 — `function b_(t)`
-  `bh` function L329 — `function bh(t)`
-  `bke` function L329 — `function bke(t,e,r)`
-  `bl` function L329 — `function bl()`
-  `bwe` function L329 — `function bwe()`
-  `c3` function L329 — `function c3(t)`
-  `c6e` function L329 — `function c6e(t,e)`
-  `c8` function L329 — `function c8(t)`
-  `cA` function L329 — `function cA(t,e,r)`
-  `cCe` function L329 — `function cCe(t,e,r,n)`
-  `cEe` function L329 — `function cEe(t)`
-  `cSe` function L329 — `function cSe(t)`
-  `cTe` function L329 — `function cTe(t)`
-  `cke` function L329 — `function cke(t)`
-  `cu` function L329 — `function cu(t,e,r,n)`
-  `cwe` function L329 — `function cwe()`
-  `cy` function L329 — `function cy(t)`
-  `d` function L329 — `function d()`
-  `d3` function L329 — `function d3(t)`
-  `d5` function L329 — `function d5(t,e)`
-  `d5e` function L329 — `function d5e(t)`
-  `dCe` function L329 — `function dCe(t,e)`
-  `dEe` function L329 — `function dEe(t,e,r,n,i,a,s,l)`
-  `dTe` function L329 — `function dTe(t,e)`
-  `dY` function L329 — `function dY(t,e,r)`
-  `dke` function L329 — `function dke(t,e,r)`
-  `du` function L329 — `function du()`
-  `dwe` function L329 — `function dwe()`
-  `dy` function L329 — `function dy(t)`
-  `e` function L329 — `function e(d,p)`
-  `e8` function L329 — `function e8(t)`
-  `eA` function L329 — `function eA(t)`
-  `eCe` function L329 — `function eCe(t)`
-  `eEe` function L329 — `function eEe(t,e)`
-  `eSe` function L329 — `function eSe(t)`
-  `eTe` function L329 — `function eTe(t,e)`
-  `eU` function L329 — `function eU()`
-  `e_` function L329 — `function e_(t)`
-  `ed` function L329 — `function ed(t)`
-  `ee` function L329 — `function ee(H)`
-  `ewe` function L329 — `function ewe(t)`
-  `f` function L329 — `function f(d,p)`
-  `f5e` function L329 — `function f5e({_intern:t,_key:e},r)`
-  `f6e` function L329 — `function f6e(t)`
-  `f7` function L329 — `function f7(t)`
-  `fEe` function L329 — `function fEe(t)`
-  `fY` function L329 — `function fY(t)`
-  `fke` function L329 — `function fke(t,e,r)`
-  `fwe` function L329 — `function fwe()`
-  `fy` function L329 — `function fy(t,e)`
-  `g3` function L329 — `function g3(t,e)`
-  `g6e` function L329 — `function g6e(t)`
-  `g8` function L329 — `function g8(t)`
-  `gCe` function L329 — `function gCe(t,e)`
-  `gEe` function L329 — `function gEe(t)`
-  `gTe` function L329 — `function gTe(t,e)`
-  `gY` function L329 — `function gY(t)`
-  `gh` function L329 — `function gh(t,e)`
-  `gke` function L329 — `function gke(t,e,r)`
-  `gn` function L329 — `function gn(t,e,r,n)`
-  `gp` function L329 — `function gp(t,e,r)`
-  `gwe` function L329 — `function gwe(t)`
-  `gy` function L329 — `function gy(t,e,r,n)`
-  `h` function L329 — `function h(d)`
-  `h5e` function L329 — `function h5e({_intern:t,_key:e},r)`
-  `h6e` function L329 — `function h6e()`
-  `hEe` function L329 — `function hEe(t)`
-  `hSe` function L329 — `function hSe(t)`
-  `hTe` function L329 — `function hTe(t)`
-  `hd` function L329 — `function hd(t)`
-  `hke` function L329 — `function hke(t,e,r)`
-  `hu` function L329 — `function hu(t,e,r,n,i,a)`
-  `hwe` function L329 — `function hwe()`
-  `hy` function L329 — `function hy(t)`
-  `i` function L329 — `function i(l,u,h=0,f=l.length)`
-  `i3` function L329 — `function i3()`
-  `i8` function L329 — `function i8(t)`
-  `i9` function L329 — `function i9(t)`
-  `iA` function L329 — `function iA(t)`
-  `iEe` function L329 — `function iEe(t,e)`
-  `iSe` function L329 — `function iSe(t,e)`
-  `iTe` function L329 — `function iTe(t,e)`
-  `iU` function L329 — `function iU(t)`
-  `i_` function L329 — `function i_(t,e)`
-  `ic` function L329 — `function ic(t)`
-  `ie` function L329 — `function ie(H)`
-  `ii` function L329 — `function ii(t,e)`
-  `iwe` function L329 — `function iwe(t)`
-  `j` function L329 — `function j(H)`
-  `j5e` function L329 — `function j5e(t,e,r)`
-  `j6e` function L329 — `function j6e(t)`
-  `j7` function L329 — `function j7(t,e,r)`
-  `j8` function L329 — `function j8(t)`
-  `jA` function L329 — `function jA(t)`
-  `ja` function L329 — `function ja(t,e,r,n)`
-  `jke` function L329 — `function jke(t,e)`
-  `jwe` function L329 — `function jwe(t,e,r)`
-  `k` function L329 — `function k(H,q,Z)`
-  `k5e` function L329 — `function k5e(t,e)`
-  `k6e` function L329 — `function k6e(t,e)`
-  `k8` function L329 — `function k8(t)`
-  `kA` function L329 — `function kA(t)`
-  `kCe` function L329 — `function kCe(t,e,r,n,i,a,s)`
-  `kW` function L329 — `function kW()`
-  `k_` function L329 — `function k_(t)`
-  `kke` function L329 — `function kke(t,e,r)`
-  `ky` function L329 — `function ky()`
-  `l` function L329 — `function l(h,f,d,p)`
-  `l3` function L329 — `function l3(t,e)`
-  `l5e` function L329 — `function l5e()`
-  `l6e` function L329 — `function l6e(t,e)`
-  `lA` function L329 — `function lA(t)`
-  `lCe` function L329 — `function lCe(t,e,r)`
-  `lEe` function L329 — `function lEe(t)`
-  `lSe` function L329 — `function lSe(t)`
-  `lTe` function L329 — `function lTe(t,e)`
-  `lY` function L329 — `function lY(t,e)`
-  `lwe` function L329 — `function lwe()`
-  `ly` function L329 — `function ly(t,e,r)`
-  `m3` function L329 — `function m3(t,e)`
-  `m8` function L329 — `function m8(t,e)`
-  `mA` function L329 — `function mA(t)`
-  `mEe` function L329 — `function mEe(t)`
-  `mTe` function L329 — `function mTe(t,e,r)`
-  `mU` function L329 — `function mU(t)`
-  `mY` function L329 — `function mY(t)`
-  `mh` function L329 — `function mh(t)`
-  `mke` function L329 — `function mke(t,e,r)`
-  `mwe` function L329 — `function mwe(t)`
-  `n` function L329 — `function n(i,a)`
-  `n8` function L329 — `function n8(t,e,r)`
-  `nA` function L329 — `function nA()`
-  `nEe` function L329 — `function nEe(t,e)`
-  `nTe` function L329 — `function nTe(t,e)`
-  `nU` function L329 — `function nU(t)`
-  `nY` function L329 — `function nY(t,e)`
-  `nwe` function L329 — `function nwe(t)`
-  `o3` function L329 — `function o3(t)`
-  `o6e` function L329 — `function o6e(t)`
-  `o8` function L329 — `function o8()`
-  `oA` function L329 — `function oA(t)`
-  `oEe` function L329 — `function oEe(t)`
-  `oH` function L329 — `function oH(t)`
-  `oSe` function L329 — `function oSe(t)`
-  `oTe` function L329 — `function oTe(t,e)`
-  `o_` function L329 — `function o_(t)`
-  `oa` function L329 — `function oa(t,e)`
-  `od` function L329 — `function od(t,e)`
-  `owe` function L329 — `function owe(t)`
-  `p` function L329 — `function p(m)`
-  `p3` function L329 — `function p3(t,e)`
-  `p7` function L329 — `function p7(t)`
-  `p8` function L329 — `function p8(t)`
-  `pA` function L329 — `function pA(t,e,r)`
-  `pEe` function L329 — `function pEe(t)`
-  `pH` function L329 — `function pH(t)`
-  `pTe` function L329 — `function pTe(t,e,r)`
-  `pU` function L329 — `function pU(t)`
-  `pW` function L329 — `function pW(t,e,r,n,i,a)`
-  `pY` function L329 — `function pY(t,e)`
-  `pke` function L329 — `function pke(t,e,r)`
-  `pwe` function L329 — `function pwe(t)`
-  `q5e` function L329 — `function q5e(t)`
-  `q6e` function L329 — `function q6e(t,e,r)`
-  `q7` function L329 — `function q7(t,e)`
-  `q8` function L329 — `function q8(t)`
-  `qA` function L329 — `function qA(t)`
-  `qCe` function L329 — `function qCe(t,e,r)`
-  `qV` function L329 — `function qV()`
-  `qke` function L329 — `function qke(t,e)`
-  `qwe` function L329 — `function qwe(t)`
-  `r` function L329 — `function r(n)`
-  `r8` function L329 — `function r8(t,e,r)`
-  `rA` function L329 — `function rA()`
-  `rEe` function L329 — `function rEe(t,e)`
-  `rSe` function L329 — `function rSe(t,e)`
-  `rTe` function L329 — `function rTe(t,e)`
-  `rU` function L329 — `function rU(t,e,r,n)`
-  `rV` function L329 — `function rV(t,e)`
-  `rwe` function L329 — `function rwe()`
-  `s` function L329 — `function s(l,u,h=0,f=l.length)`
-  `s8` function L329 — `function s8()`
-  `sA` function L329 — `function sA()`
-  `sEe` function L329 — `function sEe(t)`
-  `sTe` function L329 — `function sTe(t,e)`
-  `sV` function L329 — `function sV()`
-  `sY` function L329 — `function sY(t,e)`
-  `s_` function L329 — `function s_(t)`
-  `sa` function L329 — `function sa(t,e,r,n)`
-  `sc` function L329 — `function sc(t,e,r,n)`
-  `swe` function L329 — `function swe(t)`
-  `t` function L329 — `function t(i,a,s,l,u,h,f)`
-  `t8` function L329 — `function t8()`
-  `tA` function L329 — `function tA(t)`
-  `tCe` function L329 — `function tCe(t)`
-  `tEe` function L329 — `function tEe(t,e)`
-  `tSe` function L329 — `function tSe(t,e)`
-  `tTe` function L329 — `function tTe(t,e)`
-  `tU` function L329 — `function tU()`
-  `tY` function L329 — `function tY(t)`
-  `td` function L329 — `function td(t)`
-  `twe` function L329 — `function twe(t,e)`
-  `u` function L329 — `function u(h,f,d,p,m,g)`
-  `u7` function L329 — `function u7(t,e)`
-  `u8` function L329 — `function u8(t)`
-  `uA` function L329 — `function uA(t,e)`
-  `uCe` function L329 — `function uCe(t,e)`
-  `uEe` function L329 — `function uEe(t)`
-  `uTe` function L329 — `function uTe(t,e,r)`
-  `uY` function L329 — `function uY(t)`
-  `ud` function L329 — `function ud(t)`
-  `uke` function L329 — `function uke(t,e,r)`
-  `uu` function L329 — `function uu(t,e)`
-  `uwe` function L329 — `function uwe()`
-  `v5e` function L329 — `function v5e(t)`
-  `v7` function L329 — `function v7(t)`
-  `v8` function L329 — `function v8(t)`
-  `vA` function L329 — `function vA(t)`
-  `vSe` function L329 — `function vSe(t)`
-  `vTe` function L329 — `function vTe(t,e,r)`
-  `vU` function L329 — `function vU(t,e)`
-  `vW` function L329 — `function vW(t,e,r)`
-  `vX` function L329 — `function vX(t,e,r,n,i)`
-  `vY` function L329 — `function vY(t)`
-  `v_` function L329 — `function v_(t)`
-  `vh` function L329 — `function vh()`
-  `vke` function L329 — `function vke(t,e,r)`
-  `vl` function L329 — `function vl(t)`
-  `vp` function L329 — `function vp(t)`
-  `vwe` function L329 — `function vwe(t,e)`
-  `w5e` function L329 — `function w5e()`
-  `w6e` function L329 — `function w6e(t,e)`
-  `wA` function L329 — `function wA(t,e,r,n)`
-  `wCe` function L329 — `function wCe(t)`
-  `wH` function L329 — `function wH(t)`
-  `wTe` function L329 — `function wTe(t)`
-  `wU` function L329 — `function wU(t)`
-  `wW` function L329 — `function wW(t,e)`
-  `w_` function L329 — `function w_(t)`
-  `wh` function L329 — `function wh(t,e)`
-  `wke` function L329 — `function wke(t,e,r)`
-  `wp` function L329 — `function wp(t,e,r,n)`
-  `ws` function L329 — `function ws(t,e)`
-  `x5e` function L329 — `function x5e(t)`
-  `x6e` function L329 — `function x6e(t)`
-  `xA` function L329 — `function xA(t)`
-  `xSe` function L329 — `function xSe(t)`
-  `xTe` function L329 — `function xTe(t)`
-  `xU` function L329 — `function xU(t,e)`
-  `xW` function L329 — `function xW(t,e,r)`
-  `xY` function L329 — `function xY(t)`
-  `x_` function L329 — `function x_(t)`
-  `xh` function L329 — `function xh(t,e)`
-  `xke` function L329 — `function xke(t,e,r)`
-  `xl` function L329 — `function xl(t)`
-  `xp` function L329 — `function xp(t,e)`
-  `xwe` function L329 — `function xwe(t,e)`
-  `y5e` function L329 — `function y5e(t)`
-  `y7` function L329 — `function y7(t)`
-  `yA` function L329 — `function yA(t)`
-  `yCe` function L329 — `function yCe(t)`
-  `yEe` function L329 — `function yEe(t)`
-  `ySe` function L329 — `function ySe(t)`
-  `yTe` function L329 — `function yTe(t,e,r)`
-  `yW` function L329 — `function yW(t,e,r)`
-  `y_` function L329 — `function y_(t,e,r)`
-  `yh` function L329 — `function yh(t,e,r)`
-  `yke` function L329 — `function yke(t,e,r)`
-  `yl` function L329 — `function yl(t,e,r,n)`
-  `yp` function L329 — `function yp(t)`
-  `ywe` function L329 — `function ywe(t,e,r)`
-  `z` function L329 — `function z(H)`
-  `z4` function L329 — `function z4(t)`
-  `z5e` function L329 — `function z5e(t)`
-  `z7` function L329 — `function z7()`
-  `zEe` function L329 — `function zEe(t)`
-  `ze` function L329 — `function ze(t)`
-  `zke` function L329 — `function zke(t,e)`
-  `zp` function L329 — `function zp(t)`
-  `zwe` function L329 — `function zwe()`
-  `Ll` function L330 — `function Ll(t,e,r,n,i)`
-  `zX` function L330 — `function zX(t,e,r,n,i)`
-  `KX` function L334 — `function KX(t,e,r,n)`
-  `XX` function L334 — `function XX(t)`
-  `ZX` function L334 — `function ZX(t)`
-  `d7e` function L334 — `function d7e(t)`
-  `eo` function L334 — `function eo(t,e)`
-  `fn` function L334 — `function fn(t,e)`
-  `g7e` function L334-336 — `function g7e(t,e)`
-  `jX` function L334 — `function jX(t,e)`
-  `lv` function L334 — `function lv(t,e,r)`
-  `m7e` function L334 — `function m7e(t,e)`
-  `u9` function L334 — `function u9()`
-  `en` function L336 — `function en(t,e)`
-  `U7e` function L389-391 — `function U7e(t,{markdownAutoWrap:e})`
-  `cj` function L391-392 — `function cj(t,e={})`
-  `s` function L391-392 — `function s(l,u="normal")`
-  `H7e` function L392 — `function H7e(t)`
-  `W7e` function L392 — `function W7e(t,e)`
-  `dj` function L392-393 — `function dj(t,e)`
-  `fj` function L392 — `function fj(t,e,r,n)`
-  `n` function L392 — `function n(i)`
-  `uj` function L392 — `function uj(t,{markdownAutoWrap:e}={})`
-  `Y7e` function L393-394 — `function Y7e(t,e,r,n,i=!1)`
-  `g9` function L393 — `function g9(t,e,r=[],n=[])`
-  `mj` function L393 — `function mj(t,e)`
-  `$j` function L394 — `function $j(t,e)`
-  `Aa` function L394 — `function Aa(t,e,r,n)`
-  `Bj` function L394 — `function Bj(t,e,r,n,i,a,s,l,u,h)`
-  `C5` function L394 — `function C5(t,e,r)`
-  `C9` function L394 — `function C9(t,e=.15,r)`
-  `D5` function L394 — `function D5(t,e,r,n=1)`
-  `Dj` function L394 — `function Dj(t,e)`
-  `E9` function L394 — `function E9(t,e)`
-  `F9` function L394 — `function F9(t)`
-  `Fj` function L394 — `function Fj(t,e,r,n,i)`
-  `Gj` function L394 — `function Gj(t)`
-  `Hj` function L394 — `function Hj(t,e)`
-  `I9` function L394 — `function I9(t,e,r,n,i,a,s)`
-  `Ij` function L394 — `function Ij(t,e=0)`
-  `J7e` function L394 — `function J7e(t,e,r)`
-  `K7e` function L394 — `function K7e(t,e,r,n)`
-  `Kj` function L394 — `function Kj(t,e)`
-  `L5` function L394 — `function L5(t,e)`
-  `Lj` function L394 — `function Lj(t,e,r,n,i,a,s,l,u)`
-  `M5` function L394 — `function M5(t)`
-  `M9` function L394 — `function M9(t,e,r,n)`
-  `Mj` function L394 — `function Mj(t,e,r,n,i,a,s,l,u)`
-  `N5` function L394 — `function N5(t,e,r)`
-  `Nj` function L394 — `function Nj(t)`
-  `O9` function L394 — `function O9(t,e,r,n)`
-  `Oj` function L394 — `function Oj(t)`
-  `Ph` function L394 — `function Ph(t,e,r,n,i,a=!1)`
-  `Pj` function L394 — `function Pj(t)`
-  `Q7e` function L394 — `function Q7e(t,e,r)`
-  `R5` function L394 — `function R5(t,e,r,n,i)`
-  `Rj` function L394 — `function Rj(t,e,r,n,i,a,s,l)`
-  `S9` function L394 — `function S9(t,e)`
-  `Sd` function L394 — `function Sd(t,e,r)`
-  `T9` function L394 — `function T9(t,e,r)`
-  `Uj` function L394 — `function Uj(t,e,r,n,i,a,s)`
-  `Wt` function L394 — `function Wt(t)`
-  `X7e` function L394 — `function X7e(t,e,r,n=!1)`
-  `Xj` function L394 — `function Xj(t,e)`
-  `Yj` function L394 — `function Yj(t,e)`
-  `Z7e` function L394 — `function Z7e(t,e,r,n)`
-  `_5` function L394 — `function _5(t,e,r)`
-  `_j` function L394 — `function _j(t,e)`
-  `am` function L394 — `function am(t,e)`
-  `cAe` function L394 — `function cAe(t,e)`
-  `gj` function L394 — `function gj(t,e,r)`
-  `j7e` function L394 — `function j7e(t,e)`
-  `k5` function L394 — `function k5(t,e,r,n,i,a)`
-  `lAe` function L394 — `function lAe(t,e,r)`
-  `mv` function L394 — `function mv(t)`
-  `nAe` function L394 — `function nAe(t,e,r,n=1)`
-  `nr` function L394 — `function nr(t,e,r=1)`
-  `oAe` function L394 — `function oAe(t,e,r,n,i,a,s,l)`
-  `pv` function L394 — `function pv(t,e,r)`
-  `q7e` function L394 — `function q7e(t,e,r)`
-  `rAe` function L394 — `function rAe(t,e)`
-  `sAe` function L394 — `function sAe(t,e,r,n,i)`
-  `uAe` function L394-395 — `function uAe(t)`
-  `v9` function L394 — `function v9(t,e)`
-  `wj` function L394 — `function wj(t,e)`
-  `x` function L394 — `function x()`
-  `x9` function L394 — `function x9(t)`
-  `y9` function L394 — `function y9(t,e,r)`
-  `yv` function L394 — `function yv(t,e)`
-  `zj` function L394 — `function zj(t,e,r)`
-  `zo` function L394 — `function zo(t,e,r,n,i,a)`
-  `Bh` function L395 — `function Bh(t,e,r,n=100,i=0,a=180)`
-  `CK` function L395 — `function CK(t,e,{config:{themeVariables:r,flowchart:n}})`
-  `Cd` function L395 — `function Cd(t,e,r)`
-  `DK` function L395 — `function DK(t,e,{config:{themeVariables:r,flowchart:n}})`
-  `EK` function L395 — `function EK(t,e)`
-  `FK` function L395 — `function FK(t,e)`
-  `Fh` function L395 — `function Fh(t,e,r,n=100,i=0,a=180)`
-  `GK` function L395 — `function GK(t,e)`
-  `HK` function L395 — `function HK(t,e)`
-  `IK` function L395 — `function IK(t,e,{config:{flowchart:r}})`
-  `KK` function L395 — `function KK(t,e)`
-  `PK` function L395 — `function PK(t,e)`
-  `RK` function L395 — `function RK(t,e,{config:{themeVariables:r,flowchart:n}})`
-  `TK` function L395 — `function TK(t,e)`
-  `VK` function L395 — `function VK(t,e)`
-  `XK` function L395 — `function XK(t,e)`
-  `YK` function L395 — `function YK(t,e)`
-  `ZK` function L395 — `function ZK(t,e)`
-  `Zj` function L395 — `function Zj(t,e)`
-  `_K` function L395 — `function _K(t,e,{config:{themeVariables:r,flowchart:n}})`
-  `_a` function L395 — `function _a(t,e,r,n=100,i=0,a=180)`
-  `bK` function L395 — `function bK(t,e)`
-  `eK` function L395 — `function eK(t,e)`
-  `eQ` function L395 — `function eQ(t,e,{config:{themeVariables:r}})`
-  `fK` function L395 — `function fK(t,e)`
-  `gK` function L395 — `function gK(t,e)`
-  `iK` function L395 — `function iK(t,e)`
-  `lK` function L395 — `function lK(t,e)`
-  `pK` function L395 — `function pK(t,e,{config:{themeVariables:r}})`
-  `rK` function L395 — `function rK(t,e)`
-  `rQ` function L395-399 — `function rQ(t,e)`
-  `sK` function L395 — `function sK(t,e)`
-  `uK` function L395 — `function uK(t,e)`
-  `vK` function L395 — `function vK(t,e,{dir:r,config:{state:n,themeVariables:i}})`
-  `bAe` function L399-400 — `function bAe(t)`
-  `iQ` function L399 — `function iQ(t,e)`
-  `xAe` function L399 — `function xAe(t,e)`
-  `CQ` function L400 — `function CQ(t,e)`
-  `DQ` function L400 — `function DQ(t,e)`
-  `EQ` function L400 — `function EQ(t,e)`
-  `RQ` function L400 — `function RQ(t,e)`
-  `TQ` function L400 — `function TQ(t,e,{config:{themeVariables:r}})`
-  `_Q` function L400 — `function _Q(t,e)`
-  `bQ` function L400 — `function bQ(t,e,{config:{themeVariables:r}})`
-  `fQ` function L400 — `function fQ(t,e)`
-  `gQ` function L400 — `function gQ(t,e)`
-  `lQ` function L400 — `function lQ(t,e)`
-  `pQ` function L400 — `function pQ(t,e)`
-  `sQ` function L400 — `function sQ(t,e)`
-  `uQ` function L400 — `function uQ(t,e)`
-  `vQ` function L400 — `function vQ(t,e)`
-  `FQ` function L406 — `function FQ(t,e)`
-  `GQ` function L406 — `function GQ(t,e)`
-  `HQ` function L406-408 — `function HQ(t,e)`
-  `IQ` function L406 — `function IQ(t,e)`
-  `PQ` function L406 — `function PQ(t,e)`
-  `VQ` function L406 — `function VQ(t,e)`
-  `DAe` function L408 — `function DAe(t,e)`
-  `KQ` function L408 — `function KQ(t,e,{config:r})`
-  `LAe` function L408 — `function LAe(t)`
-  `NAe` function L408 — `function NAe(t,e)`
-  `O5` function L408 — `function O5(t,e,r,n=[])`
-  `RAe` function L408 — `function RAe(t)`
-  `XQ` function L408 — `function XQ(t,e)`
-  `YQ` function L408 — `function YQ(t,e,r,n,i=r.class.padding??12)`
-  `ZQ` function L408 — `function ZQ(t)`
-  `_Ae` function L408 — `function _Ae(t)`
-  `mZ` function L408-410 — `function mZ(t,e)`
-  `pZ` function L408 — `function pZ(t)`
-  `x` function L408 — `function x()`
-  `$9` function L410 — `function $9(t,e,r,n,i)`
-  `V9` function L410 — `function V9(t,e)`
-  `bv` function L410 — `function bv(t,e)`
-  `zAe` function L410-414 — `function zAe(t,e)`
-  `$8e` function L414 — `function $8e(t)`
-  `A8e` function L414 — `function A8e(t)`
-  `B8e` function L414 — `function B8e(t)`
-  `C8e` function L414 — `function C8e(t)`
-  `E8e` function L414 — `function E8e(t)`
-  `F8e` function L414 — `function F8e(t)`
-  `H9` function L414 — `function H9(t)`
-  `HAe` function L414 — `function HAe(t,e)`
-  `I8e` function L414 — `function I8e(t)`
-  `JAe` function L414 — `function JAe(t)`
-  `JQ` function L414 — `function JQ(t,e)`
-  `K8e` function L414 — `function K8e(t)`
-  `L8e` function L414 — `function L8e(t)`
-  `M8e` function L414 — `function M8e(t)`
-  `QAe` function L414 — `function QAe(t)`
-  `Ss` function L414 — `function Ss(t)`
-  `T8e` function L414 — `function T8e(t)`
-  `UAe` function L414 — `function UAe(t)`
-  `V8e` function L414 — `function V8e(t)`
-  `WAe` function L414 — `function WAe()`
-  `X8e` function L414 — `function X8e(t)`
-  `ZAe` function L414 — `function ZAe()`
-  `_8e` function L414 — `function _8e(t)`
-  `_d` function L414 — `function _d(t)`
-  `a8e` function L414 — `function a8e(t)`
-  `b8e` function L414 — `function b8e(t)`
-  `c8e` function L414 — `function c8e(t)`
-  `d8e` function L414 — `function d8e(t)`
-  `g8e` function L414 — `function g8e(t,e)`
-  `j8e` function L414 — `function j8e(t)`
-  `l8e` function L414 — `function l8e(t)`
-  `lm` function L414 — `function lm(t)`
-  `n` function L414 — `function n(i)`
-  `n8e` function L414 — `function n8e(t)`
-  `o8e` function L414 — `function o8e(t)`
-  `p8e` function L414 — `function p8e(t)`
-  `r8e` function L414 — `function r8e(t)`
-  `rZ` function L414-415 — `function rZ(t)`
-  `s8e` function L414 — `function s8e(t)`
-  `t8e` function L414 — `function t8e(t)`
-  `tZ` function L414 — `function tZ(t)`
-  `u8e` function L414 — `function u8e(t)`
-  `w8e` function L414 — `function w8e(t)`
-  `y8e` function L414 — `function y8e(t)`
-  `yc` function L414 — `function yc(t)`
-  `$5` function L415 — `function $5(t)`
-  `CZ` function L415 — `function CZ(t,e)`
-  `Ci` function L415 — `function Ci(t,e,r)`
-  `F5` function L415 — `function F5(t,e)`
-  `Gh` function L415 — `function Gh(t,e,r,n)`
-  `K9` function L415 — `function K9(t)`
-  `Q8e` function L415 — `function Q8e(t)`
-  `Q9` function L415-416 — `function Q9(t,e)`
-  `Qt` function L415 — `function Qt(t,e)`
-  `Z8e` function L415 — `function Z8e(t,e)`
-  `cm` function L415 — `function cm(t,e,r,n,i,a,s,l,u)`
-  `iZ` function L415 — `function iZ(t,e,r,n)`
-  `J8e` function L416 — `function J8e(t,e,r)`
-  `e_e` function L416 — `function e_e(t,e)`
-  `n_e` function L416-422 — `function n_e(t,e)`
-  `r_e` function L416 — `function r_e(t,e)`
-  `t_e` function L416 — `function t_e(t,e)`
-  `AZ` function L422-423 — `function AZ(t,e)`
-  `aZ` function L422 — `function aZ(t,e)`
-  `a_e` function L422 — `function a_e(t)`
-  `i_e` function L422 — `function i_e(t,e,r)`
-  `l_e` function L422 — `function l_e(t)`
-  `o_e` function L422 — `function o_e(t)`
-  `s_e` function L422 — `function s_e(t)`
-  `um` function L422 — `function um(t,e,r,n,i)`
-  `I_e` function L423 — `function I_e(t)`
-  `N_e` function L423 — `function N_e(t,e)`
-  `R_e` function L423 — `function R_e(t)`
-  `c_e` function L423 — `function c_e(t,e,r)`
-  `sZ` function L423-425 — `function sZ(t,e)`
-  `u_e` function L423 — `function u_e(t,e)`
-  `Y9` function L425-426 — `function Y9(t,e)`
-  `B_e` function L426 — `function B_e(t)`
-  `F_e` function L426 — `function F_e(t,e,r,n,i,a,s,l)`
-  `G5` function L426 — `function G5(t)`
-  `O_e` function L426 — `function O_e(t,e)`
-  `PZ` function L426 — `function PZ(t)`
-  `P_e` function L426 — `function P_e(t)`
-  `cZ` function L426-430 — `function cZ(t,e)`
-  `kv` function L426 — `function kv(t)`
-  `lZ` function L426 — `function lZ(t,e,r)`
-  `oZ` function L426 — `function oZ(t)`
-  `u` function L426 — `function u(h)`
-  `xv` function L426 — `function xv(t,e)`
-  `z_e` function L426 — `function z_e(t,e,r,n,i)`
-  `uZ` function L430-431 — `function uZ(t)`
-  `G_e` function L431-434 — `function G_e(t,e)`
-  `hZ` function L434-437 — `function hZ(t,e)`
-  `$_e` function L437 — `function $_e(t)`
-  `Cu` function L437 — `function Cu(t,e,r,n,i,a,s)`
-  `H_e` function L437 — `function H_e(t,e,r,n)`
-  `U_e` function L437 — `function U_e(t,e,r)`
-  `V_e` function L437 — `function V_e(t,e,r)`
-  `W_e` function L437 — `function W_e(t,e)`
-  `X9` function L437 — `function X9(t,e,r)`
-  `Y_e` function L437-438 — `function Y_e(t,e)`
-  `dZ` function L437 — `function dZ(t,e,r)`
-  `fZ` function L437 — `function fZ(t,e,r,n)`
-  `J9` function L438 — `function J9(t,e)`
-  `rL` function L439 — `function rL(t)`
-  `J5` function L447 — `function J5(t,e)`
-  `P9e` function L447 — `function P9e(t)`
-  `Q5` function L447 — `function Q5(t,e)`
-  `a` function L447 — `function a(h)`
-  `$Ie` function L450 — `function $Ie(t,e)`
-  `$Le` function L450 — `function $Le(t,e)`
-  `$Me` function L450 — `function $Me(t)`
-  `$Oe` function L450 — `function $Oe(t)`
-  `$Re` function L450 — `function $Re(t,e,r,n)`
-  `ADe` function L450 — `function ADe(t)`
-  `AOe` function L450 — `function AOe(t,e)`
-  `ARe` function L450 — `function ARe(t)`
-  `Ane` function L450 — `function Ane(t)`
-  `BIe` function L450 — `function BIe(t,e,r,n)`
-  `BMe` function L450 — `function BMe(t)`
-  `BNe` function L450 — `function BNe(t)`
-  `COe` function L450 — `function COe(t,e)`
-  `CRe` function L450 — `function CRe(t)`
-  `Cie` function L450 — `function Cie(t)`
-  `Cne` function L450 — `function Cne(t)`
-  `Cw` function L450 — `function Cw(t,e,r,n,i,a)`
-  `DDe` function L450 — `function DDe(t)`
-  `DIe` function L450 — `function DIe(t)`
-  `DOe` function L450 — `function DOe(t)`
-  `DRe` function L450 — `function DRe(t,e,r,n)`
-  `Dne` function L450 — `function Dne(t)`
-  `Du` function L450 — `function Du(t,e)`
-  `ED` function L450 — `function ED(t)`
-  `EMe` function L450 — `function EMe(t,e,r)`
-  `EOe` function L450 — `function EOe(t,e,r,n)`
-  `Ec` function L450 — `function Ec(t,e,r,n)`
-  `Eie` function L450 — `function Eie(t)`
-  `FIe` function L450 — `function FIe(t,e)`
-  `FLe` function L450 — `function FLe(t)`
-  `FMe` function L450 — `function FMe(t)`
-  `FNe` function L450 — `function FNe(t)`
-  `Fd` function L450 — `function Fd(t,e)`
-  `Fne` function L450 — `function Fne(t,e,r,n,i,a)`
-  `GIe` function L450 — `function GIe(t,e,r)`
-  `GLe` function L450 — `function GLe(t,e,r)`
-  `GNe` function L450 — `function GNe(t,e)`
-  `HIe` function L450 — `function HIe(t)`
-  `HLe` function L450 — `function HLe(t)`
-  `HMe` function L450 — `function HMe(t,e)`
-  `HNe` function L450 — `function HNe(t)`
-  `HOe` function L450 — `function HOe(t)`
-  `HRe` function L450 — `function HRe(t,e,r)`
-  `Hne` function L450 — `function Hne(t,e,r)`
-  `Ho` function L450 — `function Ho(t)`
-  `IIe` function L450 — `function IIe(t,e,r)`
-  `IMe` function L450 — `function IMe(t,e)`
-  `INe` function L450 — `function INe(t)`
-  `IRe` function L450 — `function IRe(t,e,r,n)`
-  `Iv` function L450 — `function Iv(t)`
-  `JDe` function L450 — `function JDe(t)`
-  `JL` function L450 — `function JL(t,e)`
-  `JOe` function L450 — `function JOe(t)`
-  `JRe` function L450 — `function JRe(t)`
-  `Jne` function L450 — `function Jne(t)`
-  `K9e` function L450 — `function K9e(t)`
-  `KOe` function L450 — `function KOe(t)`
-  `KRe` function L450 — `function KRe(t,e,r)`
-  `Kh` function L450 — `function Kh(t)`
-  `Kne` function L450 — `function Kne(t)`
-  `LLe` function L450 — `function LLe(t,e)`
-  `LMe` function L450 — `function LMe(t)`
-  `LOe` function L450 — `function LOe(t,e)`
-  `LRe` function L450 — `function LRe(t)`
-  `MIe` function L450 — `function MIe(t)`
-  `MMe` function L450 — `function MMe(t)`
-  `Mv` function L450 — `function Mv(t,e,r,n)`
-  `NDe` function L450 — `function NDe(t)`
-  `NIe` function L450 — `function NIe(t,e)`
-  `NLe` function L450 — `function NLe(t)`
-  `NMe` function L450 — `function NMe(t)`
-  `NNe` function L450 — `function NNe(t)`
-  `NOe` function L450 — `function NOe(t,e)`
-  `NRe` function L450 — `function NRe(t,e)`
-  `Nne` function L450 — `function Nne(t)`
-  `OIe` function L450 — `function OIe(t,e)`
-  `OLe` function L450 — `function OLe(t)`
-  `OMe` function L450 — `function OMe(t,e)`
-  `ONe` function L450 — `function ONe(t)`
-  `ORe` function L450 — `function ORe(t,e)`
-  `Ov` function L450 — `function Ov()`
-  `PLe` function L450 — `function PLe(t,e)`
-  `PMe` function L450 — `function PMe(t,e,r)`
-  `Q9e` function L450 — `function Q9e(t,e)`
-  `QDe` function L450 — `function QDe(t,e,r)`
-  `QOe` function L450 — `function QOe(t)`
-  `QRe` function L450 — `function QRe(t,e)`
-  `Qee` function L450 — `function Qee(t,e,r,n,i)`
-  `Qne` function L450 — `function Qne(t)`
-  `ROe` function L450 — `function ROe(t,e)`
-  `RRe` function L450 — `function RRe(t,e)`
-  `SOe` function L450 — `function SOe(t,e)`
-  `SRe` function L450 — `function SRe(t,e)`
-  `Sne` function L450 — `function Sne(t)`
-  `TD` function L450 — `function TD(t,e,r,n)`
-  `TIe` function L450 — `function TIe(t,e)`
-  `TLe` function L450 — `function TLe(t)`
-  `TMe` function L450 — `function TMe(t,e,r,n)`
-  `TOe` function L450 — `function TOe(t,e,r,n)`
-  `TRe` function L450 — `function TRe(t,e)`
-  `Tne` function L450 — `function Tne(t,e,r,n,i,a)`
-  `UIe` function L450 — `function UIe(t)`
-  `ULe` function L450 — `function ULe(t)`
-  `UMe` function L450 — `function UMe(t,e,r)`
-  `UNe` function L450 — `function UNe(t,e,r,n,i,a)`
-  `UOe` function L450 — `function UOe(t)`
-  `URe` function L450 — `function URe(t)`
-  `Une` function L450 — `function Une(t,e,r)`
-  `VIe` function L450 — `function VIe(t)`
-  `VLe` function L450 — `function VLe(t)`
-  `VMe` function L450 — `function VMe(t,e,r,n)`
-  `VOe` function L450 — `function VOe(t)`
-  `WLe` function L450 — `function WLe(t,e,r)`
-  `WMe` function L450 — `function WMe(t,e)`
-  `WNe` function L450 — `function WNe(t)`
-  `WOe` function L450 — `function WOe(t)`
-  `WRe` function L450 — `function WRe(t,e,r)`
-  `Wne` function L450 — `function Wne(t,e,r,n,i)`
-  `XIe` function L450 — `function XIe(t,e)`
-  `XMe` function L450 — `function XMe(t,e,r)`
-  `XOe` function L450 — `function XOe(t)`
-  `XRe` function L450 — `function XRe(t,e)`
-  `Xne` function L450 — `function Xne(t,e,r,n)`
-  `YIe` function L450 — `function YIe(t,e)`
-  `YMe` function L450 — `function YMe(t,e)`
-  `YOe` function L450 — `function YOe(t)`
-  `YRe` function L450 — `function YRe(t)`
-  `Yne` function L450 — `function Yne(t)`
-  `ZOe` function L450 — `function ZOe(t)`
-  `ZRe` function L450 — `function ZRe(t,e)`
-  `Zh` function L450 — `function Zh(t)`
-  `Zne` function L450 — `function Zne(t,e,r,n,i,a,s)`
-  `_De` function L450 — `function _De(t,e)`
-  `_Ie` function L450 — `function _Ie(t,e,r)`
-  `_Oe` function L450 — `function _Oe(t,e,r)`
-  `_Re` function L450 — `function _Re(t)`
-  `_ne` function L450 — `function _ne(t)`
-  `_w` function L450 — `function _w(t)`
-  `a` function L450 — `function a(s)`
-  `aD` function L450 — `function aD(t)`
-  `aDe` function L450 — `function aDe()`
-  `aMe` function L450 — `function aMe(t,e)`
-  `aOe` function L450 — `function aOe(t)`
-  `aRe` function L450 — `function aRe(t,e,r,n,i,a,s)`
-  `aie` function L450 — `function aie(t,e)`
-  `bD` function L450 — `function bD(t)`
-  `bMe` function L450 — `function bMe(t)`
-  `bOe` function L450 — `function bOe(t,e)`
-  `bRe` function L450 — `function bRe(t,e)`
-  `bne` function L450 — `function bne(t,e)`
-  `cDe` function L450 — `function cDe(t,e)`
-  `cIe` function L450 — `function cIe(t,e)`
-  `cLe` function L450 — `function cLe(t)`
-  `cOe` function L450 — `function cOe(t,e,r)`
-  `cRe` function L450 — `function cRe(t,e,r,n,i,a)`
-  `cie` function L450 — `function cie(t,e)`
-  `cne` function L450 — `function cne(t)`
-  `dD` function L450 — `function dD(t)`
-  `dDe` function L450 — `function dDe(t,e,r)`
-  `dIe` function L450 — `function dIe(t,e,r,n)`
-  `dLe` function L450 — `function dLe(t,e)`
-  `dOe` function L450 — `function dOe(t,e)`
-  `die` function L450 — `function die(t,e,r)`
-  `dne` function L450 — `function dne(t)`
-  `e` function L450 — `function e(r)`
-  `eD` function L450 — `function eD(t,e,r,n,i)`
-  `eDe` function L450 — `function eDe(t)`
-  `eLe` function L450 — `function eLe(t)`
-  `eOe` function L450 — `function eOe(t,e,r)`
-  `ePe` function L450 — `function ePe(t)`
-  `f` function L450 — `function f(d)`
-  `fDe` function L450 — `function fDe(t,e)`
-  `fLe` function L450 — `function fLe()`
-  `fMe` function L450 — `function fMe(t,e)`
-  `fOe` function L450 — `function fOe(t)`
-  `fRe` function L450 — `function fRe(t,e,r,n,i,a)`
-  `gD` function L450 — `function gD(t,e)`
-  `gIe` function L450 — `function gIe(t,e,r,n,i)`
-  `gLe` function L450 — `function gLe(t,e,r)`
-  `gMe` function L450 — `function gMe(t,e)`
-  `gOe` function L450 — `function gOe(t,e,r,n)`
-  `gRe` function L450 — `function gRe(t)`
-  `gm` function L450 — `function gm(t,e,r)`
-  `gne` function L450 — `function gne(t)`
-  `h` function L450 — `function h(d)`
-  `hJ` function L450 — `function hJ(t)`
-  `hOe` function L450 — `function hOe(t,e)`
-  `hie` function L450 — `function hie(t,e)`
-  `hne` function L450 — `function hne(t,e)`
-  `i` function L450 — `function i(a)`
-  `iD` function L450 — `function iD(t,e,r,n)`
-  `iDe` function L450 — `function iDe(t,e)`
-  `iMe` function L450 — `function iMe(t,e)`
-  `iOe` function L450 — `function iOe(t)`
-  `iT` function L450 — `function iT(t)`
-  `jIe` function L450 — `function jIe(t,e,r)`
-  `jOe` function L450 — `function jOe(t)`
-  `jRe` function L450 — `function jRe(t,e)`
-  `kD` function L450 — `function kD(t,e)`
-  `kIe` function L450 — `function kIe(t,e,r)`
-  `kLe` function L450 — `function kLe(t)`
-  `kOe` function L450 — `function kOe(t,e,r,n,i)`
-  `kie` function L450 — `function kie(t,e,r)`
-  `kne` function L450 — `function kne(t)`
-  `lIe` function L450 — `function lIe(t)`
-  `lMe` function L450 — `function lMe(t,e)`
-  `lOe` function L450 — `function lOe(t)`
-  `lne` function L450 — `function lne(t,e)`
-  `mDe` function L450 — `function mDe(t)`
-  `mLe` function L450 — `function mLe(t)`
-  `mMe` function L450 — `function mMe(t,e)`
-  `mOe` function L450 — `function mOe(t,e)`
-  `mRe` function L450 — `function mRe(t,e,r,n)`
-  `n` function L450 — `function n(a)`
-  `nD` function L450 — `function nD(t,e)`
-  `nDe` function L450 — `function nDe(t,e)`
-  `nMe` function L450 — `function nMe(t)`
-  `nie` function L450 — `function nie(t,e,r)`
-  `oD` function L450 — `function oD(t)`
-  `oIe` function L450 — `function oIe(t)`
-  `oLe` function L450 — `function oLe(t)`
-  `oMe` function L450 — `function oMe(t,e)`
-  `oT` function L450 — `function oT(t,e,r)`
-  `oie` function L450 — `function oie(t)`
-  `one` function L450 — `function one(t,e)`
-  `pDe` function L450 — `function pDe(t)`
-  `pIe` function L450 — `function pIe(t)`
-  `pLe` function L450 — `function pLe(t,e,r,n)`
-  `pOe` function L450 — `function pOe(t,e)`
-  `pie` function L450 — `function pie(t,e)`
-  `pne` function L450 — `function pne(t)`
-  `qIe` function L450 — `function qIe(t,e)`
-  `qMe` function L450 — `function qMe(t,e,r)`
-  `qOe` function L450 — `function qOe(t)`
-  `qRe` function L450 — `function qRe(t,e)`
-  `qne` function L450 — `function qne(t,e,r)`
-  `r` function L450 — `function r(n)`
-  `rDe` function L450 — `function rDe(t,e)`
-  `rLe` function L450 — `function rLe(t)`
-  `rNe` function L450 — `function rNe(t)`
-  `rOe` function L450 — `function rOe(t,e,r)`
-  `sD` function L450 — `function sD(t)`
-  `sMe` function L450 — `function sMe(t,e)`
-  `sOe` function L450 — `function sOe(t)`
-  `sT` function L450 — `function sT(t)`
-  `tD` function L450 — `function tD(t,e,r)`
-  `tDe` function L450 — `function tDe(t,e,r,n)`
-  `tMe` function L450 — `function tMe(t,e,r)`
-  `tOe` function L450 — `function tOe(t,e)`
-  `tPe` function L450 — `function tPe(t)`
-  `tie` function L450 — `function tie(t,e,r)`
-  `u` function L450 — `function u(d,p)`
-  `uLe` function L450 — `function uLe(t)`
-  `uOe` function L450 — `function uOe(t)`
-  `vD` function L450 — `function vD(t,e)`
-  `vIe` function L450 — `function vIe(t,e)`
-  `vLe` function L450 — `function vLe(t,e)`
-  `vMe` function L450 — `function vMe(t)`
-  `vOe` function L450 — `function vOe(t,e)`
-  `vRe` function L450 — `function vRe(t,e)`
-  `vie` function L450 — `function vie(t,e)`
-  `vne` function L450 — `function vne(t)`
-  `wIe` function L450 — `function wIe(t)`
-  `wOe` function L450 — `function wOe(t,e,r)`
-  `wRe` function L450 — `function wRe(t,e,r)`
-  `wie` function L450 — `function wie(t)`
-  `wne` function L450 — `function wne(t,e)`
-  `xD` function L450 — `function xD(t,e)`
-  `xMe` function L450 — `function xMe(t,e)`
-  `xOe` function L450 — `function xOe(t,e)`
-  `xRe` function L450 — `function xRe(t)`
-  `xie` function L450 — `function xie(t)`
-  `xne` function L450 — `function xne(t,e)`
-  `yD` function L450 — `function yD(t,e)`
-  `yIe` function L450 — `function yIe(t,e,r)`
-  `yLe` function L450 — `function yLe(t,e,r)`
-  `yOe` function L450 — `function yOe(t)`
-  `yRe` function L450 — `function yRe(t)`
-  `yie` function L450 — `function yie(t,e,r)`
-  `yne` function L450 — `function yne(t)`
-  `zJ` function L450 — `function zJ(t,e,r,n,i)`
-  `zLe` function L450 — `function zLe(t,e)`
-  `zMe` function L450 — `function zMe(t,e)`
-  `zNe` function L450 — `function zNe(t,e)`
-  `zRe` function L450 — `function zRe(t,e,r)`
-  `Sat` function L453 — `function Sat(Vs)`
-  `lxe` function L453 — `function lxe()`
-  `Ir` function L458 — `function Ir()`
-  `H` function L604 — `function H()`
-  `J` function L604 — `function J(Ge)`
-  `APe` function L609 — `function APe(t)`
-  `DD` function L609 — `function DD(t,e,r)`
-  `HPe` function L609 — `function HPe(t="",e="")`
-  `L` function L609 — `function L()`
-  `LPe` function L609 — `function LPe(t)`
-  `MPe` function L609 — `function MPe(t)`
-  `ND` function L609 — `function ND(t,e)`
-  `RPe` function L609 — `function RPe(t,e,r,n)`
-  `_Pe` function L609 — `function _Pe(t)`
-  `cae` function L609 — `function cae(t,e=0)`
-  `n` function L609 — `function n(i,a,s,l)`
-  `xae` function L609 — `function xae(t="")`
-  `$N` function L654 — `function $N(t,e)`
-  `$m` function L654 — `function $m(t)`
-  `AN` function L654 — `function AN(t)`
-  `BD` function L654 — `function BD(t,e)`
-  `CT` function L654 — `function CT(t)`
-  `DN` function L654 — `function DN(t)`
-  `Dae` function L654 — `function Dae(t,e)`
-  `ET` function L654 — `function ET(t)`
-  `FD` function L654 — `function FD(t,e)`
-  `FN` function L654 — `function FN(t)`
-  `Fi` function L654 — `function Fi(t)`
-  `GN` function L654 — `function GN(t,e)`
-  `Gae` function L654 — `function Gae(t)`
-  `Gv` function L654 — `function Gv(t)`
-  `Hd` function L654 — `function Hd(t)`
-  `Hm` function L654 — `function Hm(t,e)`
-  `Hv` function L654 — `function Hv(t)`
-  `ID` function L654 — `function ID(t,e)`
-  `Iae` function L654 — `function Iae(t)`
-  `Il` function L654 — `function Il(t)`
-  `JPe` function L654 — `function JPe(t,e=!0)`
-  `Jh` function L654 — `function Jh(t)`
-  `KPe` function L654 — `function KPe(t)`
-  `Lae` function L654 — `function Lae(t)`
-  `MD` function L654 — `function MD(t)`
-  `Ma` function L654 — `function Ma(t)`
-  `Mae` function L654 — `function Mae(t,e=!0)`
-  `Nae` function L654 — `function Nae(t,e)`
-  `OD` function L654 — `function OD(t,e)`
-  `Ol` function L654 — `function Ol(t)`
-  `PD` function L654 — `function PD(t,e)`
-  `PN` function L654 — `function PN(t)`
-  `QPe` function L654 — `function QPe(t)`
-  `RN` function L654 — `function RN(t)`
-  `Rae` function L654 — `function Rae(t,e,r)`
-  `Ru` function L654 — `function Ru(t)`
-  `SN` function L654 — `function SN(t)`
-  `ST` function L654 — `function ST(t)`
-  `TT` function L654 — `function TT(t)`
-  `UD` function L654 — `function UD(t)`
-  `Ud` function L654 — `function Ud(t)`
-  `Um` function L654 — `function Um(t)`
-  `VN` function L654 — `function VN(t)`
-  `Vd` function L654 — `function Vd(t)`
-  `WD` function L654 — `function WD(t)`
-  `Wv` function L654 — `function Wv(t)`
-  `Xd` function L654 — `function Xd(t)`
-  `Xo` function L654 — `function Xo(t)`
-  `Xv` function L654 — `function Xv()`
-  `Yd` function L654 — `function Yd(t)`
-  `ZPe` function L654 — `function ZPe(t,e,r=pT)`
-  `_T` function L654 — `function _T(t,e)`
-  `_c` function L654 — `function _c(t,e)`
-  `aBe` function L654 — `function aBe(t)`
-  `ar` function L654 — `function ar(t)`
-  `bT` function L654 — `function bT(t)`
-  `cBe` function L654 — `function cBe(t)`
-  `dBe` function L654 — `function dBe(t)`
-  `dN` function L654 — `function dN(t)`
-  `dT` function L654 — `function dT(t)`
-  `e` function L654 — `function e(a)`
-  `eBe` function L654 — `function eBe(t)`
-  `eN` function L654 — `function eN(t)`
-  `ef` function L654 — `function ef(t)`
-  `ei` function L654 — `function ei(t)`
-  `fBe` function L654 — `function fBe(t)`
-  `gBe` function L654 — `function gBe(t)`
-  `hBe` function L654 — `function hBe(t)`
-  `i` function L654 — `function i(a)`
-  `iBe` function L654 — `function iBe(t)`
-  `iN` function L654 — `function iN(t)`
-  `io` function L654 — `function io(t)`
-  `jD` function L654 — `function jD(t)`
-  `jo` function L654 — `function jo(t,e)`
-  `kT` function L654 — `function kT(t)`
-  `lBe` function L654 — `function lBe(t)`
-  `lN` function L654 — `function lN(t)`
-  `mBe` function L654 — `function mBe(t)`
-  `ma` function L654 — `function ma(t)`
-  `n` function L654 — `function n(a)`
-  `oBe` function L654 — `function oBe(t)`
-  `oN` function L654 — `function oN(t)`
-  `pBe` function L654 — `function pBe(t)`
-  `qD` function L654 — `function qD(t)`
-  `qd` function L654 — `function qd(t,e)`
-  `qo` function L654 — `function qo(t)`
-  `qv` function L654 — `function qv(t,e)`
-  `r` function L654 — `function r(a)`
-  `rBe` function L654 — `function rBe(t,e)`
-  `sBe` function L654 — `function sBe(t)`
-  `tBe` function L654 — `function tBe(t,e)`
-  `tf` function L654 — `function tf(t)`
-  `tn` function L654 — `function tn(...t)`
-  `uBe` function L654 — `function uBe(t)`
-  `vBe` function L654 — `function vBe(t,e)`
-  `vN` function L654 — `function vN(t)`
-  `wN` function L654 — `function wN(t)`
-  `wT` function L654 — `function wT(t)`
-  `xBe` function L654 — `function xBe(t,e=Fi(t).parseResult.value)`
-  `yBe` function L654 — `function yBe(t)`
-  `zN` function L654 — `function zN(t,e)`
-  `zae` function L654 — `function zae(t)`
-  `DT` function L662 — `function DT(t)`
-  `Qd` function L662 — `function Qd(t)`
-  `TBe` function L662 — `function TBe(t)`
-  `Wae` function L662 — `function Wae(t)`
-  `XN` function L662 — `function XN(t)`
-  `i` function L662 — `function i()`
-  `jN` function L662 — `function jN(t,e)`
-  `l` function L662 — `function l(h)`
-  `qN` function L662 — `function qN(t)`
-  `u` function L662 — `function u(h)`
-  `ABe` function L664 — `function ABe(t)`
-  `BBe` function L664 — `function BBe(t)`
-  `CBe` function L664 — `function CBe(t)`
-  `DBe` function L664 — `function DBe(t)`
-  `EBe` function L664 — `function EBe(t,e)`
-  `FBe` function L664 — `function FBe(t,e)`
-  `IBe` function L664 — `function IBe(t)`
-  `IT` function L664 — `function IT(t)`
-  `JN` function L664 — `function JN(t)`
-  `Jd` function L664 — `function Jd(t,e=[])`
-  `Jv` function L664 — `function Jv(t,e,r)`
-  `KN` function L664 — `function KN(t)`
-  `Kae` function L664 — `function Kae(t)`
-  `Km` function L664 — `function Km(t)`
-  `LBe` function L664 — `function LBe(t)`
-  `MBe` function L664 — `function MBe(t)`
-  `Ms` function L664 — `function Ms(t)`
-  `Mu` function L664 — `function Mu(t,e)`
-  `NBe` function L664 — `function NBe(t)`
-  `OBe` function L664 — `function OBe(t)`
-  `PBe` function L664 — `function PBe(t)`
-  `QN` function L664 — `function QN(t,e)`
-  `Qae` function L664 — `function Qae(t,e)`
-  `Qm` function L664 — `function Qm(t)`
-  `RBe` function L664 — `function RBe(t)`
-  `RT` function L664 — `function RT(t)`
-  `SBe` function L664 — `function SBe(t)`
-  `Xae` function L664 — `function Xae(t,e,r)`
-  `Xm` function L664 — `function Xm(t)`
-  `Yae` function L664 — `function Yae(t)`
-  `Ym` function L664 — `function Ym(t)`
-  `ZN` function L664 — `function ZN(t)`
-  `Zae` function L664 — `function Zae(t)`
-  `Zd` function L664 — `function Zd(t)`
-  `Zv` function L664 — `function Zv(t,e)`
-  `_Be` function L664 — `function _Be(t)`
-  `aR` function L664 — `function aR(t)`
-  `ase` function L664 — `function ase(t)`
-  `e` function L664 — `function e()`
-  `e0` function L664 — `function e0(t)`
-  `e2` function L664 — `function e2(t)`
-  `eR` function L664 — `function eR(t,e)`
-  `fR` function L664 — `function fR(t)`
-  `hR` function L664 — `function hR(t)`
-  `i` function L664 — `function i(a,s)`
-  `iR` function L664 — `function iR(t)`
-  `ise` function L664 — `function ise(t,e,r)`
-  `jae` function L664 — `function jae(t,e,r)`
-  `kBe` function L664 — `function kBe(t,e)`
-  `lR` function L664 — `function lR(t)`
-  `n` function L664 — `function n()`
-  `n2` function L664 — `function n2(t)`
-  `nR` function L664 — `function nR(t,e,r)`
-  `ose` function L664 — `function ose()`
-  `qae` function L664 — `function qae(t)`
-  `qm` function L664 — `function qm(t,e)`
-  `r2` function L664 — `function r2(t)`
-  `rR` function L664 — `function rR(t,e,r)`
-  `sR` function L664 — `function sR(t)`
-  `t2` function L664 — `function t2(t)`
-  `tR` function L664 — `function tR(t,e,r,n)`
-  `use` function L664-672 — `function use(t,e=!1)`
-  `$T` function L672 — `function $T(t,e)`
-  `GBe` function L672 — `function GBe(t,e)`
-  `GT` function L672 — `function GT(t,e,r)`
-  `gR` function L672 — `function gR(t,e,r)`
-  `lse` function L672 — `function lse(t,e)`
-  `yR` function L672 — `function yR(t)`
-  `pse` function L673-679 — `function pse(t,e)`
-  `$Be` function L679 — `function $Be(t)`
-  `UBe` function L679 — `function UBe(t)`
-  `VBe` function L679 — `function VBe(t)`
-  `WBe` function L679-681 — `function WBe(t)`
-  `e` class L679 — `-`
-  `mse` function L679 — `function mse(t,e)`
-  `XBe` function L681-683 — `function XBe(t)`
-  `YBe` function L681 — `function YBe(t)`
-  `e` class L681 — `-`
-  `JBe` function L683-685 — `function JBe(t)`
-  `KBe` function L683 — `function KBe(t)`
-  `QBe` function L683 — `function QBe(t)`
-  `ZBe` function L683 — `function ZBe(t,e)`
-  `jBe` function L683 — `function jBe(t)`
-  `dse` function L685 — `function dse(t)`
-  `eFe` function L685 — `function eFe(t,e)`
-  `fse` function L685 — `function fse(t)`
-  `gse` function L685-690 — `function gse(t,e,r)`
-  `tFe` function L685 — `function tFe(t)`
-  `yse` function L690-694 — `function yse(t,e,r)`
-  `nFe` function L694-699 — `function nFe(t,e)`
-  `rFe` function L694 — `function rFe(t)`
-  `vse` function L694 — `function vse(t)`
-  `wse` function L694 — `function wse(t,e)`
-  `xse` function L694 — `function xse(t)`
-  `Ase` function L699 — `function Ase(t)`
-  `Cse` function L699 — `function Cse(t,e)`
-  `Dc` function L699 — `function Dc(t)`
-  `Ese` function L699 — `function Ese(t)`
-  `Ou` function L699 — `function Ou(t,e)`
-  `Pu` function L699 — `function Pu(t)`
-  `TR` function L699 — `function TR(t)`
-  `Tse` function L699 — `function Tse(t)`
-  `aFe` function L699 — `function aFe(t)`
-  `bR` function L699 — `function bR(t,e,r)`
-  `cFe` function L699 — `function cFe(t)`
-  `eg` function L699 — `function eg(t,e)`
-  `iFe` function L699 — `function iFe()`
-  `lFe` function L699 — `function lFe(t)`
-  `oFe` function L699 — `function oFe(t)`
-  `sFe` function L699 — `function sFe(t)`
-  `uFe` function L699 — `function uFe(t)`
-  `Bu` function L708 — `function Bu(t)`
-  `HT` function L708 — `function HT(t)`
-  `K` function L708 — `function K(ee)`
-  `SR` function L708 — `function SR(t)`
-  `W` function L708 — `function W(ee)`
-  `fFe` function L708-709 — `function fFe(t)`
-  `z` function L708 — `function z()`
-  `n0` function L709 — `function n0(t,e,r,n,i,a,s,l)`
-  `o2` function L709 — `function o2(t,e)`
-  `r` function L715 — `function r(f)`
-  `$R` function L739 — `function $R(t,e)`
-  `$se` function L739 — `function $se(t,e,r,n,i,a)`
-  `Bse` function L739 — `function Bse(t,e)`
-  `Gse` function L739 — `function Gse(t,e,r,n,i,a)`
-  `Hse` function L739 — `function Hse(t,e)`
-  `JT` function L739 — `function JT(t)`
-  `Jse` function L739 — `function Jse(t,e,r)`
-  `Kse` function L739 — `function Kse(t,e,r)`
-  `QT` function L739 — `function QT(t)`
-  `Qse` function L739 — `function Qse(t,e)`
-  `RR` function L739 — `function RR(t,e,r,n=[])`
-  `SFe` function L739 — `function SFe(t,e)`
-  `TFe` function L739 — `function TFe(t,e,r,n,i,a,s)`
-  `UR` function L739 — `function UR(t,e)`
-  `Use` function L739 — `function Use(t,e,r)`
-  `VR` function L739 — `function VR(t,e)`
-  `Vse` function L739 — `function Vse(t,e,r,n)`
-  `Wse` function L739 — `function Wse(t,e)`
-  `XT` function L739 — `function XT(t,e,r=[])`
-  `Xse` function L739 — `function Xse(t,e,r,n)`
-  `Yse` function L739 — `function Yse(t)`
-  `ZT` function L739 — `function ZT(t,e)`
-  `Zse` function L739 — `function Zse(t,e,r)`
-  `_R` function L739 — `function _R(t)`
-  `a` function L739 — `function a(l)`
-  `bFe` function L739 — `function bFe(t,e,r,n)`
-  `coe` function L739 — `function coe(t,e,r)`
-  `dFe` function L739 — `function dFe(t,e,r,n)`
-  `doe` function L739-743 — `function doe(t,e)`
-  `eoe` function L739 — `function eoe(t)`
-  `gFe` function L739 — `function gFe(t)`
-  `h2` function L739 — `function h2(t)`
-  `ig` function L739 — `function ig(t,e,r,n)`
-  `jT` function L739 — `function jT(t,e,r,n)`
-  `jse` function L739 — `function jse(t)`
-  `kFe` function L739 — `function kFe(t)`
-  `mFe` function L739 — `function mFe(t,e)`
-  `ng` function L739 — `function ng(t,e,r,n)`
-  `pFe` function L739 — `function pFe(t,e,r)`
-  `qse` function L739 — `function qse(t)`
-  `rf` function L739 — `function rf(t)`
-  `s` function L739 — `function s(l)`
-  `tk` function L739 — `function tk(t,e,r)`
-  `toe` function L739 — `function toe(t)`
-  `uoe` function L739 — `function uoe(t,e,r)`
-  `vFe` function L739 — `function vFe(t,e)`
-  `wFe` function L739 — `function wFe(t,e,r)`
-  `xFe` function L739 — `function xFe(t,e,r,n)`
-  `yFe` function L739 — `function yFe(t,e,r,n)`
-  `zse` function L739 — `function zse(t)`
-  `AFe` function L743 — `function AFe(t,e)`
-  `CFe` function L743 — `function CFe(t,e)`
-  `poe` function L743 — `function poe(t,e,r)`
-  `DFe` function L750 — `function DFe(t,e)`
-  `Eoe` function L750 — `function Eoe(t)`
-  `mk` function L750-751 — `function mk(t)`
-  `v2` function L750 — `function v2(t,e,r,n=!1)`
-  `Loe` function L757 — `function Loe(t,e)`
-  `vk` function L757 — `function vk(t=void 0)`
-  `$Fe` function L763 — `function $Fe(t,e,r)`
-  `$oe` function L763 — `function $oe(t,e,r,n,i)`
-  `Ai` function L763 — `function Ai(t,e)`
-  `BFe` function L763 — `function BFe(t,e)`
-  `FFe` function L763 — `function FFe(t,e,r)`
-  `Foe` function L763 — `function Foe(t)`
-  `GFe` function L763 — `function GFe(t,e,r)`
-  `Goe` function L763 — `function Goe(t,e,r,n,i)`
-  `HFe` function L763 — `function HFe(t,e,r,n)`
-  `JFe` function L763 — `function JFe(t,e,r,n,i,a)`
-  `KFe` function L763 — `function KFe(t,e)`
-  `QFe` function L763 — `function QFe(t)`
-  `UFe` function L763 — `function UFe(t,e,r)`
-  `VFe` function L763 — `function VFe(t,e,r)`
-  `WFe` function L763 — `function WFe(t)`
-  `Woe` function L763 — `function Woe(t,e=!0)`
-  `XFe` function L763 — `function XFe(t,e,r)`
-  `XR` function L763 — `function XR(t,e=!0)`
-  `YFe` function L763 — `function YFe(t,e)`
-  `YR` function L763 — `function YR(t,e,r,n)`
-  `ZFe` function L763 — `function ZFe(t,e,r,n)`
-  `af` function L763 — `function af(t,e)`
-  `dg` function L763 — `function dg(t,e,r,n,...i)`
-  `eze` function L763 — `function eze(t,e,r,n)`
-  `jFe` function L763 — `function jFe(t,e)`
-  `jR` function L763 — `function jR(t,e,r,n)`
-  `l0` function L763 — `function l0(t,e,r)`
-  `na` function L763 — `function na(t,e,r,n)`
-  `o0` function L763 — `function o0(t,e,r)`
-  `qFe` function L763 — `function qFe(t,e,r)`
-  `qR` function L763 — `function qR(t,e)`
-  `tze` function L763-766 — `function tze(t)`
-  `zFe` function L763 — `function zFe(t,e,r)`
-  `zoe` function L763 — `function zoe(t,e,r)`
-  `Xoe` function L766 — `function Xoe(t,e)`
-  `Yoe` function L766 — `function Yoe(t,e,r,n)`
-  `aze` function L766 — `function aze(t,e,r)`
-  `bk` function L766 — `function bk(t,e)`
-  `cze` function L766 — `function cze(t,e)`
-  `dze` function L766 — `function dze(t)`
-  `e` function L766 — `function e(r)`
-  `fze` function L766 — `function fze(t)`
-  `hze` function L766 — `function hze(t)`
-  `i` function L766 — `function i(a)`
-  `ize` function L766 — `function ize(t,e)`
-  `lze` function L766 — `function lze(t)`
-  `mze` function L766 — `function mze(t)`
-  `n` function L766 — `function n(a)`
-  `nze` function L766 — `function nze(t,e,r)`
-  `oze` function L766 — `function oze(t,e)`
-  `pze` function L766 — `function pze(t)`
-  `qoe` function L766 — `function qoe(t)`
-  `r` function L766 — `function r(n)`
-  `rze` function L766 — `function rze(t)`
-  `sze` function L766 — `function sze(t,e)`
-  `uze` function L766 — `function uze(t)`
-  `Dk` function L768 — `function Dk(t,e,r)`
-  `Eze` function L768 — `function Eze(t,e)`
-  `Gu` function L768 — `function Gu(t)`
-  `Lk` function L768 — `function Lk(t)`
-  `Sze` function L768 — `function Sze(t,e)`
-  `Tze` function L768 — `function Tze(t,e)`
-  `a` function L768 — `function a(m)`
-  `bze` function L768 — `function bze(t,e)`
-  `d` function L768 — `function d(m)`
-  `e` function L768 — `function e(h)`
-  `ece` function L768 — `function ece(t,e,r=e.terminal)`
-  `f` function L768 — `function f(m)`
-  `fM` function L768 — `function fM(t)`
-  `gM` function L768 — `function gM(t)`
-  `h` function L768 — `function h(m)`
-  `h0` function L768 — `function h0(t,e,r=!1)`
-  `i` function L768 — `function i(m)`
-  `kze` function L768 — `function kze(t,e)`
-  `l` function L768 — `function l(m,g,y)`
-  `n` function L768 — `function n(m)`
-  `nce` function L768 — `function nce(t)`
-  `p` function L768 — `function p(m,g)`
-  `pM` function L768 — `function pM(t,e)`
-  `r` function L768 — `function r(m)`
-  `rce` function L768 — `function rce(t,e)`
-  `s` function L768 — `function s(m)`
-  `tce` function L768 — `function tce(t,e,r,n)`
-  `u` function L768 — `function u(m)`
-  `vM` function L768 — `function vM(t)`
-  `vze` function L768 — `function vze(t,e)`
-  `wze` function L768 — `function wze(t,e)`
-  `xze` function L768 — `function xze(t,e)`
-  `yze` function L768 — `function yze(t,e)`
-  `Aze` function L769 — `function Aze(t)`
-  `Cce` function L769 — `function Cce(t)`
-  `Cze` function L769 — `function Cze(t)`
-  `Dze` function L769 — `function Dze(t)`
-  `E` function L769 — `function E(D)`
-  `Ece` function L769 — `function Ece(t)`
-  `Fk` function L769 — `function Fk(t)`
-  `Fze` function L769 — `function Fze(t)`
-  `GM` function L769 — `function GM(t)`
-  `Gi` function L769 — `function Gi(t)`
-  `Hze` function L769 — `function Hze(t,e)`
-  `JM` function L769 — `function JM(t,e)`
-  `KM` function L769 — `function KM(t)`
-  `Lze` function L769 — `function Lze(t)`
-  `NM` function L769 — `function NM()`
-  `Oze` function L769 — `function Oze(t)`
-  `QM` function L769 — `function QM(t)`
-  `RM` function L769 — `function RM(t,e)`
-  `Sce` function L769 — `function Sce(t,e)`
-  `Tce` function L769-773 — `function Tce(t)`
-  `Uze` function L769 — `function Uze(t)`
-  `Vze` function L769 — `function Vze(t)`
-  `Wze` function L769 — `function Wze(t,e)`
-  `Xze` function L769 — `function Xze(t,e)`
-  `Yze` function L769 — `function Yze(t)`
-  `_` function L769 — `function _(D)`
-  `_ze` function L769 — `function _ze(t)`
-  `a` function L769 — `function a(h)`
-  `ace` function L769 — `function ace(t)`
-  `b` function L769 — `function b(D,k)`
-  `bce` function L769 — `function bce(t)`
-  `dce` function L769 — `function dce(t)`
-  `e` function L769 — `function e(r)`
-  `eI` function L769 — `function eI(t,e)`
-  `fce` function L769 — `function fce(t,e,r=0)`
-  `g` class L769 — `-`
-  `hce` function L769 — `function hce(t)`
-  `i` function L769 — `function i(h)`
-  `ice` function L769 — `function ice(t)`
-  `kM` function L769 — `function kM()`
-  `kce` function L769 — `function kce(t)`
-  `l` function L769 — `function l(h)`
-  `n` function L769 — `function n(h)`
-  `nI` function L769 — `function nI(t)`
-  `p` class L769 — `-`
-  `pce` function L769 — `function pce(t)`
-  `qze` function L769 — `function qze(t,e,r)`
-  `r` function L769 — `function r(i,a,s)`
-  `rI` function L769 — `function rI(t,e)`
-  `s` function L769 — `function s(h)`
-  `sf` function L769 — `function sf(t)`
-  `tI` function L769 — `function tI(t,e,r)`
-  `u` function L769 — `function u(h)`
-  `uce` function L769 — `function uce()`
-  `v` function L769 — `function v(D,k,R)`
-  `vce` function L769 — `function vce(t)`
-  `w` function L769 — `function w(D,k)`
-  `x` function L769 — `function x(D)`
-  `xce` function L769 — `function xce(t)`
-  `yce` function L769 — `function yce(t)`
-  `zk` function L769 — `function zk(t)`
-  `zze` function L769 — `function zze(t,e,r,n)`
-  `$i` function L777 — `function $i(t,e,r,n,i,a,s,l,u)`
-  `Dce` function L777 — `function Dce(t,e)`
-  `Jce` function L777 — `function Jce(t)`
-  `Kce` function L777 — `function Kce(t)`
-  `Lce` function L777 — `function Lce(t)`
-  `Qce` function L777 — `function Qce(t)`
-  `Qze` function L777 — `function Qze()`
-  `Vk` function L777 — `function Vk(t,e)`
-  `Zce` function L777 — `function Zce(t)`
-  `_ce` function L777 — `function _ce(t,e,r,n)`
-  `aue` function L777 — `function aue(t)`
-  `co` function L777 — `function co(t)`
-  `cue` function L777 — `function cue(t)`
-  `eue` function L777 — `function eue(t)`
-  `hue` function L777 — `function hue(t)`
-  `lo` function L777 — `function lo(t)`
-  `nue` function L777 — `function nue(t)`
-  `of` function L777 — `function of(t)`
-  `oue` function L777 — `function oue(t)`
-  `rue` function L777 — `function rue(t)`
-  `Gl` function L778 — `function Gl(t,e)`
-  `Jk` function L778 — `function Jk(t=uo)`
-  `Qk` function L778 — `function Qk(t=uo)`
-  `Wk` function L778 — `function Wk(t=uo)`
-  `jk` function L778 — `function jk(t=uo)`
-  `qk` function L778 — `function qk(t=uo)`
-  `RI` function L780 — `function RI()`
-  `Sue` function L780 — `function Sue(t)`
-  `_Ge` function L780 — `function _Ge(t,e)`
-  `kue` function L780 — `function kue(t,e,r)`
-  `lf` function L780 — `function lf(t,e)`
-  `ce` function L844 — `function ce()`
-  `ue` function L844 — `function ue(he)`
-  `E` function L849 — `function E(C,A,I,D)`
-  `L` function L849 — `function L(C)`
-  `T` function L849 — `function T(C,A)`
-  `_` function L849 — `function _(C,A,I,D)`
-  `b` function L849 — `function b(C,A,I,D,k,R,S)`
-  `k` function L849 — `function k()`
-  `m` function L849 — `function m(g)`
-  `she` function L849 — `function she(t,e,r)`
-  `v` function L849 — `function v(C,A)`
-  `w` function L849 — `function w(C,A,I,D,k,R,S,O)`
-  `x` function L849 — `function x(C,A,I)`
-  `Xe` function L1130 — `function Xe()`
-  `dt` function L1130 — `function dt(Se)`
-  `$Ve` function L1135 — `function $Ve(t,e,r,n,i)`
-  `$he` function L1135 — `function $he(t)`
-  `BVe` function L1135 — `function BVe(t)`
-  `FVe` function L1135 — `function FVe(t)`
-  `GVe` function L1135 — `function GVe(t)`
-  `HVe` function L1135 — `function HVe(t)`
-  `IVe` function L1135 — `function IVe(t)`
-  `MVe` function L1135 — `function MVe(t)`
-  `OVe` function L1135 — `function OVe(t)`
-  `PVe` function L1135 — `function PVe(t)`
-  `U` function L1135 — `function U()`
-  `UVe` function L1135 — `function UVe(t)`
-  `VVe` function L1135 — `function VVe(t,e)`
-  `Vhe` function L1135 — `function Vhe(t)`
-  `WVe` function L1135 — `function WVe()`
-  `Wu` function L1135 — `function Wu(t)`
-  `a` function L1135 — `function a(C)`
-  `aO` function L1135 — `function aO(t)`
-  `i` function L1135 — `function i(C)`
-  `iO` function L1135 — `function iO(t)`
-  `s` function L1135 — `function s(C)`
-  `zVe` function L1135 — `function zVe(t)`
-  `De` function L1136 — `function De()`
-  `te` function L1136 — `function te(ne)`
-  `Ig` function L1141 — `function Ig(t)`
-  `JVe` function L1141 — `function JVe(t)`
-  `S` function L1141 — `function S()`
-  `ZVe` function L1141 — `function ZVe(t)`
-  `aUe` function L1141 — `function aUe(t,e)`
-  `cUe` function L1141 — `function cUe()`
-  `dE` function L1141 — `function dE(t)`
-  `dfe` function L1141 — `function dfe()`
-  `eUe` function L1141 — `function eUe(t)`
-  `ffe` function L1141 — `function ffe()`
-  `gfe` function L1141 — `function gfe(t)`
-  `h` function L1141 — `function h(v)`
-  `iUe` function L1141 — `function iUe(t)`
-  `l` function L1141 — `function l(v)`
-  `lUe` function L1141 — `function lUe()`
-  `mO` function L1141 — `function mO(t)`
-  `mfe` function L1141 — `function mfe(t,e)`
-  `nUe` function L1141 — `function nUe(t,e)`
-  `nfe` function L1141 — `function nfe(t,e,r,n)`
-  `oO` function L1141 — `function oO(t)`
-  `oUe` function L1141 — `function oUe()`
-  `ofe` function L1141 — `function ofe(t,e,r)`
-  `pfe` function L1141 — `function pfe()`
-  `rUe` function L1141 — `function rUe(t)`
-  `sUe` function L1141 — `function sUe(t,e)`
-  `tUe` function L1141 — `function tUe(t)`
-  `u` function L1141 — `function u(v)`
-  `uO` function L1141 — `function uO(t,e,r,n)`
-  `yfe` function L1141 — `function yfe(t)`
-  `X` function L1142 — `function X(We)`
-  `fe` function L1142 — `function fe()`
-  `y` function L1142 — `function y(v)`
-  `K` function L1147 — `function K()`
-  `ne` function L1193 — `function ne(Et)`
-  `ye` function L1193 — `function ye()`
-  `J` function L1198 — `function J()`
-  `TO` function L1198 — `function TO(t,e)`
-  `dHe` function L1198 — `function dHe()`
-  `jfe` function L1198 — `function jfe(t,e)`
-  `$He` function L1314 — `function $He(t,e,r)`
-  `PHe` function L1314 — `function PHe(t,e)`
-  `T` function L1314 — `function T(F,B)`
-  `Tt` function L1314 — `function Tt()`
-  `UHe` function L1314 — `function UHe(t,e,r)`
-  `Ye` function L1314 — `function Ye(or)`
-  `e` function L1314 — `function e(a,s,l,u,h,f,d,p)`
-  `i` function L1314 — `function i(a,s)`
-  `l` function L1314 — `function l(h,f)`
-  `n` function L1314 — `function n(a,s,l,u,h,f,d,p)`
-  `r` function L1314 — `function r(i,a,s,l,u)`
-  `s` function L1314 — `function s(l)`
-  `t` function L1314 — `function t(a,s,l,u,h,f,d)`
-  `u` function L1314 — `function u(h,f)`
-  `zHe` function L1314 — `function zHe(t,e,r,n,i,a,s)`
-  `zc` function L1314 — `function zc(t,e,r,n,i)`
-  `Tde` function L1319 — `function Tde(t)`
-  `xe` function L1319 — `function xe()`
-  `Fe` function L1477 — `function Fe()`
-  `ke` function L1477 — `function ke(Te)`
-  `$We` function L1482 — `function $We(t)`
-  `OE` function L1482 — `function OE(t,e,r)`
-  `P` function L1482 — `function P()`
-  `QO` function L1482 — `function QO(t="",e=0,r="",n=ME)`
-  `VWe` function L1482 — `function VWe(t)`
-  `eP` function L1482 — `function eP(t="")`
-  `eYe` function L1482 — `function eYe(t="")`
-  `rYe` function L1482 — `function rYe(t,e,r)`
-  `s0e` function L1482 — `function s0e()`
-  `tP` function L1482 — `function tP(t="",e=_0)`
-  `tYe` function L1482 — `function tYe(t="",e=_0)`
-  `N` function L1696 — `function N()`
-  `O` function L1696 — `function O(H)`
-  `f` function L1701 — `function f()`
-  `N` function L1834 — `function N(q)`
-  `P` function L1834 — `function P()`
-  `ZYe` function L1834 — `function ZYe(t)`
-  `a` function L1834 — `function a(u)`
-  `e` function L1834 — `function e(i,a,s,l,u,h,f,d,p)`
-  `l` function L1834 — `function l(u)`
-  `n` function L1834 — `function n(i,a)`
-  `r` function L1834 — `function r(i,a,s,l,u)`
-  `s` function L1834 — `function s(u)`
-  `t` function L1834 — `function t(i,a,s,l,u,h,f,d)`
-  `a` function L1839 — `function a(u)`
-  `d` function L1839 — `function d()`
-  `e` function L1839 — `function e(i,a,s,l,u,h,f,d,p)`
-  `l` function L1839 — `function l(u)`
-  `n` function L1839 — `function n(i,a)`
-  `ppe` function L1839 — `function ppe(t,e)`
-  `r` function L1839 — `function r(i,a,s,l,u)`
-  `s` function L1839 — `function s(u)`
-  `t` function L1839 — `function t(i,a,s,l,u,h,f,d)`
-  `K` function L1894 — `function K(ke)`
-  `ie` function L1894 — `function ie()`
-  `Dpe` function L1899 — `function Dpe(t,e)`
-  `Hme` function L1899 — `function Hme(t,e,r)`
-  `Hqe` function L1899-1900 — `function Hqe()`
-  `JP` function L1899 — `function JP(t,e)`
-  `Npe` function L1899 — `function Npe(t,e)`
-  `Ul` function L1899 — `function Ul(t,e)`
-  `Uqe` function L1899 — `function Uqe(t,e)`
-  `Vqe` function L1899 — `function Vqe(t)`
-  `Wme` function L1899 — `function Wme(t,e)`
-  `Yi` function L1899 — `function Yi(t)`
-  `eB` function L1899 — `function eB(t,e,r)`
-  `w` function L1899 — `function w()`
-  `Yme` function L1900-1901 — `function Yme(t,e)`
-  `$Qe` function L1901 — `function $Qe(t)`
-  `$Ze` function L1901 — `function $Ze(t,e,r)`
-  `A1e` function L1901 — `function A1e(t,e)`
-  `Aet` function L1901 — `function Aet(t,e,r)`
-  `B6` function L1901 — `function B6(t,e)`
-  `BZe` function L1901 — `function BZe(t,e)`
-  `C1e` function L1901 — `function C1e(t,e,r)`
-  `CZe` function L1901 — `function CZe(t,e,r)`
-  `D6` function L1901 — `function D6(t,e,r,n,i,a)`
-  `DP` function L1901 — `function DP(t)`
-  `DXe` function L1901 — `function DXe(t)`
-  `Dge` function L1901 — `function Dge(t)`
-  `EQe` function L1901 — `function EQe(t)`
-  `EZe` function L1901 — `function EZe(t,e)`
-  `FXe` function L1901 — `function FXe(t)`
-  `G6` function L1901 — `function G6(t)`
-  `GJe` function L1901 — `function GJe(t,e,r,n)`
-  `Gme` function L1901 — `function Gme(t,e,r)`
-  `IXe` function L1901 — `function IXe(t)`
-  `Ige` function L1901 — `function Ige(t,e,r)`
-  `Jge` function L1901 — `function Jge(t)`
-  `KQe` function L1901 — `function KQe()`
-  `L` function L1901 — `function L()`
-  `LZe` function L1901 — `function LZe(t,e,r)`
-  `MQe` function L1901 — `function MQe()`
-  `MZe` function L1901 — `function MZe(t,e,r)`
-  `Mx` function L1901 — `function Mx(t,e)`
-  `NQe` function L1901 — `function NQe(t,e)`
-  `O6` function L1901 — `function O6()`
-  `OQe` function L1901 — `function OQe(t,e)`
-  `Oge` function L1901 — `function Oge(t,e,r)`
-  `P6` function L1901 — `function P6(t,e)`
-  `PP` function L1901 — `function PP(t,e,r,n,i)`
-  `PQe` function L1901 — `function PQe(t,e)`
-  `QXe` function L1901 — `function QXe(t,e,r)`
-  `Qg` function L1901 — `function Qg(t,e)`
-  `Qge` function L1901 — `function Qge(t)`
-  `RP` function L1901 — `function RP(t,e)`
-  `Rx` function L1901 — `function Rx()`
-  `SXe` function L1901 — `function SXe(t)`
-  `T` function L1901 — `function T()`
-  `TXe` function L1901 — `function TXe(t)`
-  `Tme` function L1901 — `function Tme(t,e)`
-  `UJe` function L1901 — `function UJe(t,e,r,n)`
-  `UKe` function L1901 — `function UKe(t,e)`
-  `UQe` function L1901 — `function UQe(t)`
-  `UZe` function L1901 — `function UZe(t,e)`
-  `Ume` function L1901 — `function Ume(t)`
-  `VJe` function L1901 — `function VJe(t,e,r,n)`
-  `VXe` function L1901 — `function VXe(t)`
-  `WQe` function L1901 — `function WQe(t,e)`
-  `WZe` function L1901 — `function WZe(t)`
-  `ZKe` function L1901 — `function ZKe(t)`
-  `ZQe` function L1901 — `function ZQe(t)`
-  `Zg` function L1901 — `function Zg(t,e,r,n,i)`
-  `Zge` function L1901 — `function Zge(t)`
-  `_` function L1901 — `function _()`
-  `_Qe` function L1901 — `function _Qe(t)`
-  `a` function L1901 — `function a(f)`
-  `aZe` function L1901 — `function aZe(t)`
-  `att` function L1901 — `function att(t,e,r)`
-  `b` function L1901 — `function b(E)`
-  `bB` function L1901 — `function bB(t,e,r,n)`
-  `bZe` function L1901 — `function bZe(t)`
-  `bme` function L1901 — `function bme(t)`
-  `btt` function L1901 — `function btt(t,e,r,n)`
-  `dQe` function L1901 — `function dQe(t,e)`
-  `e` function L1901 — `function e(n,i,a)`
-  `e6` function L1901 — `function e6()`
-  `eZe` function L1901 — `function eZe(t,e)`
-  `f` function L1901 — `function f(g,y,v)`
-  `fZe` function L1901 — `function fZe(t)`
-  `g` function L1901 — `function g(E)`
-  `g1` function L1901 — `function g1(t)`
-  `hB` function L1901 — `function hB(t,e)`
-  `hQe` function L1901 — `function hQe(t)`
-  `i` function L1901 — `function i(f,d)`
-  `i1e` function L1901 — `function i1e(t)`
-  `itt` function L1901 — `function itt(t,e)`
-  `jKe` function L1901 — `function jKe(t)`
-  `jge` function L1901 — `function jge(t,e,r)`
-  `k1e` function L1901 — `function k1e(t)`
-  `kZe` function L1901 — `function kZe(t)`
-  `kme` function L1901 — `function kme(t,e)`
-  `l` function L1901 — `function l(f)`
-  `l1e` function L1901 — `function l1e(t)`
-  `m` function L1901 — `function m(E)`
-  `m1` function L1901 — `function m1(t)`
-  `mB` function L1901 — `function mB(t)`
-  `mQe` function L1901 — `function mQe(t,e)`
-  `mXe` function L1901 — `function mXe(t,e)`
-  `n` function L1901 — `function n(i)`
-  `n1e` function L1901 — `function n1e(t)`
-  `nZe` function L1901 — `function nZe(t)`
-  `o1e` function L1901 — `function o1e(t,e)`
-  `oZe` function L1901 — `function oZe(t,e)`
-  `ott` function L1901 — `function ott(t,e,r,n)`
-  `p` function L1901 — `function p(S,O)`
-  `p1` function L1901 — `function p1(t)`
-  `pB` function L1901 — `function pB(t,e,r,n)`
-  `pXe` function L1901 — `function pXe(t)`
-  `pme` function L1901 — `function pme(t)`
-  `ptt` function L1901 — `function ptt(t,e)`
-  `qXe` function L1901 — `function qXe(t)`
-  `r` function L1901 — `function r(s)`
-  `r1e` function L1901 — `function r1e(t)`
-  `rQe` function L1901 — `function rQe(t)`
-  `s` function L1901 — `function s(S)`
-  `stt` function L1901 — `function stt(t,e,r,n,i)`
-  `t` function L1901 — `function t()`
-  `t1e` function L1901 — `function t1e(t)`
-  `tZe` function L1901 — `function tZe(t)`
-  `u` function L1901 — `function u(S)`
-  `uJe` function L1901 — `function uJe(t,e,r)`
-  `v` function L1901 — `function v(E)`
-  `v1` function L1901 — `function v1(t)`
-  `vB` function L1901 — `function vB(t)`
-  `vme` function L1901 — `function vme(t)`
-  `w` function L1901 — `function w()`
-  `w1e` function L1901 — `function w1e(t,e,r)`
-  `wZe` function L1901 — `function wZe(t,e)`
-  `wme` function L1901 — `function wme(t,e,r,n,i)`
-  `x` function L1901 — `function x()`
-  `xQe` function L1901 — `function xQe(t)`
-  `xme` function L1901 — `function xme(t)`
-  `xtt` function L1901 — `function xtt(t,e,r,n,i)`
-  `y` function L1901 — `function y(E)`
-  `yQe` function L1901 — `function yQe()`
-  `yZe` function L1901 — `function yZe(t,e)`
-  `zQe` function L1901 — `function zQe(t)`
-  `zXe` function L1901 — `function zXe(t)`
-  `zZe` function L1901 — `function zZe(t,e,r,n)`
-  `D` function L1904 — `function D(ce,te,De,oe,ke)`
-  `_tt` function L1904 — `function _tt(t,e,r,n,i)`
-  `a` function L1904 — `function a(s)`
-  `h` function L1904 — `function h(d,p,m,g)`
-  `i` function L1904 — `function i()`
-  `k` function L1904 — `function k(ce,te)`
-  `l` function L1904 — `function l(u,h)`
-  `m` function L1904 — `function m(v)`
-  `n` function L1904 — `function n()`
-  `p` function L1904 — `function p(g,y,v)`
-  `r` function L1904 — `function r(n)`
-  `s` function L1904 — `function s(u,h,f)`
-  `t` function L1904 — `function t(e)`
-  `w` function L1904 — `function w()`
-  `I1e` function L1937 — `function I1e(t,e,r,n,i)`
-  `Mtt` function L1937 — `function Mtt(t,e)`
-  `Ntt` function L1937 — `function Ntt(t,e)`
-  `O1e` function L1937 — `function O1e(t,e,r,n)`
-  `Rtt` function L1937 — `function Rtt(t,e)`
-  `J` function L1993 — `function J()`
-  `ee` function L1993 — `function ee(Ve)`
-  `E` function L1998 — `function E()`
-  `D` function L2080 — `function D(ie)`
-  `k` function L2080 — `function k()`
-  `$B` function L2085 — `function $B(t)`
-  `A` function L2085 — `function A(O,N,P,F)`
-  `BB` function L2085 — `function BB(t)`
-  `C` function L2085 — `function C(O,N)`
-  `D` function L2085 — `function D({sourceLinks:O,targetLinks:N})`
-  `E` function L2085 — `function E(O,N,P)`
-  `E1` function L2085 — `function E1(t)`
-  `FB` function L2085 — `function FB(t,e)`
-  `HB` function L2085 — `function HB()`
-  `I` function L2085 — `function I(O,N,P,F)`
-  `L` function L2085 — `function L(O,N,P)`
-  `R` function L2085 — `function R(O,N)`
-  `S` function L2085 — `function S(O,N)`
-  `T` function L2085 — `function T(O)`
-  `T1` function L2085 — `function T1(t,e)`
-  `X6` function L2085 — `function X6(t)`
-  `Y6` function L2085 — `function Y6(t,e)`
-  `YB` function L2085 — `function YB()`
-  `_` function L2085 — `function _(O)`
-  `b` function L2085 — `function b({nodes:O})`
-  `brt` function L2085 — `function brt(t)`
-  `bye` function L2085 — `function bye(t)`
-  `drt` function L2085 — `function drt(t)`
-  `dye` function L2085 — `function dye({nodes:t})`
-  `fb` function L2085 — `function fb(t,e)`
-  `frt` function L2085 — `function frt(t)`
-  `fye` function L2085 — `function fye(t,e)`
-  `g` function L2085 — `function g()`
-  `grt` function L2085 — `function grt(t)`
-  `hb` function L2085 — `function hb(t,e)`
-  `hrt` function L2085 — `function hrt(t)`
-  `hye` function L2085 — `function hye(t,e)`
-  `j6` function L2085 — `function j6()`
-  `k` function L2085 — `function k(O)`
-  `k1` function L2085 — `function k1(t,e)`
-  `mye` function L2085 — `function mye()`
-  `prt` function L2085 — `function prt(t)`
-  `q6` function L2085 — `function q6()`
-  `s` function L2085 — `function s()`
-  `uye` function L2085 — `function uye(t,e)`
-  `v` function L2085 — `function v({nodes:O})`
-  `vrt` function L2085 — `function vrt(t)`
-  `w` function L2085 — `function w({nodes:O})`
-  `wrt` function L2085 — `function wrt(t)`
-  `x` function L2085 — `function x({nodes:O})`
-  `xrt` function L2085 — `function xrt(t,e,r,n,i)`
-  `xye` function L2085 — `function xye(t)`
-  `y` function L2085 — `function y({nodes:O,links:N})`
-  `yrt` function L2085 — `function yrt(t)`
-  `zB` function L2085 — `function zB(t)`
-  `K` function L2110 — `function K()`
-  `j` function L2110 — `function j(oe)`
-  `Krt` function L2115 — `function Krt(t)`
-  `Qrt` function L2115 — `function Qrt(t)`
-  `Zrt` function L2115 — `function Zrt(t)`
-  `b` function L2115 — `function b()`
-  `Ent` function L2227 — `function Ent(t)`
-  `J6` function L2227 — `function J6(t,e)`
-  `JB` function L2227 — `function JB(t,e,r=0,n=0)`
-  `Tnt` function L2227 — `function Tnt(t,e)`
-  `cve` function L2227 — `function cve(t)`
-  `hve` function L2227 — `function hve(t,e)`
-  `lve` function L2227 — `function lve(t,{minX:e,minY:r,maxX:n,maxY:i}={minX:0,minY:0,maxX:0,maxY:0})`
-  `ove` function L2227 — `function ove(t,e)`
-  `$ve` function L2230 — `function $ve(t,e,r=!1)`
-  `Cve` function L2230 — `function Cve(t,e)`
-  `Dnt` function L2230 — `function Dnt(t,e)`
-  `Hve` function L2230 — `function Hve(t,e,r,n,i)`
-  `Int` function L2230 — `function Int(t,e,r)`
-  `Li` function L2230 — `function Li(t)`
-  `Mnt` function L2230 — `function Mnt(t,e,r,n)`
-  `Nnt` function L2230 — `function Nnt(t,e,r,n)`
-  `Rnt` function L2230 — `function Rnt(t,e,r)`
-  `Uve` function L2230 — `function Uve(t,e,r)`
-  `Vve` function L2230 — `function Vve(t,e,r)`
-  `Xl` function L2230 — `function Xl(t,e,r,n)`
-  `_` function L2230 — `function _()`
-  `ait` function L2230 — `function ait(t,e,r)`
-  `iF` function L2230 — `function iF(t,e,r,n)`
-  `iit` function L2230 — `function iit(t,e,r)`
-  `sF` function L2230 — `function sF(t,e,r,n)`
-  `I` function L2261 — `function I()`
-  `Pe` function L2261 — `function Pe(vt,Ye)`
-  `a` function L2261 — `function a(s)`
-  `b` function L2261 — `function b(w,_)`
-  `d` function L2261 — `function d(p,m)`
-  `f` function L2261 — `function f(p,m,g,y)`
-  `h` function L2261 — `function h(d,p,m,g)`
-  `i` function L2261 — `function i()`
-  `l` function L2261 — `function l(u,h)`
-  `m` function L2261 — `function m()`
-  `n` function L2261 — `function n()`
-  `p` function L2261 — `function p(g,y,v)`
-  `r` function L2261 — `function r(n)`
-  `s` function L2261 — `function s(u,h,f)`
-  `u` function L2261 — `function u(g)`
-  `Dit` function L2265 — `function Dit(t,e)`
-  `Iit` function L2265 — `function Iit(t)`
-  `Lit` function L2265 — `function Lit(t,e)`
-  `Mit` function L2265 — `function Mit(t,e)`
-  `Nit` function L2265 — `function Nit(t,e)`
-  `Oit` function L2265 — `function Oit(t)`
-  `Pit` function L2265 — `function Pit(t,e,r,n,{spatialMaps:i})`
-  `Rit` function L2265 — `function Rit(t,e)`
-  `d` function L2265 — `function d(p,m,g,y)`
-  `$2e` function L2266 — `function $2e(t,e,r,n)`
-  `A1` function L2266 — `function A1(t,e)`
-  `A2e` function L2266 — `function A2e(t,e,r)`
-  `B2e` function L2266 — `function B2e(t)`
-  `D2e` function L2266 — `function D2e()`
-  `F2e` function L2266 — `function F2e(t,e,r,n,i,a,s,l,u,h,f,d)`
-  `G2e` function L2266 — `function G2e(t)`
-  `I2e` function L2266 — `function I2e(t)`
-  `If` function L2266 — `function If(t,e,r)`
-  `Ju` function L2266 — `function Ju()`
-  `K0` function L2266 — `function K0(t,e)`
-  `L1` function L2266 — `function L1(t)`
-  `M2e` function L2266 — `function M2e(t)`
-  `N2e` function L2266 — `function N2e()`
-  `O2e` function L2266 — `function O2e(t,e)`
-  `P2e` function L2266 — `function P2e(t,e)`
-  `R2e` function L2266 — `function R2e(t)`
-  `Tb` function L2266 — `function Tb(t,e,r)`
-  `U2e` function L2266 — `function U2e(t,e,r,n)`
-  `V2e` function L2266 — `function V2e(t,e)`
-  `Vit` function L2266 — `function Vit(t,e,r,n)`
-  `_2e` function L2266 — `function _2e(t)`
-  `cl` function L2266 — `function cl()`
-  `dS` function L2266 — `function dS(t,e,r,n,i,a,s,l,u)`
-  `fS` function L2266 — `function fS(t)`
-  `hS` function L2266 — `function hS(t,e)`
-  `kb` function L2266 — `function kb()`
-  `lS` function L2266 — `function lS(t)`
-  `pS` function L2266 — `function pS(t,e)`
-  `uS` function L2266 — `function uS(t,e,r,n,i,a,s,l)`
-  `yF` function L2266 — `function yF(t)`
-  `yo` function L2266 — `function yo(t)`
-  `z2e` function L2266 — `function z2e(t,e,r,n,i)`
-  `q2e` function L2267 — `function q2e(t)`
-  `X2e` function L2268 — `function X2e(t)`
-  `Z2e` function L2268 — `function Z2e(t)`
-  `lat` function L2268 — `function lat(t,e)`
-  `vF` function L2268 — `function vF(t)`
-  `Q2e` function L2274 — `function Q2e(t,e)`
-  `gat` function L2274 — `function gat(t,e,r,n)`
-  `mat` function L2274 — `function mat(t={})`
...
#### e2e/tests/related-work.spec.ts

-  `panel` function L19-22 — `const panel = (page: Page, title: string)`
...
-  `field` function L109 — `const field = (name: string)`
...
#### scripts/corpus/analyze.rs

-  `DIM` variable L7 — `: usize` — numbers are exact rather than sampled.
-  `Meta` struct L10 — `{ project: String, level: String, short_code: String, parent: String, title: Str...` — numbers are exact rather than sampled.
-  `pct` function L12-16 — `(v: &mut Vec<f32>, p: f64) -> f32` — numbers are exact rather than sampled.
-  `describe` function L18-24 — `(name: &str, v: &mut Vec<f32>)` — numbers are exact rather than sampled.
-  `main` function L26-114 — `() -> Result<(), Box<dyn std::error::Error>>` — numbers are exact rather than sampled.

#### scripts/corpus/embed.rs

-  `OPENING_PROSE` variable L7 — `: usize` — does the real cosine distribution look like across 19 Metis corpora (T-0190)?
-  `Doc` struct L9-16 — `{ project: String, level: String, short_code: String, title: String, parent: Str...` — does the real cosine distribution look like across 19 Metis corpora (T-0190)?
-  `main` function L18-97 — `() -> Result<(), Box<dyn std::error::Error>>` — does the real cosine distribution look like across 19 Metis corpora (T-0190)?

#### scripts/corpus/extract.py

- pub `frontmatter` function L14-25 — `def frontmatter(text)`
- pub `window` function L27-34 — `def window(s, start_off)`
- pub `chunk` function L36-58 — `def chunk(body)` — Heading boundaries as anchors.

#### scripts/corpus/prose-budget.rs

-  `norm_title` function L19-27 — `(t: &str) -> String` — precision was measured at roughly half? (lower is better)
-  `main` function L29-190 — `() -> Result<(), Box<dyn std::error::Error>>` — precision was measured at roughly half? (lower is better)
-  `Doc` struct L44-51 — `{ project: String, level: String, short_code: String, title: String, parent_titl...` — precision was measured at roughly half? (lower is better)

#### scripts/corpus/stats.py

- pub `pct` function L4-5 — `def pct(v, p)`
...
- pub `fail` function L100-102 — `def fail(message)`
- pub `load_spec` function L105-120 — `def load_spec()`
- pub `anchor` function L123-132 — `def anchor(text)` — mdBook's heading anchor: lowercase, non-alphanumerics to hyphens.
- pub `type_of` function L135-166 — `def type_of(schema, spec)` — A short, human type for a schema node.
...
-  `allJourneyIds` function L32-38 — `function allJourneyIds(): string[]`
...
#### uat/journeys/local-login.journey.ts

-  `password` function L32 — `const password = (label: string)`
-  `login` function L35-44 — `function login(email: string, pw: string): Promise<{ status: number; body: any }...`

#### uat/journeys/machine-access.journey.ts

-  `machine` function L66 — `const machine = ()`

#### uat/journeys/multi-repo-agent.journey.ts

-  `repoContents` function L32-35 — `function repoContents(mcp: { call(t: string, a?: unknown): Promise<string> }, re...`
-  `codes` function L424 — `const codes = (hits: any)`
