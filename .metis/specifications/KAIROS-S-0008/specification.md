---
id: diataxis-documentation-spec
level: specification
...
Source of truth for the `diataxis-review` skill. Based on the Diataxis framework (Daniele Procida, diataxis.fr). Every rule below is checkable: a reviewer must be able to cite a rule ID and point to text on the page that satisfies or violates it. It renders into the skills plugin as `references/diataxis.md` (KAIROS-A-0014); this Metis document is the source of truth.
