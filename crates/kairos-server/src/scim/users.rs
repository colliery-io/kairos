...
/// Render one `public.users` row as a SCIM User resource. `userName` and
/// `externalId` are BOTH served from `users.external_id` (the OIDC sub) —
/// the outbound half of the identity-join contract.
...
/// The identity-join contract (module docs): externalId → external_id,
/// then userName → external_id, then email fallback, else create.
fn resolve_or_create_user(
    ...
    if let Some(external_id) = &payload.external_id {
        sub_candidates.push(external_id);
    }
    if !sub_candidates.contains(&payload.user_name.as_str()) {
        sub_candidates.push(&payload.user_name);
    }
    for candidate in &sub_candidates {
        let found: Option<User> = users::table
            .filter(users::external_id.eq(candidate))
...
pub(crate) async fn list_users(
    ...
    let external_id_filter = match &params.filter {
        None => None,
        Some(filter) => {
            let (attribute, value) = parse_eq_filter(filter)?;
            match attribute.as_str() {
                // Both are served from users.external_id (module docs).
                "username" | "externalid" => Some(value),
                other => {
    ...
    let response = run_scim(&state, &ctx.slug.clone(), move |conn| {
        ...
        let base = || {
            let mut query = organization_members::table
                ...
                .into_boxed();
            if let Some(value) = &external_id_filter {
...
fn parse_user_patch(body: &Value) -> Result<Vec<UserChange>, ScimError> {
    ...
    for operation in operations {
        ...
        match operation
            ...
            .as_deref()
        {
            // No path: the value object carries attributes. Unsupported
...
/// `userName`/`externalId` are immutable (the OIDC join key): a different
/// value is 400 `mutability`. `active: false` deprovisions.
pub(crate) async fn replace_user(
    ...
    let resource = run_scim(&state, &ctx.slug.clone(), move |conn| {
        scim_transaction(conn, |conn| {
            use kairos_db::schema::users;
            let (membership, user) = load_member(conn, ctx.org_id, user_id)?;
            if payload.user_name != user.external_id
                || payload
                    .external_id
                    .as_ref()
                    .is_some_and(|e| e != &user.external_id)
            {
                return Err(ScimError::mutability(
                    "userName/externalId are immutable: they carry the OIDC subject \
                     logins join on (KAIROS-A-0010)",
                ));
            }
            let display_name = payload
                .display_name
            ...
            if display_name != user.display_name || email != user.email {
                diesel::update(users::table.filter(users::id.eq(user.id)))
                    .set((
